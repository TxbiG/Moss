...
#include <Moss/Physics/Constraints/Constraint.h>
#include <Moss/Physics/Constraints/ContactConstraintManager.h>
...
{
	{
		...
		else
		{
			...
			MOSS_ASSERT(split_index < mNumSplits);
...
void LargeIslandSplitter::Splits::MarkBatchProcessed(uint inNumProcessed, bool &outLastIteration, bool &outFinalBatch)
{
	...
	{
		MOSS_ASSERT(total_items_processed == num_items_in_split); // Should not overflow, that means we're retiring more items than we should process
...
{
	MOSS_ASSERT(mSplitMasks == nullptr);
	MOSS_ASSERT(mContactAndConstraintsSplitIdx == nullptr);
	MOSS_ASSERT(mContactAndConstraintIndices == nullptr);
	MOSS_ASSERT(mSplitIslands == nullptr);
}
...
uint LargeIslandSplitter::AssignToNonParallelSplit(const Body *inBody)
{
	...
	{
		MOSS_ASSERT(idx < mNumActiveBodies);
...
bool LargeIslandSplitter::SplitIsland(uint32 inIslandIndex, const IslandBuilder &inIslandBuilder, const BodyManager &inBodyManager, const ContactConstraintManager &inContactManager, Constraint **inActiveConstraints, CalculateSolverSteps &ioStepsCalculator)
{
	...
	for (uint s = 0; s < cNumSplits; ++s)
	{
		...
		MOSS_ASSERT(contact_buffer_cur[s] == mContactAndConstraintIndices + split.mContactBufferEnd);
		MOSS_ASSERT(constraint_buffer_cur[s] == mContactAndConstraintIndices + split.mConstraintBufferEnd);
	}
...
void LargeIslandSplitter::Reset(TempAllocator *inTempAllocator)
{
	...
	MOSS_ASSERT(mContactAndConstraintsNextFree.load(memory_order_relaxed) == mContactAndConstraintsSize);
	MOSS_ASSERT(mNextSplitIsland.load(memory_order_relaxed) == mNumSplitIslands);
