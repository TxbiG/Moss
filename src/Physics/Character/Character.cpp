...
#include <Moss/Physics/Collision/CollideShape.h>
#include <Moss/Physics/Collision/Shape/RotatedTranslatedShape.h>
#include <Moss/Physics/Collision/Shape/ScaledShape.h>
...
const char *CharacterBase::ToString(EGroundState inState) {
	switch (inState)
	{
...
void CharacterBase::RestoreState(StateRecorder &inStream) {
	...
	mGroundMaterial = PhysicsMaterial::Default; // Cannot restore material
}
...
Character::Character(const CharacterSettings *inSettings, RVec3Arg inPosition, QuatArg inRotation, uint64 inUserData, PhysicsSystem *inSystem) : CharacterBase(inSettings, inSystem), mLayer(inSettings->mLayer) {
...
void Character::CheckCollision(RMat44Arg inCenterOfMassTransform, Vec3Arg inMovementDirection, float inMaxSeparationDistance, const Shape *inShape, 
	RVec3Arg inBaseOffset, CollideShapeCollector &ioCollector, bool inLockBodies) const {
	...
	sGetNarrowPhaseQuery(mSystem, inLockBodies).CollideShape(inShape, Vec3::One(), inCenterOfMassTransform, settings, inBaseOffset, ioCollector, broadphase_layer_filter, object_layer_filter, body_filter);
}

void Character::CheckCollision(RVec3Arg inPosition, QuatArg inRotation, Vec3Arg inMovementDirection, float inMaxSeparationDistance, const Shape *inShape, 
	RVec3Arg inBaseOffset, CollideShapeCollector &ioCollector, bool inLockBodies) const {
	// Calculate center of mass transform
	RMat44 center_of_mass = RMat44::RotationTranslation(inRotation, inPosition).PreTranslated(inShape->GetCenterOfMass());
...
void Character::PostSimulation(float inMaxSeparationDistance, bool inLockBodies) {
	...
	if (lock.Succeeded())
	{
		...
		// Update ground state
		RMat44 inv_transform = RMat44::InverseRotationTranslation(char_rot, char_pos);
	...
	else
	{
		...
		mGroundMaterial = PhysicsMaterial::Default;
		mGroundVelocity = Vec3::Zero();
		mGroundUserData = 0;
	}
}
...
void CharacterVsCharacterCollisionSimple::Remove(const CharacterVirtual *inCharacter) {
	TArray<CharacterVirtual *>::iterator i = std::find(mCharacters.begin(), mCharacters.end(), inCharacter);
	if (i != mCharacters.end())
		mCharacters.erase(i);
}

void CharacterVsCharacterCollisionSimple::CollideCharacter(const CharacterVirtual *inCharacter, RMat44Arg inCenterOfMassTransform, 
	const CollideShapeSettings &inCollideShapeSettings, RVec3Arg inBaseOffset, CollideShapeCollector &ioCollector) const {
	...
	AABox bounds1 = shape1->GetWorldSpaceBounds(transform1, Vec3::One());
	...
	for (const CharacterVirtual *c : mCharacters)
			...
			&& !ioCollector.ShouldEarlyOut())
		{
			...
			AABox bounds2 = shape2->GetWorldSpaceBounds(transform2, Vec3::One());
			bounds2.ExpandBy(Vec3::Replicate(settings.mMaxSeparationDistance));
			...
			CollisionDispatch::CollideShapeVsShape(shape1, shape2, Vec3::One(), Vec3::One(), transform1, transform2, SubShapeIDCreator(), SubShapeIDCreator(), settings, ioCollector);
		}
...
void CharacterVsCharacterCollisionSimple::CastCharacter(const CharacterVirtual *inCharacter, RMat44Arg inCenterOfMassTransform, Vec3Arg inDirection, 
	const ShapeCastSettings &inShapeCastSettings, RVec3Arg inBaseOffset, CastShapeCollector &ioCollector) const {
	// Convert shape cast relative to inBaseOffset
	Mat44 transform1 = inCenterOfMassTransform.PostTranslated(-inBaseOffset).ToMat44();
	ShapeCast shape_cast(inCharacter->GetShape(), Vec3::One(), transform1, inDirection);
	...
	for (const CharacterVirtual *c : mCharacters)
			...
			&& !ioCollector.ShouldEarlyOut())
		{
			...
			CollisionDispatch::CastShapeVsShapeWorldSpace(shape_cast, inShapeCastSettings, shape2, Vec3::One(), { }, transform2, SubShapeIDCreator(), SubShapeIDCreator(), ioCollector);
		}
...
void CharacterVirtual::GetAdjustedBodyVelocity(const Body& inBody, Vec3 &outLinearVelocity, Vec3 &outAngularVelocity) const {
	...
	{
		outLinearVelocity = outAngularVelocity = Vec3::Zero();
	}
...
Vec3 CharacterVirtual::CalculateCharacterGroundVelocity(RVec3Arg inCenterOfMass, Vec3Arg inLinearVelocity, Vec3Arg inAngularVelocity, float inDeltaTime) const {
	...
	Quat rotation = Quat::Rotation(inAngularVelocity / angular_velocity_len, angular_velocity_len * inDeltaTime);
...
void CharacterVirtual::FillContactProperties(const CharacterVirtual *inCharacter, Contact &outContact, const Body &inBody, Vec3Arg inUp, RVec3Arg inBaseOffset, 
	const taCollector &inCollector, const CollideShapeResult &inResult) {
	...
	outContact.mContactNormal = -inResult.mPenetrationAxis.NormalizedOr(Vec3::Zero());
...
void CharacterVirtual::FillCharacterContactProperties(Contact &outContact, const CharacterVirtual *inOtherCharacter, RVec3Arg inBaseOffset, const CollideShapeResult &inResult) {
	outContact.mPosition = inBaseOffset + inResult.mContactPointOn2;
	outContact.mLinearVelocity = inOtherCharacter->GetLinearVelocity();
	outContact.mSurfaceNormal = outContact.mContactNormal = -inResult.mPenetrationAxis.NormalizedOr(Vec3::Zero());
	outContact.mDistance = -inResult.mPenetrationDepth;
	...
	outContact.mMaterial = PhysicsMaterial::Default;
}
...
void CharacterVirtual::ContactCollector::AddHit(const CollideShapeResult &inResult) {
	...
	if (inResult.mBodyID2.IsInvalid())
	{
		...
		MOSS_ASSERT(mOtherCharacter != nullptr);
...
void CharacterVirtual::CheckCollision(RVec3Arg inPosition, QuatArg inRotation, Vec3Arg inMovementDirection, float inMaxSeparationDistance, 
	const Shape *inShape, RVec3Arg inBaseOffset, CollideShapeCollector &ioCollector, const BroadPhaseLayerFilter &inBroadPhaseLayerFilter, 
	...
	// Collide shape
	(mSystem->GetNarrowPhaseQuery().*collide_shape_function)(inShape, Vec3::One(), transform, settings, inBaseOffset, ioCollector, inBroadPhaseLayerFilter, inObjectLayerFilter, body_filter, inShapeFilter);
...
void CharacterVirtual::GetContactsAtPosition(RVec3Arg inPosition, Vec3Arg inMovementDirection, const Shape *inShape, TempContactList &outContacts, 
	const BroadPhaseLayerFilter &inBroadPhaseLayerFilter, const ObjectLayerFilter &inObjectLayerFilter, const BodyFilter &inBodyFilter, const ShapeFilter &inShapeFilter) const {
...
inline static bool sCorrectFractionForCharacterPadding(const Shape *inShape, Mat44Arg inStart, Vec3Arg inDisplacement, Vec3Arg inScale, const T &inPolygon, float &ioFraction) {
	...
	else if (inShape->GetSubType() == EShapeSubType::RotatedTranslated)
	{
		...
		return sCorrectFractionForCharacterPadding(rt_shape->GetInnerShape(), inStart * Mat44::Rotation(rt_shape->GetRotation()), inDisplacement, rt_shape->TransformScale(inScale), inPolygon, ioFraction);
	}
	...
	{
		MOSS_ASSERT(false, "Not supported yet!");
		return false;
	}
}

bool CharacterVirtual::GetFirstContactForSweep(RVec3Arg inPosition, Vec3Arg inDisplacement, Contact &outContact, const IgnoredContactList &inIgnoredContacts, 
	const BroadPhaseLayerFilter &inBroadPhaseLayerFilter, const ObjectLayerFilter &inObjectLayerFilter, const BodyFilter &inBodyFilter, const ShapeFilter &inShapeFilter) const {
	...
	RShapeCast shape_cast(mShape, Vec3::One(), start, inDisplacement);
	...
	if (face.size() >= 2)
	{
		...
		corrected = sCorrectFractionForCharacterPadding(mShape, start.GetRotation(), inDisplacement, Vec3::One(), add_cvx, outContact.mFraction);
	}
	if (!corrected)
	{
...
void CharacterVirtual::SolveConstraints(Vec3Arg inVelocity, float inDeltaTime, float inTimeRemaining, ConstraintList &ioConstraints, IgnoredContactList &ioIgnoredContacts, 
	...
	TArray<Constraint *, STLTempAllocator<Constraint *>> sorted_constraints(inAllocator);
	...
	for (uint32 iteration = 0; iteration < mMaxConstraintIterations; iteration++)
	{
		...
		if (inDrawConstraints)
		{
			...
			DebugRenderer::Instance->DrawPlane(offset, constraint->mPlane.GetNormal(), Color::Cyan, 1.0f);

			// Draw 2nd constraint plane
			if (other_constraint != nullptr)
				DebugRenderer::Instance->DrawPlane(offset, other_constraint->mPlane.GetNormal(), Color::Blue, 1.0f);

			// Draw starting velocity
			DebugRenderer::Instance->DrawArrow(offset, offset + velocity, Color::Green, 0.05f);

			// Draw resulting velocity
			DebugRenderer::Instance->DrawArrow(offset, offset + new_velocity, Color::Red, 0.05f);
		}
#endif // MOSS_DEBUG_RENDERER
...
void CharacterVirtual::UpdateSupportingContact(bool inSkipContactVelocityCheck, TempAllocator &inAllocator) {
	...
	RMat44 inv_transform = RMat44::InverseRotationTranslation(mRotation, mPosition);
...
void CharacterVirtual::MoveShape(RVec3 &ioPosition, Vec3Arg inVelocity, float inDeltaTime, ContactList *outActiveContacts, const BroadPhaseLayerFilter &inBroadPhaseLayerFilter, 
	const ObjectLayerFilter &inObjectLayerFilter, const BodyFilter &inBodyFilter, const ShapeFilter &inShapeFilter, TempAllocator &inAllocator
#ifndef MOSS_DEBUG_RENDERER
	, bool inDrawConstraints
#endif // MOSS_DEBUG_RENDERER
	) {
	MOSS_DET_LOG("CharacterVirtual::MoveShape: pos: " << ioPosition << " vel: " << inVelocity << " dt: " << inDeltaTime);
	...
	for (uint32 iteration = 0; iteration < mMaxCollisionIterations && time_remaining >= mMinTimeRemaining; iteration++)
	{
		...
		for (const Contact &c : contacts)
			MOSS_DET_LOG("contact: " << c.mPosition << " vel: " << c.mLinearVelocity << " cnormal: " << c.mContactNormal << " snormal: " << c.mSurfaceNormal << " dist: " << c.mDistance << " fraction: " << c.mFraction << " body: " << c.mBodyB << " subshape: " << c.mSubShapeIDB);
#endif // MOSS_ENABLE_DETERMINISM_LOG
		...
		if (draw_constraints)
		{
			...
			{
				// Draw contact point
				DebugRenderer::Instance->DrawMarker(c.mContact->mPosition, Color::Yellow, 0.05f);
				Vec3 dist_to_plane = -c.mPlane.GetConstant() * c.mPlane.GetNormal();

				// Draw arrow towards surface that we're hitting
				DebugRenderer::Instance->DrawArrow(c.mContact->mPosition, c.mContact->mPosition - dist_to_plane, Color::Yellow, 0.05f);

				// Draw plane around the player position indicating the space that we can move
				DebugRenderer::Instance->DrawPlane(mPosition + dist_to_plane, c.mPlane.GetNormal(), Color::Cyan, 1.0f);
				DebugRenderer::Instance->DrawArrow(mPosition + dist_to_plane, mPosition + dist_to_plane + c.mContact->mSurfaceNormal, Color::Red, 0.05f);
			}
		}
#endif // MOSS_DEBUG_RENDERER
...
void CharacterVirtual::StartTrackingContactChanges() {
	...
	MOSS_ASSERT(mListenerContacts.empty());
	if (mListener == nullptr)
		return;
...
void CharacterVirtual::FinishTrackingContactChanges() {
	...
	MOSS_ASSERT(count >= 0, "Called FinishTrackingContactChanges more times than StartTrackingContactChanges");
	if (count > 0)
		return;
	...
	for (const Contact &c : mActiveContacts)
		if (c.mHadCollision)
		{
			...
			MOSS_ASSERT(it != mListenerContacts.end());
			it->second.mCount = 1;
		}
...
void CharacterVirtual::Update(float inDeltaTime, Vec3Arg inGravity, const BroadPhaseLayerFilter &inBroadPhaseLayerFilter, const ObjectLayerFilter &inObjectLayerFilter, 
	...
	StartTrackingContactChanges();
	MOSS_SCOPE_EXIT([this]() { FinishTrackingContactChanges(); });
...
void CharacterVirtual::RefreshContacts(const BroadPhaseLayerFilter &inBroadPhaseLayerFilter, const ObjectLayerFilter &inObjectLayerFilter, 
	const BodyFilter &inBodyFilter, const ShapeFilter &inShapeFilter, TempAllocator &inAllocator) {
	// Determine the contacts
	TempContactList contacts(inAllocator);
	contacts.reserve(mMaxNumHits);
	GetContactsAtPosition(mPosition, mLinearVelocity.NormalizedOr(Vec3::Zero()), mShape, contacts, inBroadPhaseLayerFilter, inObjectLayerFilter, inBodyFilter, inShapeFilter);
...
void CharacterVirtual::MoveToContact(RVec3Arg inPosition, const Contact &inContact, const BroadPhaseLayerFilter &inBroadPhaseLayerFilter, 
	const ObjectLayerFilter &inObjectLayerFilter, const BodyFilter &inBodyFilter, const ShapeFilter &inShapeFilter, TempAllocator &inAllocator) {
	...
	StoreActiveContacts(contacts, inAllocator);
	MOSS_ASSERT(mGroundState != EGroundState::InAir);

	// Ensure that the rigid body ends up at the new position
	UpdateInnerBodyTransform();
}

bool CharacterVirtual::SetShape(const Shape *inShape, float inMaxPenetrationDepth, const BroadPhaseLayerFilter &inBroadPhaseLayerFilter, 
	const ObjectLayerFilter &inObjectLayerFilter, const BodyFilter &inBodyFilter, const ShapeFilter &inShapeFilter, TempAllocator &inAllocator) {
	...
	if (inShape != mShape && inShape != nullptr)
	{
		if (inMaxPenetrationDepth < FLT_MAX)
		{
			...
			TempContactList contacts(inAllocator);
			contacts.reserve(mMaxNumHits);
			GetContactsAtPosition(mPosition, mLinearVelocity.NormalizedOr(Vec3::Zero()), inShape, contacts, inBroadPhaseLayerFilter, inObjectLayerFilter, inBodyFilter, inShapeFilter);
...
bool CharacterVirtual::WalkStairs(float inDeltaTime, Vec3Arg inStepUp, Vec3Arg inStepForward, Vec3Arg inStepForwardTest, Vec3Arg inStepDownExtra, 
	const BroadPhaseLayerFilter &inBroadPhaseLayerFilter, const ObjectLayerFilter &inObjectLayerFilter, const BodyFilter &inBodyFilter, const ShapeFilter &inShapeFilter, TempAllocator &inAllocator) {
	StartTrackingContactChanges();
	MOSS_SCOPE_EXIT([this]() { FinishTrackingContactChanges(); });
	...
	// Draw sweep up
	if (sDrawWalkStairs)
		DebugRenderer::Instance->DrawArrow(mPosition, up_position, Color::White, 0.01f);
#endif // MOSS_DEBUG_RENDERER
	...
	TArray<Vec3, STLTempAllocator<Vec3>> steep_slope_normals(inAllocator);
	...
	if (sDrawWalkStairs)
		DebugRenderer::Instance->DrawArrow(up_position, new_position, Color::White, 0.01f);
#endif // MOSS_DEBUG_RENDERER
	...
	if (sDrawWalkStairs)
	{
		...
		DebugRenderer::Instance->DrawArrow(new_position, debug_pos, Color::White, 0.01f);
		DebugRenderer::Instance->DrawArrow(contact.mPosition, contact.mPosition + contact.mSurfaceNormal, Color::White, 0.01f);
		mShape->Draw(DebugRenderer::Instance, GetCenterOfMassTransform(debug_pos, mRotation, mShape), Vec3::One(), Color::White, false, true);
	}
#endif // MOSS_DEBUG_RENDERER
	...
	if (IsSlopeTooSteep(contact.mSurfaceNormal))
	{
		...
		if (sDrawWalkStairs)
			DebugRenderer::Instance->DrawArrow(up_position, test_position, Color::Cyan, 0.01f);
	#endif // MOSS_DEBUG_RENDERER
		...
		if (sDrawWalkStairs)
		{
			...
			DebugRenderer::Instance->DrawArrow(test_position, debug_pos, Color::Cyan, 0.01f);
			DebugRenderer::Instance->DrawArrow(test_contact.mPosition, test_contact.mPosition + test_contact.mSurfaceNormal, Color::Cyan, 0.01f);
			mShape->Draw(DebugRenderer::Instance, GetCenterOfMassTransform(debug_pos, mRotation, mShape), Vec3::One(), Color::Cyan, false, true);
		}
	#endif // MOSS_DEBUG_RENDERER
...
bool CharacterVirtual::StickToFloor(Vec3Arg inStepDown, const BroadPhaseLayerFilter &inBroadPhaseLayerFilter, const ObjectLayerFilter &inObjectLayerFilter, 
	const BodyFilter &inBodyFilter, const ShapeFilter &inShapeFilter, TempAllocator &inAllocator) {
	StartTrackingContactChanges();
	MOSS_SCOPE_EXIT([this]() { FinishTrackingContactChanges(); });
	...
	{
		DebugRenderer::Instance->DrawArrow(mPosition, new_position, Color::Orange, 0.01f);
		mShape->Draw(DebugRenderer::Instance, GetCenterOfMassTransform(new_position, mRotation, mShape), Vec3::One(), Color::Orange, false, true);
	}
#endif // MOSS_DEBUG_RENDERER
...
void CharacterVirtual::ExtendedUpdate(float inDeltaTime, Vec3Arg inGravity, const ExtendedUpdateSettings &inSettings, const BroadPhaseLayerFilter &inBroadPhaseLayerFilter, 
	const ObjectLayerFilter &inObjectLayerFilter, const BodyFilter &inBodyFilter, const ShapeFilter &inShapeFilter, TempAllocator &inAllocator) {
	StartTrackingContactChanges();
	MOSS_SCOPE_EXIT([this]() { FinishTrackingContactChanges(); });
...
void CharacterVirtual::Contact::RestoreState(StateRecorder &inStream) {
	...
	mMaterial = PhysicsMaterial::Default; // Cannot restore material
}
