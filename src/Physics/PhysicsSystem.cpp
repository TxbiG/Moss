...
#include <Moss/Physics/PhysicsStepListener.h>
#include <Moss/Physics/Collision/BroadPhase/BroadPhaseBruteForce.h>
#include <Moss/Physics/Collision/BroadPhase/BroadPhaseQuadTree.h>
#include <Moss/Physics/Collision/CollisionDispatch.h>
#include <Moss/Physics/Collision/AABoxCast.h>
...
#include <Moss/Physics/Collision/ManifoldBetweenTwoFaces.h>
...
static const Color cColorUpdateBroadPhaseFinalize = Color::GetDistinctColor(1);
static const Color cColorUpdateBroadPhasePrepare = Color::GetDistinctColor(2);
static const Color cColorFindCollisions = Color::GetDistinctColor(3);
static const Color cColorApplyGravity = Color::GetDistinctColor(4);
static const Color cColorSetupVelocityConstraints = Color::GetDistinctColor(5);
static const Color cColorBuildIslandsFromConstraints = Color::GetDistinctColor(6);
static const Color cColorDetermineActiveConstraints = Color::GetDistinctColor(7);
static const Color cColorFinalizeIslands = Color::GetDistinctColor(8);
static const Color cColorContactRemovedCallbacks = Color::GetDistinctColor(9);
static const Color cColorBodySetIslandIndex = Color::GetDistinctColor(10);
static const Color cColorStartNextStep = Color::GetDistinctColor(11);
static const Color cColorSolveVelocityConstraints = Color::GetDistinctColor(12);
static const Color cColorPreIntegrateVelocity = Color::GetDistinctColor(13);
static const Color cColorIntegrateVelocity = Color::GetDistinctColor(14);
static const Color cColorPostIntegrateVelocity = Color::GetDistinctColor(15);
static const Color cColorResolveCCDContacts = Color::GetDistinctColor(16);
static const Color cColorSolvePositionConstraints = Color::GetDistinctColor(17);
static const Color cColorFindCCDContacts = Color::GetDistinctColor(18);
static const Color cColorStepListeners = Color::GetDistinctColor(19);
static const Color cColorSoftBodyPrepare = Color::GetDistinctColor(20);
static const Color cColorSoftBodyCollide = Color::GetDistinctColor(21);
static const Color cColorSoftBodySimulate = Color::GetDistinctColor(22);
static const Color cColorSoftBodyFinalize = Color::GetDistinctColor(23);
...
void PhysicsSystem::AddStepListener(PhysicsStepListener *inListener)
{
	...
	MOSS_ASSERT(std::find(mStepListeners.begin(), mStepListeners.end(), inListener) == mStepListeners.end());
...
void PhysicsSystem::RemoveStepListener(PhysicsStepListener *inListener)
{
	...
	MOSS_ASSERT(i != mStepListeners.end());
...
EPhysicsUpdateError PhysicsSystem::Update(float inDeltaTime, int inCollisionSteps, TempAllocator *inTempAllocator, JobSystem *inJobSystem)
{
	...
	MOSS_DET_LOG("PhysicsSystem::Update: dt: " << inDeltaTime << " steps: " << inCollisionSteps);

	MOSS_ASSERT(inCollisionSteps > 0);
	MOSS_ASSERT(inDeltaTime >= 0.0f);
	...
	MOSS_ASSERT(context.mBodyPairs == nullptr);
...

	{
		...
		for (int step_idx = 0; step_idx < inCollisionSteps; ++step_idx)
		{
			...
			step.mUpdateBroadphaseFinalize = inJobSystem->CreateJob("UpdateBroadPhaseFinalize", cColorUpdateBroadPhaseFinalize, [&context, &step]()
				{
					...
					MOSS_ASSERT(step.mActiveFindCollisionJobs.load(memory_order_relaxed) == 0);
			...
			if (is_first_step)
			{
				...
				MOSS_ASSERT(context.mActiveConstraints == nullptr);
				...
				step.mApplyGravity[i] = inJobSystem->CreateJob("ApplyGravity", cColorApplyGravity, [&context, &step]()
					{
						...
						JobHandle::RemoveDependencies(step.mFindCollisions);
				...
				step.mSetupVelocityConstraints[i] = inJobSystem->CreateJob("SetupVelocityConstraints", cColorSetupVelocityConstraints, [&context, &step]()
					{
						...
						JobHandle::RemoveDependencies(step.mSolveVelocityConstraints);
				...
				step.mDetermineActiveConstraints[i] = inJobSystem->CreateJob("DetermineActiveConstraints", cColorDetermineActiveConstraints, [&context, &step]()
					{
						...
						JobHandle::RemoveDependencies(step.mSetupVelocityConstraints);
						JobHandle::RemoveDependencies(step.mFindCollisions);
					}, num_step_listener_jobs > 0? num_step_listener_jobs : previous_step_dependency_count); // depends on: step listeners (or previous step if no step listeners)
				...
				step.mStepListeners[i] = inJobSystem->CreateJob("StepListeners", cColorStepListeners, [&context, &step]()
					{
						...
						JobHandle::RemoveDependencies(step.mApplyGravity);
						JobHandle::RemoveDependencies(step.mDetermineActiveConstraints);
					}, previous_step_dependency_count);
			...
			step.mBodySetIslandIndex = inJobSystem->CreateJob("BodySetIslandIndex", cColorBodySetIslandIndex, [&context, &step]()
				{
					...
					JobHandle::RemoveDependencies(step.mSolvePositionConstraints);
				...
				step.mStartNextStep = inJobSystem->CreateJob("StartNextStep", cColorStartNextStep, [this, next_step]()
					{
						...
						if (next_step->mStepListeners.empty())
						{
							...
							JobHandle::RemoveDependencies(next_step->mApplyGravity);
							JobHandle::RemoveDependencies(next_step->mDetermineActiveConstraints);
						}
						else
						{
							// Kick the step listeners job first
							JobHandle::RemoveDependencies(next_step->mStepListeners);
						}
			...
			step.mPreIntegrateVelocity = inJobSystem->CreateJob("PreIntegrateVelocity", cColorPreIntegrateVelocity, [&context, &step]()
				{
					...
					JobHandle::RemoveDependencies(step.mIntegrateVelocity);
	...
	{
		MOSS_PROFILE("Build job barrier");

		TStaticArray<JobHandle, cMaxPhysicsJobs> handles;
	...
	MOSS_ASSERT(errors == EPhysicsUpdateError::None, "An error occurred during the physics update, see EPhysicsUpdateError for more information");
	return errors;
}
...
void PhysicsSystem::JobDetermineActiveConstraints(PhysicsUpdateContext::Step *ioStep) const
{
	...
	Constraint **active_constraints = (Constraint **)MOSS_STACK_ALLOC(cDetermineActiveConstraintsBatchSize * sizeof(Constraint *));
...
void PhysicsSystem::JobSetupVelocityConstraints(float inDeltaTime, PhysicsUpdateContext::Step *ioStep) const
{
	...
	for (;;)
	{
		...
		ConstraintManager::SetupVelocityConstraints(ioStep->mContext->mActiveConstraints + constraint_idx, min<uint32>(cSetupVelocityConstraintsBatchSize, num_constraints - constraint_idx), inDeltaTime);
	}
}
...
void PhysicsSystem::JobBuildIslandsFromConstraints(PhysicsUpdateContext *ioContext, PhysicsUpdateContext::Step *ioStep)
{
	...
	// Build the islands
	ConstraintManager::BuildIslands(ioStep->mContext->mActiveConstraints, ioStep->mNumActiveConstraints, mIslandBuilder, mBodyManager);
}
...
void PhysicsSystem::JobFindCollisions(PhysicsUpdateContext::Step *ioStep, int inJobIndex)
{
	...
	BodyID *active_bodies = (BodyID *)MOSS_STACK_ALLOC(cActiveBodiesBatchSize * sizeof(BodyID));
...
void PhysicsSystem::DefaultSimCollideBodyVsBody(const Body &inBody1, const Body &inBody2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, CollideShapeSettings &ioCollideShapeSettings, CollideShapeCollector &ioCollector, const ShapeFilter &inShapeFilter)
{
	...
	if (inBody1.GetEnhancedInternalEdgeRemovalWithBody(inBody2))
	{
		...
		InternalEdgeRemovingCollector::CollideShapeVsShape(inBody1.GetShape(), inBody2.GetShape(), Vec3::One(), Vec3::One(), inCenterOfMassTransform1, inCenterOfMassTransform2, part1, part2, ioCollideShapeSettings, ioCollector, inShapeFilter);
	}
	else
	{
		// Regular collide
		CollisionDispatch::CollideShapeVsShape(inBody1.GetShape(), inBody2.GetShape(), Vec3::One(), Vec3::One(), inCenterOfMassTransform1, inCenterOfMassTransform2, part1, part2, ioCollideShapeSettings, ioCollector, inShapeFilter);
	}
}
...
void PhysicsSystem::ProcessBodyPair(ContactAllocator &ioContactAllocator, const BodyPair &inBodyPair)
{
	...
	MOSS_ASSERT(body1->IsActive());

	MOSS_DET_LOG("ProcessBodyPair: id1: " << inBodyPair.mBodyA << " id2: " << inBodyPair.mBodyB << " p1: " << body1->GetCenterOfMassPosition() << " p2: " << body2->GetCenterOfMassPosition() << " r1: " << body1->GetRotation() << " r2: " << body2->GetRotation());
	...
	if (!pair_handled)
	{
		...
		Mat44 transform1 = Mat44::Rotation(body1->GetRotation());
			...
			&& body1->GetUseManifoldReductionWithBody(*body2))	// Check body flag
		{
			...
			using Manifolds = TStaticArray<MyManifold, 32>;
			...
			public:
				...
				virtual void	AddHit(const CollideShapeResult &inResult) override
				{
					...
					MOSS_ASSERT(mBody1->GetMotionType() >= mBody2->GetMotionType());
					MOSS_ASSERT(!ShouldEarlyOut());
					...
					ManifoldBetweenTwoFaces(inResult.mContactPointOn1, inResult.mContactPointOn2, inResult.mPenetrationAxis, settings.mSpeculativeContactDistance + settings.mManifoldTolerance, inResult.mShape1Face, inResult.mShape2Face, manifold->mRelativeContactPointsOn1, manifold->mRelativeContactPointsOn2 MOSS_IF_DEBUG_RENDERER(, mBody1->GetCenterOfMassPosition()));
					...
					if (manifold->mRelativeContactPointsOn1.size() > 32)
						PruneContactPoints(manifold->mFirstWorldSpaceNormal, manifold->mRelativeContactPointsOn1, manifold->mRelativeContactPointsOn2 MOSS_IF_DEBUG_RENDERER(, manifold->mBaseOffset));
				}
			...
			for (ContactManifold &manifold : collector.mManifolds)
			{
				...
				if (manifold.mRelativeContactPointsOn1.size() > 4)
					PruneContactPoints(manifold.mWorldSpaceNormal, manifold.mRelativeContactPointsOn1, manifold.mRelativeContactPointsOn2 MOSS_IF_DEBUG_RENDERER(, manifold.mBaseOffset));
		...
		else
		{
			...
			public:
				...
				virtual void	AddHit(const CollideShapeResult &inResult) override
				{
					...
					const PhysicsSettings &settings = mSystem->mPhysicsSettings;
					ManifoldBetweenTwoFaces(inResult.mContactPointOn1, inResult.mContactPointOn2, inResult.mPenetrationAxis, settings.mSpeculativeContactDistance + settings.mManifoldTolerance, inResult.mShape1Face, inResult.mShape2Face, manifold.mRelativeContactPointsOn1, manifold.mRelativeContactPointsOn2 MOSS_IF_DEBUG_RENDERER(, manifold.mBaseOffset));
...
MOSS_CLANG_SUPPRESS_WARNING("-Wundefined-func-template") // ConstraintManager::WarmStartVelocityConstraints / ContactConstraintManager::WarmStartVelocityConstraints is instantiated in the cpp file
	...
	for (;;)
	{
			...
			case LargeIslandSplitter::EStatus::BatchRetrieved:
				{
					if (first_iteration)
					{
						...
						ConstraintManager::WarmStartVelocityConstraints(active_constraints, constraints_begin, constraints_end, warm_start_impulse_ratio, dummy);
						mContactManager.WarmStartVelocityConstraints(contacts_begin, contacts_end, warm_start_impulse_ratio, dummy);
					}
					else
					{
						// Solve velocity constraints
						ConstraintManager::SolveVelocityConstraints(active_constraints, constraints_begin, constraints_end, delta_time);
		...
		if (check_islands)
		{
			...
			if (!has_contacts && !has_constraints)
			{
				...
				{
					MOSS_ASSERT(!mIslandBuilder.GetConstraintsInIsland(island_idx, constraints_begin, constraints_end));
					MOSS_ASSERT(!mIslandBuilder.GetContactsInIsland(island_idx, contacts_begin, contacts_end));
				}
			#endif // MOSS_DEBUG
			...
			if (mPhysicsSettings.mDeterministicSimulation)
			{
				...
				ConstraintManager::SortConstraints(active_constraints, constraints_begin, constraints_end);
			...
			ConstraintManager::WarmStartVelocityConstraints(active_constraints, constraints_begin, constraints_end, warm_start_impulse_ratio, steps_calculator);
...
void PhysicsSystem::JobPreIntegrateVelocity(PhysicsUpdateContext *ioContext, PhysicsUpdateContext::Step *ioStep)
{
	...
	MOSS_ASSERT(ioStep->mCCDBodies == nullptr);
	...
	MOSS_ASSERT(ioStep->mActiveBodyToCCDBody == nullptr);
...
void PhysicsSystem::JobIntegrateVelocity(const PhysicsUpdateContext *ioContext, PhysicsUpdateContext::Step *ioStep)
{
	...
	for (;;)
	{
		...
		while (active_body_idx < active_body_idx_end)
		{
			...
			MOSS_DET_LOG("JobIntegrateVelocity: id: " << body_id << " v: " << body.GetLinearVelocity() << " w: " << body.GetAngularVelocity());
			...
			case EMotionQuality::LinearCast:
					...
					&& !body.IsSensor()) // We don't support CCD sensors
				{
					...
					MOSS_ASSERT(inner_radius > 0.0f, "The shape has no inner radius, this makes the shape unsuitable for the linear cast motion quality as we cannot move it without risking tunneling.");
					...
					if (delta_pos.LengthSq() > linear_cast_threshold_sq)
					{
						...
						MOSS_ASSERT(active_body_idx < ioStep->mNumActiveBodyToCCDBody);
...
void PhysicsSystem::JobPostIntegrateVelocity(PhysicsUpdateContext *ioContext, PhysicsUpdateContext::Step *ioStep) const
{
	...
	MOSS_ASSERT(ioStep->mNumCCDBodies <= mBodyManager.GetNumActiveCCDBodies());
...
inline static PhysicsUpdateContext::Step::CCDBody *sGetCCDBody(const Body &inBody, PhysicsUpdateContext::Step *inStep)
{
	...
	MOSS_ASSERT(active_index < inStep->mNumActiveBodyToCCDBody); // Ensure that the body has a mapping to CCD body
	...
	MOSS_ASSERT(ccd_body->mBodyID1 == inBody.GetID(), "We found the wrong CCD body!");
	return ccd_body;
}
...
void PhysicsSystem::JobFindCCDContacts(const PhysicsUpdateContext *ioContext, PhysicsUpdateContext::Step *ioStep)
{
	...
	for (;;)
	{
		...
		if (sDrawMotionQualityLinearCast)
		{
			...
			body.GetShape()->Draw(DebugRenderer::Instance, com, Vec3::One(), Color::Green, false, true);
			DebugRenderer::Instance->DrawArrow(com.GetTranslation(), com.GetTranslation() + ccd_body.mDeltaPosition, Color::Green, 0.1f);
			body.GetShape()->Draw(DebugRenderer::Instance, com.PostTranslated(ccd_body.mDeltaPosition), Vec3::One(), Color::Red, false, true);
		}
	#endif // MOSS_DEBUG_RENDERER
		...
		public:
			...
			virtual void				AddHit(const BroadPhaseCastResult &inResult) override
			{
				...
				MOSS_ASSERT(inResult.mFraction <= GetEarlyOutFraction(), "This hit should not have been passed on to the collector");
		...
		RShapeCast shape_cast(body.GetShape(), Vec3::One(), body.GetCenterOfMassTransform(), ccd_body.mDeltaPosition);
		...
		if (ccd_body.mFractionPlusSlop < 1.0f)
		{
			...
			else
			{
				...
				{
					Vec3 average_contact_point = Vec3::Zero();
...
void PhysicsSystem::JobResolveCCDContacts(PhysicsUpdateContext *ioContext, PhysicsUpdateContext::Step *ioStep)
{
	...
	if (num_ccd_bodies > 0)
	{
		...
		MOSS_SCOPE_EXIT([temp_allocator, sorted_ccd_bodies, num_ccd_bodies]{ temp_allocator->Free(sorted_ccd_bodies, num_ccd_bodies * sizeof(CCDBody *)); });
		{
			MOSS_PROFILE("Sort");
		...
		static constexpr int cBodiesBatch = 64;
		BodyID *bodies_to_activate = (BodyID *)MOSS_STACK_ALLOC(cBodiesBatch * sizeof(BodyID));
		...
		for (uint32 i = 0; i < num_ccd_bodies; ++i)
		{
			...
			if (!ccd_body->mBodyID2.IsInvalid())
			{
				...
				{
					MOSS_ASSERT(ccd_body2->mBodyID2 != ccd_body->mBodyID1, "If we collided with another body, that other body should have ignored collisions with us!");
				...
				if (ccd_body2 == nullptr || ccd_body2->mFraction >= ccd_body->mFraction)
				{
					...
					if (sDrawMotionQualityLinearCast)
					{
						...
						body1.GetShape()->Draw(DebugRenderer::Instance, collision_transform, Vec3::One(), Color::Yellow, false, true);
						...
						body1.GetShape()->Draw(DebugRenderer::Instance, collision_transform_plus_slop, Vec3::One(), Color::Orange, false, true);

						// Draw contact normal
						DebugRenderer::Instance->DrawArrow(ccd_body->mContactPointOn2, ccd_body->mContactPointOn2 - ccd_body->mContactNormal, Color::Yellow, 0.1f);

						// Draw post contact velocity
						DebugRenderer::Instance->DrawArrow(collision_transform.GetTranslation(), collision_transform.GetTranslation() + body1.GetLinearVelocity(), Color::Orange, 0.1f);
						DebugRenderer::Instance->DrawArrow(collision_transform.GetTranslation(), collision_transform.GetTranslation() + body1.GetAngularVelocity(), Color::Purple, 0.1f);
					}
				#endif // MOSS_DEBUG_RENDERER
				}
			}
...
void PhysicsSystem::JobSolvePositionConstraints(PhysicsUpdateContext *ioContext, PhysicsUpdateContext::Step *ioStep)
{
	...
	BodiesToSleep bodies_to_sleep(mBodyManager, (BodyID *)MOSS_STACK_ALLOC(BodiesToSleep::cBodiesToSleepSize * sizeof(BodyID)));
	...
	for (;;)
	{
		...
		if (check_split_islands)
		{
			...
			case LargeIslandSplitter::EStatus::BatchRetrieved:
				// Solve the batch
				ConstraintManager::SolvePositionConstraints(active_constraints, constraints_begin, constraints_end, delta_time, baumgarte);
...
void PhysicsSystem::JobSoftBodyFinalize(PhysicsUpdateContext *ioContext)
{
	...
	BodyID *bodies_to_update_bounds = (BodyID *)MOSS_STACK_ALLOC(cBodiesBatch * sizeof(BodyID));
	int num_bodies_to_update_bounds = 0;
	BodyID *bodies_to_put_to_sleep = (BodyID *)MOSS_STACK_ALLOC(cBodiesBatch * sizeof(BodyID));
...
bool PhysicsSystem::RestoreState(StateRecorder &inStream, const StateRecorderFilter *inFilter)
{
	...
	if (uint8(state) & uint8(EStateRecorderState::Bodies))
	{
		...
		{
			TArray<BodyID> bodies;
			for (const Body *b : mBodyManager.GetBodies())
				if (BodyManager::IsValidBodyPointer(b) && b->IsInBroadPhase())
