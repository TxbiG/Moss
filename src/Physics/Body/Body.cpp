...
void MotionProperties::SetMassProperties(EAllowedDOFs inAllowedDOFs, const MassProperties &inMassProperties)
{
	...
	if (allowed_rotation_axis == 0)
	{
		...
		mInvInertiaDiagonal = Vec3::Zero();
		mInertiaRotation = Quat::Identity();
	}
	else
	{
		// Set inverse inertia
		...
		else
		{
			...
			mInvInertiaDiagonal = Vec3::Replicate(2.5f * mInvMass);
			mInertiaRotation = Quat::Identity();
		}
	}

	MOSS_ASSERT(mInvMass != 0.0f || mInvInertiaDiagonal != Vec3::Zero(), "Can't lock all axes, use a static body for this. This will crash with a division by zero later!");
}
...
bool MassProperties::DecomposePrincipalMomentsOfInertia(Mat44 &outRotation, Vec3 &outDiagonal) const
{
	...
	Matrix<3, 3> eigen_vec = Matrix<3, 3>::Identity();
	TVec<3> eigen_val;
	...
	Mat44 new_inertia = outRotation * Mat44::Scale(outDiagonal) * outRotation.Inversed();
	for (int i = 0; i < 3; ++i)
...
Vec3 MassProperties::GetEquivalentSolidBoxSize(float inMass, Vec3Arg inInertiaDiagonal)
{
...
void MassProperties::Scale(Vec3Arg inScale)
{
	...
	Vec3 xyz_sq = Vec3::Replicate(Vec3::Replicate(0.5f).Dot(diagonal)) - diagonal;
...
void MassProperties::Translate(Vec3Arg inTranslation)
{
	...
	mInertia += mMass * (Mat44::Scale(inTranslation.Dot(inTranslation)) - Mat44::OuterProduct(inTranslation, inTranslation));
...
void Body::SetMotionType(EMotionType inMotionType)
{
	...
	if (mMotionProperties != nullptr)
	{
		...
		case EMotionType::Static:
			// Stop the object
			mMotionProperties->mLinearVelocity = Vec3::Zero();
			mMotionProperties->mAngularVelocity = Vec3::Zero();
			[[fallthrough]];
...
{
	mBounds = mShape->GetWorldSpaceBounds(GetCenterOfMassTransform(), Vec3::One());
}
...
void Body::GetSubmergedVolume(RVec3Arg inSurfacePosition, Vec3Arg inSurfaceNormal, float &outTotalVolume, float &outSubmergedVolume, Vec3 &outRelativeCenterOfBuoyancy) const
{
	...
	Plane surface_relative_to_body = Plane::FromPointAndNormal(inSurfacePosition - mPosition, inSurfaceNormal);

	// Calculate amount of volume that is submerged and what the center of buoyancy is
	mShape->GetSubmergedVolume(rotation, Vec3::One(), surface_relative_to_body, outTotalVolume, outSubmergedVolume, outRelativeCenterOfBuoyancy MOSS_IF_DEBUG_RENDERER(, mPosition));
}
...
bool Body::ApplyBuoyancyImpulse(float inTotalVolume, float inSubmergedVolume, Vec3Arg inRelativeCenterOfBuoyancy, float inBuoyancy, float inLinearDrag, float inAngularDrag, Vec3Arg inFluidVelocity, Vec3Arg inGravity, float inDeltaTime)
{
	...
	if (inSubmergedVolume > 0.0f)
	{
		...
		if (Shape::DrawSubmergedVolumes)
		{
			...
			DebugRenderer::Instance->DrawMarker(center_of_buoyancy, Color::White, 2.0f);
			DebugRenderer::Instance->DrawText3D(center_of_buoyancy, StringFormat("%.3f / %.3f", (double)inSubmergedVolume, (double)inTotalVolume));
		}
	#endif // MOSS_DEBUG_RENDERER
...
BodyCreationSettings Body::GetBodyCreationSettings() const
{
	...
	result.mLinearVelocity = mMotionProperties != nullptr? mMotionProperties->GetLinearVelocity() : Vec3::Zero();
	result.mAngularVelocity = mMotionProperties != nullptr? mMotionProperties->GetAngularVelocity() : Vec3::Zero();
	...
	if (mMotionProperties != nullptr)
	{
		...
		else
		{
			...
			Vec3 diagonal = Vec3::Max(inv_inertia.GetDiagonal3(), Vec3::Replicate(FLT_MIN));
			result.mMassPropertiesOverride.mInertia = Mat44::Scale(diagonal.Reciprocal());
		}
	}
	else
	{
		result.mMassPropertiesOverride.mMass = FLT_MAX;
		result.mMassPropertiesOverride.mInertia = Mat44::Scale(Vec3::Replicate(FLT_MAX));
	}
...
SoftBodyCreationSettings Body::GetSoftBodyCreationSettings() const
{
	...
	void BodyManager::SetOverrideAllowActivation(bool inValue)
	{
	...
	void BodyManager::SetOverrideAllowDeactivation(bool inValue)
	{
...
inline void BodyManager::DeleteBody(Body *inBody)
{
...
Body *BodyManager::AllocateSoftBody(const SoftBodyCreationSettings &inSoftBodyCreationSettings) const
{
	...
	mp->SetLinearVelocity(Vec3::Zero());
	mp->SetAngularVelocity(Vec3::Zero());
	...
	body->SetPositionAndRotationInternal(inSoftBodyCreationSettings.mPosition, inSoftBodyCreationSettings.mMakeRotationIdentity? Quat::Identity() : inSoftBodyCreationSettings.mRotation);
	...
	for (const BodyID *b = inBodyIDs, *b_end = inBodyIDs + inNumber; b < b_end; b++)
		if (!b->IsInvalid())
		{
				...
				&& body.mMotionProperties->mIndexInActiveBodies != Body::cInactiveIndex)
			{
				...
				// Reset velocity
				body.mMotionProperties->mLinearVelocity = Vec3::Zero();
				body.mMotionProperties->mAngularVelocity = Vec3::Zero();
...
{
	MOSS_IF_ENABLE_ASSERTS(PhysicsLock::CheckLock(this, EPhysicsLockTypes::PerBody));
...
{
	MOSS_IF_ENABLE_ASSERTS(PhysicsLock::CheckUnlock(this, EPhysicsLockTypes::PerBody));
...
void BodyManager::LockAllBodies() const
{
	...
	PhysicsLock::Lock(mBodiesMutex MOSS_IF_ENABLE_ASSERTS(, this, EPhysicsLockTypes::BodiesList));
}

void BodyManager::UnlockAllBodies() const
{
	PhysicsLock::Unlock(mBodiesMutex MOSS_IF_ENABLE_ASSERTS(, this, EPhysicsLockTypes::BodiesList));
		...
		if (sIsValidBodyPointer(body) && body->IsInBroadPhase() && (!inBodyFilter || inBodyFilter->ShouldDraw(*body)))
		{
			...
			if (is_sensor)
				...
				case EShapeColor::InstanceColor:
					...
					color = Color::GetDistinctColor(body->mID.GetIndex());
					break;

				case EShapeColor::ShapeTypeColor:
					color = ShapeFunctions::Get(body->GetShape()->GetSubType()).mColor;
					break;
				...
				case EShapeColor::SleepColor:
					...
					case EMotionType::Kinematic:
						color = body->IsActive()? Color::Green : Color::Red;
						break;

					case EMotionType::Dynamic:
						color = body->IsActive()? Color::Yellow : Color::Red;
						break;
					...
					case EMotionType::Dynamic:
						{
							...
							color = idx != Body::cInactiveIndex? Color::GetDistinctColor(idx) : Color::LightGrey;
						}
						break;
			...
			if (inDrawSettings.mDrawGetSupportFunction)
				body->mShape->DrawGetSupportFunction(inRenderer, body->GetCenterOfMassTransform(), Vec3::One(), color, inDrawSettings.mDrawSupportDirection);

			// Draw the results of GetSupportingFace
			if (inDrawSettings.mDrawGetSupportingFace)
				body->mShape->DrawGetSupportingFace(inRenderer, body->GetCenterOfMassTransform(), Vec3::One());

			// Draw the shape
			if (inDrawSettings.mDrawShape)
				body->mShape->Draw(inRenderer, body->GetCenterOfMassTransform(), Vec3::One(), color, inDrawSettings.mDrawShapeColor == EShapeColor::MaterialColor, inDrawSettings.mDrawShapeWireframe || is_sensor);
			...
			if (inDrawSettings.mDrawVelocity)
			{
				...
				inRenderer->DrawArrow(pos, pos + body->GetLinearVelocity(), Color::Green, 0.1f);
				inRenderer->DrawArrow(pos, pos + body->GetAngularVelocity(), Color::Red, 0.1f);
			}
			...
			if (inDrawSettings.mDrawMassAndInertia && body->IsDynamic())
			{
				...
				if (mp->GetInverseMass() > 0.0f
					&& !Vec3::Equals(mp->GetInverseInertiaDiagonal(), Vec3::Zero()).TestAnyXYZTrue())
				{
					// Invert mass again
					...
					Vec3 box_size = MassProperties::GetEquivalentSolidBoxSize(mass, diagonal);

					// Draw box with equivalent inertia
					inRenderer->DrawWireBox(body->GetCenterOfMassTransform() * Mat44::Rotation(mp->GetInertiaRotation()), AABox(-0.5f * box_size, 0.5f * box_size), Color::Orange);

					// Draw mass
					inRenderer->DrawText3D(body->GetCenterOfMassPosition(), StringFormat("%.2f", (double)mass), Color::Orange, 0.2f);
				}
			}
...
BodyCreationSettings::BCSResult BodyCreationSettings::RestoreWithChildren(StreamIn &inStream, IDToShapeMap &ioShapeMap, IDToMaterialMap &ioMaterialMap, IDToGroupFilterMap &ioGroupFilterMap)
{
	BCSResult result;
	...
	// Read shape
	Shape::ShapeResult shape_result = Shape::RestoreWithChildren(inStream, ioShapeMap, ioMaterialMap);
...
TwoBodyConstraint *BodyInterface::CreateConstraint(const TwoBodyConstraintSettings *inSettings, const BodyID &inBodyID1, const BodyID &inBodyID2)
{
	...
	return inSettings->Create(body1 != nullptr? *body1 : Body::FixedToWorld, body2 != nullptr? *body2 : Body::FixedToWorld);
}
