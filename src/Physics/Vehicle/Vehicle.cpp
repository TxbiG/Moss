...
void TrackedVehicleController::Draw(DebugRenderer *inRenderer) const
{
	...
	inRenderer->DrawText3D(body->GetPosition(), status, Color::White, constraint_size);
	...
	for (const VehicleTrack &t : mTracks)
	{
		...
		DebugRenderer::Instance->DrawText3D(ws_position, StringFormat("W: %.1f", (double)t.mAngularVelocity), Color::White, constraint_size);
	}
	...
	for (const Wheel *w_base : mConstraint.GetWheels())
	{
		...
		inRenderer->DrawLine(ws_position, min_suspension_pos, Color::Red);
		inRenderer->DrawLine(min_suspension_pos, max_suspension_pos, Color::Green);

		// Draw current length
		RVec3 wheel_pos = ws_position + ws_direction * w->GetSuspensionLength();
		inRenderer->DrawMarker(wheel_pos, w->GetSuspensionLength() < settings->mSuspensionMinLength? Color::Red : Color::Green, constraint_size);
		...
		inRenderer->DrawLine(wheel_pos, wheel_pos + wheel_forward, Color::Red);
		inRenderer->DrawLine(wheel_pos, wheel_pos + wheel_up, Color::Green);
		inRenderer->DrawLine(wheel_pos, wheel_pos + wheel_right, Color::Blue);
		inRenderer->DrawLine(wheel_pos, wheel_pos + steering_axis, Color::Yellow);

		// Draw wheel
		RMat44 wheel_transform(Vec4(wheel_up, 0.0f), Vec4(wheel_right, 0.0f), Vec4(wheel_forward, 0.0f), wheel_pos);
		wheel_transform.SetRotation(wheel_transform.GetRotation() * Mat44::RotationY(-w->GetRotationAngle()));
		inRenderer->DrawCylinder(wheel_transform, settings->mWidth * 0.5f, settings->mRadius, w->GetSuspensionLength() <= settings->mSuspensionMinLength? Color::Red : Color::Green, DebugRenderer::ECastShadow::Off, DebugRenderer::EDrawMode::Wireframe);

		if (w->HasContact())
		{
			// Draw contact
			inRenderer->DrawLine(w->GetContactPosition(), w->GetContactPosition() + w->GetContactNormal(), Color::Yellow);
			inRenderer->DrawLine(w->GetContactPosition(), w->GetContactPosition() + w->GetContactLongitudinal(), Color::Red);
			inRenderer->DrawLine(w->GetContactPosition(), w->GetContactPosition() + w->GetContactLateral(), Color::Blue);

			DebugRenderer::Instance->DrawText3D(w->GetContactPosition(), StringFormat("S: %.2f", (double)w->GetSuspensionLength()), Color::White, constraint_size);
		}
	}
}
...
void VehicleConstraint::GetWheelLocalBasis(const Wheel *inWheel, Vec3 &outForward, Vec3 &outUp, Vec3 &outRight) const
{
	...
	Quat steer_rotation = Quat::Rotation(settings->mSteeringAxis, inWheel->mSteerAngle);
...
void VehicleConstraint::CalculatePitchRollConstraintProperties(RMat44Arg inBodyTransform)
{
	...
	if (mCosMaxPitchRollAngle > -1.0f)
	{
		...
		if (mCosPitchRollAngle < mCosMaxPitchRollAngle)
		{
			...
			mPitchRollPart.CalculateConstraintProperties(*mBody, Body::FixedToWorld, mPitchRollRotationAxis);
		}
		else
...
void VehicleConstraint::WarmStartVelocityConstraint(float inWarmStartImpulseRatio)
{
	...
	mPitchRollPart.WarmStart(*mBody, Body::FixedToWorld, inWarmStartImpulseRatio);
}
...
bool VehicleConstraint::SolveVelocityConstraint(float inDeltaTime)
{
	...
	if (mPitchRollPart.IsActive())
		impulse |= mPitchRollPart.SolveVelocityConstraint(*mBody, Body::FixedToWorld, mPitchRollRotationAxis, 0, FLT_MAX);
...
bool VehicleConstraint::SolvePositionConstraint(float inDeltaTime, float inBaumgarte)
{
	...
	CalculatePitchRollConstraintProperties(body_transform);
	if (mPitchRollPart.IsActive())
		impulse |= mPitchRollPart.SolvePositionConstraint(*mBody, Body::FixedToWorld, mCosPitchRollAngle - mCosMaxPitchRollAngle, inBaumgarte);
...
void VehicleEngine::DrawRPM(DebugRenderer *inRenderer, RVec3Arg inPosition, Vec3Arg inForward, Vec3Arg inUp, float inSize, float inShiftDownRPM, float inShiftUpRPM) const
{
	...
	draw_pie(0, mMinRPM, Color::Grey);

	// Draw segment until inShiftDownRPM
	if (mCurrentRPM < inShiftDownRPM)
	{
		draw_pie(mMinRPM, mCurrentRPM, Color::Red);
		draw_pie(mCurrentRPM, inShiftDownRPM, Color::DarkRed);
	}
	else
	{
		draw_pie(mMinRPM, inShiftDownRPM, Color::Red);
	}

	// Draw segment between inShiftDownRPM and inShiftUpRPM
	if (mCurrentRPM > inShiftDownRPM && mCurrentRPM < inShiftUpRPM)
	{
		draw_pie(inShiftDownRPM, mCurrentRPM, Color::Orange);
		draw_pie(mCurrentRPM, inShiftUpRPM, Color::DarkOrange);
	}
	else
	{
		draw_pie(inShiftDownRPM, inShiftUpRPM, mCurrentRPM <= inShiftDownRPM? Color::DarkOrange : Color::Orange);
	}

	// Draw segment above inShiftUpRPM
	if (mCurrentRPM > inShiftUpRPM)
	{
		draw_pie(inShiftUpRPM, mCurrentRPM, Color::Green);
		draw_pie(mCurrentRPM, mMaxRPM, Color::DarkGreen);
	}
	else
	{
		draw_pie(inShiftUpRPM, mMaxRPM, Color::DarkGreen);
	}
}
...
void WheeledVehicleController::Draw(DebugRenderer *inRenderer) const
{
	...
	if (mTransmission.GetCurrentRatio() != 0.0f)
	{
		...
		inRenderer->DrawLine(rpm_meter_pos, rpm_meter_pos + Quat::Rotation(rpm_meter_fwd, mEngine.ConvertRPMToAngle(wheel_speed_at_clutch)) * (rpm_meter_up * 1.1f * mRPMMeterSize), Color::Yellow);
	}
	...
	for (const Wheel *w_base : mConstraint.GetWheels())
	{
		...
		if (w->HasContact())
		{
			...
			DebugRenderer::Instance->DrawText3D(wheel_pos, StringFormat("W: %.1f, S: %.2f\nSlipLateral: %.1f, SlipLong: %.2f\nFrLateral: %.1f, FrLong: %.1f", (double)w->GetAngularVelocity(), (double)w->GetSuspensionLength(), (double)RadiansToDegrees(w->mLateralSlip), (double)w->mLongitudinalSlip, (double)w->mCombinedLateralFriction, (double)w->mCombinedLongitudinalFriction), Color::White, constraint_size);
		}
		else
		{
			// Draw 'no hit'
			DebugRenderer::Instance->DrawText3D(wheel_pos, StringFormat("W: %.1f", (double)w->GetAngularVelocity()), Color::Red, constraint_size);
		}
	}
}
...
void MotorcycleController::PreCollide(float inDeltaTime, PhysicsSystem &inPhysicsSystem)
{
	...
	if (mEnableLeanController)
	{
		...
		if (abs(w_angle) > mMaxLeanAngle)
			mTargetLean = Quat::Rotation(forward, Sign(w_angle) * mMaxLeanAngle) * adjusted_world_up;
...
void MotorcycleController::Draw(DebugRenderer *inRenderer) const
{
	...
	Vec3 up = body->GetRotation() * mConstraint.GetLocalUp();
	inRenderer->DrawArrow(center_of_mass, center_of_mass + up, Color::Yellow, 0.1f);
	inRenderer->DrawArrow(center_of_mass, center_of_mass + mTargetLean, Color::Red, 0.1f);
}
...
bool VehicleCollisionTesterCastSphere::Collide(PhysicsSystem &inPhysicsSystem, const VehicleConstraint &inVehicleConstraint, uint32 inWheelIndex, RVec3Arg inOrigin, Vec3Arg inDirection, const BodyID &inVehicleBodyID, Body *&outBody, SubShapeID &outSubShapeID, RVec3 &outContactPosition, Vec3 &outContactNormal, float &outSuspensionLength) const
{
	...
	RShapeCast shape_cast(&sphere, Vec3::One(), RMat44::Translation(inOrigin), inDirection * shape_cast_length);
...
bool VehicleCollisionTesterCastCylinder::Collide(PhysicsSystem &inPhysicsSystem, const VehicleConstraint &inVehicleConstraint, uint32 inWheelIndex, RVec3Arg inOrigin, Vec3Arg inDirection, const BodyID &inVehicleBodyID, Body *&outBody, SubShapeID &outSubShapeID, RVec3 &outContactPosition, Vec3 &outContactNormal, float &outSuspensionLength) const
{
	...
	RMat44 shape_cast_start = inVehicleConstraint.GetWheelWorldTransform(inWheelIndex, Vec3::AxisY(), Vec3::AxisX());
	...
	RShapeCast shape_cast(&cylinder, Vec3::One(), shape_cast_start, inDirection * max_suspension_length);
...
void VehicleCollisionTesterCastCylinder::PredictContactProperties(PhysicsSystem &inPhysicsSystem, const VehicleConstraint &inVehicleConstraint, uint32 inWheelIndex, RVec3Arg inOrigin, Vec3Arg inDirection, const BodyID &inVehicleBodyID, Body *&ioBody, SubShapeID &ioSubShapeID, RVec3 &ioContactPosition, Vec3 &ioContactNormal, float &ioSuspensionLength) const
{
	...
	if (d_dot_n < -1.0e-6f)
	{
		...
		RMat44 wheel_transform = inVehicleConstraint.GetWheelWorldTransform(inWheelIndex, Vec3::AxisY(), Vec3::AxisX());
