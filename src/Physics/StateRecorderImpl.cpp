...
void StateRecorderImpl::ReadBytes(void *outData, size_t inNumBytes)
{
	if (IsValidating())
	{
		...
		void *data = MOSS_STACK_ALLOC(inNumBytes);
		...
		if (memcmp(data, outData, inNumBytes) != 0)
		{
			...
			for (size_t i = 0; i < inNumBytes; ++i)
			{
				...
				if (b1 != b2)
					MOSS_TRACE("Offset %d: %02X -> %02X", i, b1, b2);
			}
			MOSS_BREAKPOINT;
		}
...
bool StateRecorderImpl::IsEqual(StateRecorderImpl &inReference)
{
	...
	if (fail)
	{
		MOSS_TRACE("Failed to properly recover state, different stream length!");
		return false;
	}
	...
	for (std::streamoff i = 0, l = this_len; !fail && i < l; ++i)
	{
		...
		if (fail)
		{
			MOSS_TRACE("Failed to properly recover state, different at offset %d!", (int)i);
			return false;
		}
	}
