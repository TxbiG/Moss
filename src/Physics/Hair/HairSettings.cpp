...
void HairSettings::Resample(Array<SVertex> &ioVertices, Array<SStrand> &ioStrands, uint32 inNumVerticesPerStrand)
{
...
void HairSettings::Init(float &outMaxDistSqHairToScalp)
{
	...
	for (const SStrand &strand : mSimStrands)
	{
		...
		for (uint32 i = 1; i < vertex_count; ++i)
		{
			...
			g[i] = v2.mInvMass > 0.0f? (material.mGravityPreloadFactor / v2.mInvMass) * mInitialGravity * gravity_sampler.Sample(v2.mStrandFraction) : Vec3::Zero();
		}
		...
		for (uint32 i = 1; i < vertex_count - 1; ++i)
		{
			...
			Mat44 r1_frame = Mat44::Rotation(bishop[i - 1]);
			...
			if (sin_angle > 1.0e-6f)
			{
				...
				normal2 = Quat::Rotation(t1_cross_t2, ASin(sin_angle)) * normal2;
			...
			Quat modeled_bishop = Quat::FromTo(tangent2, modeled_tangent2) * bishop[i];
...
void HairSettings::InitCompute(ComputeSystem *inComputeSystem)
{
	...
	for (size_t s = 0, ns = mSimStrands.size(); s < ns; ++s)
	{
		...
		for (uint32 v = 0, nv = strand.VertexCount(); v < nv; ++v)
		{
			...
			vertices_bishop[idx] = Vec4::LoadFloat4(&in_v.mBishop).CompressUnitVector();
			vertices_omega0[idx] = Vec4::LoadFloat4(&in_v.mOmega0).CompressUnitVector();
