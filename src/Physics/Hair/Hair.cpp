...
void Hair::InitializeContext(UpdateContext &outCtx, float inDeltaTime, const PhysicsSystem &inSystem)
{
	...
	RMat44 prev_com = RMat44::RotationTranslation(mPrevRotation, mPrevPosition);
				...
				if (lock.Succeeded())
				{
						...
						&& !body.IsSensor())
					{
						...
						struct LeafShapeCollector : public TransformedShapeCollector
						{
							...
							{
								mHits.emplace_back(Mat44::RotationTranslation(inResult.mShapeRotation, Vec3(inResult.mShapePositionCOM)),
						...
						body.GetShape()->CollectTransformedShapes(mLocalBounds, com.GetTranslation(), com.GetQuaternion(), Vec3::One(), SubShapeIDCreator(), collector, { });
					}
				}
			}
				...
				if (ScaleHelpers::IsInsideOut(shape.mScale))
				{
					...
					for (uint32 f = 0; f < nf; ++f)
					{
						...
						*sh = indices_start + nv;
						...
						Plane::FromPointsCCW(points[face_indices[2]], points[face_indices[1]], points[face_indices[0]]).StoreFloat4(sp);
						++sp;
					}
				}
				else
				{
					...
					for (uint32 f = 0; f < nf; ++f)
					{
						...
						*sh++ = indices_start + nv;
						...
						// Calculate plane (avoids reading from GPU memory)
						Plane::FromPointsCCW(points[face_indices[0]], points[face_indices[1]], points[face_indices[2]]).StoreFloat4(sp);
						++sp;
					}
				}
			}
...
void Hair::Draw(const DrawSettings &inSettings, DebugRenderer *inRenderer)
{
			...
			for (const HairSettings::RStrand &strand : mSettings->mRenderStrands)
			{
				...
				if (strand_idx >= inSettings.mSimulationStrandBegin && strand_idx < inSettings.mSimulationStrandEnd)
				{
					...
					for (uint32 v = strand.mStartVtx + 1; v < strand.mEndVtx; ++v)
					{
						...
						inRenderer->DrawLine(x0, x1, Color::GreenRedGradient(factor));
						x0 = x1;
					}
				}
			}
			...
			if (i >= inSettings.mSimulationStrandBegin && i < inSettings.mSimulationStrandEnd)
			{
				...
				for (uint32 v = strand.mStartVtx; v < strand.mEndVtx; ++v)
				{
					...
					if (velocity.LengthSq() > 1.0e-6f)
					{
						...
						inRenderer->DrawArrow(com * pos, com * (pos + velocity), Color::Green, arrow_size);
					}
				}
			}
		...
		for (uint32 i = 0, n = (uint32)mSettings->mSimStrands.size(); i < n; ++i)
			if (i >= inSettings.mSimulationStrandBegin && i < inSettings.mSimulationStrandEnd)
			{
				...
				for (uint32 v = strand.mStartVtx; v < strand.mEndVtx; ++v)
				{
					...
					{
						Vec3 pos = Vec3(positions[v]);
						inRenderer->DrawArrow(com * pos, com * (pos + 0.1f * angular_velocity), Color::Orange, arrow_size);
					}
				}
			}
	...
	if (inSettings.mDrawOrientations)
		for (uint32 i = 0, n = (uint32)mSettings->mSimStrands.size(); i < n; ++i)
			if (i >= inSettings.mSimulationStrandBegin && i < inSettings.mSimulationStrandEnd)
			{
				const HairSettings::SStrand &strand = mSettings->mSimStrands[i];
				for (uint32 v = strand.mStartVtx; v < strand.mEndVtx; ++v)
					inRenderer->DrawCoordinateSystem(com * Mat44::RotationTranslation(rotations[v], Vec3(positions[v])), arrow_size);
			}

	// Draw grid bounds
	if (inSettings.mDrawNeutralDensity || inSettings.mDrawGridDensity || inSettings.mDrawGridVelocity)
		inRenderer->DrawWireBox(com, mSettings->mSimulationBounds, Color::Grey);
			...
			for (uint32 y = 0; y < mSettings->mGridSize.GetY(); ++y)
				for (uint32 x = 0; x < mSettings->mGridSize.GetZ(); ++x)
				{
					...
					if (density > 0.0f)
					{
						...
						inRenderer->DrawMarker(com * pos, Color::GreenRedGradient(density * mSettings->mDensityScale), marker_size);
					}
				}
	}
		...
		for (uint32 z = 0; z < mSettings->mGridSize.GetX(); ++z)
			for (uint32 y = 0; y < mSettings->mGridSize.GetY(); ++y)
				for (uint32 x = 0; x < mSettings->mGridSize.GetZ(); ++x)
				{
					...
					Vec3 velocity = Vec3::LoadFloat3Unsafe((const Float3 &)velocity_and_density);
					if (density > 0.0f)
					{
						RVec3 pos = com * (offset + Vec3(UVec4(x, y, z, 0).ToFloat()) * scale);
						if (inSettings.mDrawGridDensity)
							inRenderer->DrawMarker(pos, Color::GreenRedGradient(density * mSettings->mDensityScale), marker_size);
						if (inSettings.mDrawGridVelocity && velocity.LengthSq() > 1.0e-6f)
							inRenderer->DrawArrow(pos, pos + com.Multiply3x3(velocity), Color::Yellow, arrow_size);
					}
				}
	}
	...
	if (inSettings.mDrawSkinPoints)
		for (uint32 i = 0, n = (uint32)mSettings->mSkinPoints.size(); i < n; ++i)
			if (i >= inSettings.mSimulationStrandBegin && i < inSettings.mSimulationStrandEnd)
			{
				...
				inRenderer->DrawWireTriangle(v0, v1, v2, Color::Yellow);
	...
	if (inSettings.mDrawInitialGravity)
		inRenderer->DrawArrow(com.GetTranslation(), com * mSettings->mInitialGravity, Color::Blue, 0.05f * mSettings->mInitialGravity.Length());
