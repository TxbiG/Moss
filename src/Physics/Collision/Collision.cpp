...
CollisionDispatch::CollideShape CollisionDispatch::CollideShape[NumSubShapeTypes][NumSubShapeTypes];
CollisionDispatch::CastShape CollisionDispatch::CastShape[NumSubShapeTypes][NumSubShapeTypes];


#ifdef MOSS_TRACK_NARROWPHASE_STATS
NarrowPhaseStat	NarrowPhaseStat::CollideShape[NumSubShapeTypes][NumSubShapeTypes];
NarrowPhaseStat	NarrowPhaseStat::CastShape[NumSubShapeTypes][NumSubShapeTypes];

thread_local TrackNarrowPhaseStat *TrackNarrowPhaseStat::Root = nullptr;
#endif
...
void TransformedShape::CollideShape(const Shape *inShape, Vec3Arg inShapeScale, RMat44Arg inCenterOfMassTransform, const CollideShapeSettings &inCollideShapeSettings, RVec3Arg inBaseOffset, CollideShapeCollector &ioCollector, const ShapeFilter &inShapeFilter) const
{
	if (mShape != nullptr)
	{
		...
		CollisionDispatch::CollideShapeVsShape(inShape, mShape, inShapeScale, GetShapeScale(), transform1, transform2, sub_shape_id1, sub_shape_id2, inCollideShapeSettings, ioCollector, inShapeFilter);
	}
}
...
void TransformedShape::CastShape(const RShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, RVec3Arg inBaseOffset, CastShapeCollector &ioCollector, const ShapeFilter &inShapeFilter) const
{
	if (mShape != nullptr)
	{
		...
		SubShapeIDCreator sub_shape_id1, sub_shape_id2(mSubShapeIDCreator);
		CollisionDispatch::CastShapeVsShapeWorldSpace(shape_cast, inShapeCastSettings, mShape, GetShapeScale(), inShapeFilter, center_of_mass_transform2, sub_shape_id1, sub_shape_id2, ioCollector);
	}
}
...
void TransformedShape::CollectTransformedShapes(const AABox &inBox, TransformedShapeCollector &ioCollector, const ShapeFilter &inShapeFilter) const
{
	if (mShape != nullptr)
	{
		...
		mShape->CollectTransformedShapes(box, Vec3::Zero(), mShapeRotation, GetShapeScale(), mSubShapeIDCreator, collector, inShapeFilter);
	}
}
...
void CastConvexVsTriangles::Cast(Vec3Arg inV0, Vec3Arg inV1, Vec3Arg inV2, uint8 inActiveEdges, const SubShapeID &inSubShapeID2)
{
	...
	if (epa.CastShape(mShapeCast.mCenterOfMassStart, mShapeCast.mDirection, mShapeCastSettings.mCollisionTolerance, mShapeCastSettings.mPenetrationTolerance, *mSupport, triangle, mSupport->GetConvexRadius(), 0.0f, mShapeCastSettings.mReturnDeepestPoint, fraction, contact_point_a, contact_point_b, contact_normal))
	{
		...
		ShapeCastResult result(fraction, contact_point_a, contact_point_b, contact_normal_world, back_facing, mSubShapeIDCreator1.GetID(), inSubShapeID2, TransformedShape::GetBodyID(mCollector.GetContext()));
...
void CastSphereVsTriangles::AddHit(bool inBackFacing, const SubShapeID &inSubShapeID2, float inFraction, Vec3Arg inContactPointA, Vec3Arg inContactPointB, Vec3Arg inContactNormal)
{
	...
	// Its a hit, store the sub shape id's
	ShapeCastResult result(inFraction, contact_point_a, contact_point_b, contact_normal_world, inBackFacing, mSubShapeIDCreator1.GetID(), inSubShapeID2, TransformedShape::GetBodyID(mCollector.GetContext()));
...
void CastSphereVsTriangles::Cast(Vec3Arg inV0, Vec3Arg inV1, Vec3Arg inV2, uint8 inActiveEdges, const SubShapeID &inSubShapeID2)
{
	...
	if (abs(v0.Dot(triangle_normal)) <= mRadius)
	{
		...
		if (q_len_sq <= Square(mRadius))
		{
			...
			Vec3 contact_normal = q_len > 0.0f? q / q_len : Vec3::AxisY();
	...
	// Test 3 vertices
	fraction = min(fraction, RaySphere(Vec3::Zero(), mDirection, v0, mRadius));
	fraction = min(fraction, RaySphere(Vec3::Zero(), mDirection, v1, mRadius));
	fraction = min(fraction, RaySphere(Vec3::Zero(), mDirection, v2, mRadius));
	...
	mSubShapeID1(inSubShapeID1)
{
	...
	mBoundsOf1.ExpandBy(Vec3::Replicate(inCollideShapeSettings.mMaxSeparationDistance));
...
void CollideConvexVsTriangles::Collide(Vec3Arg inV0, Vec3Arg inV1, Vec3Arg inV2, uint8 inActiveEdges, const SubShapeID &inSubShapeID2)
{
	...
	AABox triangle_bbox = AABox::FromTwoPoints(v0, v1);
	...
	CollideShapeResult result(point1, point2, penetration_axis_world, penetration_depth, mSubShapeID1, inSubShapeID2, TransformedShape::GetBodyID(mCollector.GetContext()));
...
void CollideSphereVsTriangles::Collide(Vec3Arg inV0, Vec3Arg inV1, Vec3Arg inV2, uint8 inActiveEdges, const SubShapeID &inSubShapeID2)
{
	...
	Vec3 penetration_axis = point2.NormalizedOr(Vec3::AxisY());
...
void CollisionDispatch::ReversedCollideShape(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, const ShapeFilter &inShapeFilter)
{
...
void CollisionDispatch::ReversedCastShape(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	// A collision collector that flips the collision results
...
void EstimateCollisionResponse(const Body &inBody1, const Body &inBody2, const ContactManifold &inManifold, CollisionEstimationResult &outResult, float inCombinedFriction, float inCombinedRestitution, float inMinVelocityForRestitution, uint inNumIterations)
{
	...
	if (motion_type1 != EMotionType::Static)
	{
		...
		outResult.mLinearVelocity1 = outResult.mAngularVelocity1 = Vec3::Zero();
	...
	if (motion_type2 != EMotionType::Static)
	{
		...
		outResult.mLinearVelocity2 = outResult.mAngularVelocity2 = Vec3::Zero();
...
#ifndef MOSS_DEBUG_RENDERER
	if (ContactConstraintManager::DrawContactPointReduction)
	{
		// Draw input polygon
		DebugRenderer::Instance->DrawWirePolygon(RMat44::Translation(inCenterOfMass), ioContactPointsOn1, Color::Orange, 0.05f);

		// Draw primary axis
		DebugRenderer::Instance->DrawArrow(inCenterOfMass + ioContactPointsOn1[point1], inCenterOfMass + ioContactPointsOn1[point2], Color::Red, 0.05f);

		// Draw contact points we kept
		for (Vec3 p : points_to_keep_on_1)
			DebugRenderer::Instance->DrawMarker(inCenterOfMass + p, Color::Green, 0.1f);
	}
#endif // MOSS_DEBUG_RENDERER
...
void ManifoldBetweenTwoFaces(Vec3Arg inContactPoint1, Vec3Arg inContactPoint2, Vec3Arg inPenetrationAxis, float inMaxContactDistance, const ConvexShape::SupportingFace &inShape1Face, const ConvexShape::SupportingFace &inShape2Face, ContactPoints &outContactPoints1, ContactPoints &outContactPoints2 MOSS_IF_DEBUG_RENDERER(, RVec3Arg inCenterOfMass))
{
...
#ifndef MOSS_DEBUG_RENDERER
	if (ContactConstraintManager::DrawContactPoint)
	{
		RVec3 cp1 = inCenterOfMass + inContactPoint1;
		RVec3 cp2 = inCenterOfMass + inContactPoint2;

		// Draw contact points
		DebugRenderer::Instance->DrawMarker(cp1, Color::Red, 0.1f);
		DebugRenderer::Instance->DrawMarker(cp2, Color::Green, 0.1f);

		// Draw contact normal
		DebugRenderer::Instance->DrawArrow(cp1, cp1 + inPenetrationAxis.Normalized(), Color::Red, 0.05f);
	}
#endif // MOSS_DEBUG_RENDERER
	...
	#ifndef MOSS_DEBUG_RENDERER
		if (ContactConstraintManager::DrawSupportingFaces)
		{
			RMat44 com = RMat44::Translation(inCenterOfMass);

			// Draw clipped poly
			DebugRenderer::Instance->DrawWirePolygon(com, clipped_face, Color::Orange);

			// Draw supporting faces
			DebugRenderer::Instance->DrawWirePolygon(com, inShape1Face, Color::Red, 0.05f);
			DebugRenderer::Instance->DrawWirePolygon(com, inShape2Face, Color::Green, 0.05f);
			...
			if (plane_normal_len > 0.0f)
			{
				...
				DebugRenderer::Instance->DrawArrow(plane_origin_ws, plane_origin_ws + plane_normal / plane_normal_len, Color::sYellow, 0.05f);
			}

			// Draw contact points that remain after distance check
			for (ContactPoints::size_type p = old_size; p < outContactPoints1.size(); ++p)
			{
				DebugRenderer::Instance->DrawMarker(inCenterOfMass + outContactPoints1[p], Color::Yellow, 0.1f);
				DebugRenderer::Instance->DrawMarker(inCenterOfMass + outContactPoints2[p], Color::Orange, 0.1f);
			}
		}
	#endif // MOSS_DEBUG_RENDERER
	}
...
void NarrowPhaseQuery::CollideShape(const Shape *inShape, Vec3Arg inShapeScale, RMat44Arg inCenterOfMassTransform, const CollideShapeSettings &inCollideShapeSettings, RVec3Arg inBaseOffset, CollideShapeCollector &ioCollector, const BroadPhaseLayerFilter &inBroadPhaseLayerFilter, const ObjectLayerFilter &inObjectLayerFilter, const BodyFilter &inBodyFilter, const ShapeFilter &inShapeFilter) const
{
	...
	bounds.ExpandBy(Vec3::Replicate(inCollideShapeSettings.mMaxSeparationDistance));
...
void NarrowPhaseStat::ReportStats()
{
...
void BroadPhaseQuadTree::LockModifications()
{
	...
	PhysicsLock::Lock(mUpdateMutex MOSS_IF_ENABLE_ASSERTS(, mLockContext, EPhysicsLockTypes::BroadPhaseUpdate));
}
...
void BroadPhaseQuadTree::UnlockModifications()
{
	...
	PhysicsLock::Unlock(mUpdateMutex MOSS_IF_ENABLE_ASSERTS(, mLockContext, EPhysicsLockTypes::BroadPhaseUpdate));
}
...
void BroadPhaseQuadTree::NotifyBodiesAABBChanged(BodyID *ioBodies, int inNumber, bool inTakeLock)
{
	...
	// This cannot run concurrently with UpdatePrepare()/UpdateFinalize()
	if (inTakeLock)
		PhysicsLock::LockShared(mUpdateMutex MOSS_IF_ENABLE_ASSERTS(, mLockContext, EPhysicsLockTypes::BroadPhaseUpdate));
	else
		MOSS_ASSERT(mUpdateMutex.is_locked());
	...
	if (inTakeLock)
		PhysicsLock::UnlockShared(mUpdateMutex MOSS_IF_ENABLE_ASSERTS(, mLockContext, EPhysicsLockTypes::BroadPhaseUpdate));
}
...
void BroadPhaseBruteForce::FindCollidingPairs(BodyID *ioActiveBodies, int inNumActiveBodies, float inSpeculativeContactDistance, const ObjectVsBroadPhaseLayerFilter &inObjectVsBroadPhaseLayerFilter, const ObjectLayerPairFilter &inObjectLayerPairFilter, BodyPairCollector &ioPairCollector) const
{
	...
	for (int b1 = 0; b1 < inNumActiveBodies; ++b1)
	{
		...
		bounds1.ExpandBy(Vec3::Replicate(inSpeculativeContactDistance));
		...
		for (size_t b2 = 0; b2 < num_bodies; ++b2)
		{
			...
			if (!Body::FindCollidingPairsCanCollide(body1, body2))
				continue;
	...
	mIsChanged(inIsChanged)
{
	...
	mChildNodeID[0] = NodeID::Invalid();
	mChildNodeID[1] = NodeID::Invalid();
	mChildNodeID[2] = NodeID::Invalid();
	mChildNodeID[3] = NodeID::Invalid();
}
...
const AABox QuadTree::cInvalidBounds(Vec3::Replicate(cLargeFloat), Vec3::Replicate(-cLargeFloat));
...
void QuadTree::UpdatePrepare(const BodyVector &inBodies, TrackingVector &ioTracking, UpdateState &outUpdateState, bool inFullRebuild)
{
	...
	if (num_node_ids > 0)
	{
		...
		if (root_node_id.IsBody()) {
			...
			root_node_id = NodeID::FromNodeIndex(root_idx);
		}
	}
	else
	{
...
void QuadTree::Partition(NodeID *ioNodeIDs, Vec3 *ioNodeCenters, int inNumber, int &outMidPoint)
{
	// Handle trivial case
	if (inNumber <= 4)
	{
	...
	Vec3 center_min = Vec3::Replicate(cLargeFloat);
	Vec3 center_max = Vec3::Replicate(-cLargeFloat);
...
void QuadTree::Partition4(NodeID *ioNodeIDs, Vec3 *ioNodeCenters, int inBegin, int inEnd, int *outSplit)
{
...
QuadTree::NodeID QuadTree::BuildTree(const BodyVector &inBodies, TrackingVector &ioTracking, NodeID *ioNodeIDs, int inNumber, uint inMaxDepthMarkChanged, AABox &outBounds)
{
	...
	stack[0].mChildIdx = -1;
	stack[0].mDepth = 0;
	stack[0].mNodeBoundsMin = Vec3::Replicate(cLargeFloat);
	stack[0].mNodeBoundsMax = Vec3::Replicate(-cLargeFloat);
	...
	for (;;)
	{
		...
		if (cur_stack.mChildIdx >= 4)
		{
			...
			prev_stack.mNodeBoundsMin = Vec3::Min(prev_stack.mNodeBoundsMin, cur_stack.mNodeBoundsMin);
			prev_stack.mNodeBoundsMax = Vec3::Max(prev_stack.mNodeBoundsMax, cur_stack.mNodeBoundsMax);
			...
			parent_node.mChildNodeID[prev_stack.mChildIdx] = NodeID::FromNodeIndex(cur_stack.mNodeIdx);
		...
		else
		{
			...
			if (num_bodies == 1)
			{
				...
				cur_stack.mNodeBoundsMin = Vec3::Min(cur_stack.mNodeBoundsMin, bounds.mMin);
				cur_stack.mNodeBoundsMax = Vec3::Max(cur_stack.mNodeBoundsMax, bounds.mMax);
			}
			else if (num_bodies > 1)
			{
				...
				new_stack.mNodeBoundsMin = Vec3::Replicate(cLargeFloat);
				new_stack.mNodeBoundsMax = Vec3::Replicate(-cLargeFloat);
	...
	// Return root
	return NodeID::FromNodeIndex(stack[0].mNodeIdx);
}
...
void QuadTree::WidenAndMarkNodeAndParentsChanged(uint32 inNodeIndex, const AABox &inNewBounds)
{
	...
	for (;;)
	{
		...
		NodeID node_id = NodeID::FromNodeIndex(node_idx);
		int child_idx = -1;
		for (int i = 0; i < 4; ++i)
...
bool QuadTree::TryInsertLeaf(TrackingVector &ioTracking, int inNodeIndex, NodeID inLeafID, const AABox &inLeafBounds, int inLeafNumBodies)
{
	...
	for (uint32 child_idx = 0; child_idx < 4; ++child_idx)
		if (node.mChildNodeID[child_idx].CompareExchange(NodeID::Invalid(), inLeafID)) // Check if we can claim it
		{
...
bool QuadTree::TryCreateNewRoot(TrackingVector &ioTracking, atomic<uint32> &ioRootNodeIndex, NodeID inLeafID, const AABox &inLeafBounds, int inLeafNumBodies)
{
	...
	new_root.mChildNodeID[0] = NodeIDF::romNodeIndex(root_idx);
	new_root.SetChildBounds(0, AABox(Vec3::Replicate(-cLargeFloat), Vec3::Replicate(cLargeFloat)));
...
void QuadTree::RemoveBodies([[maybe_unused]] const BodyVector &inBodies, TrackingVector &ioTracking, const BodyID *ioBodyIDs, int inNumber)
{
	...
	for (const BodyID *cur = ioBodyIDs, *end = ioBodyIDs + inNumber; cur < end; ++cur)
	{
		...
		node.mChildNodeID[child_idx] = NodeID::Invalid();
	...
	do
	{
		...
		else if (child_node_id.IsValid())
		{
			...
			if (top + 4 < cStackSize)
			{
				...
				Vec4 bounds_minx = Vec4::LoadFloat4Aligned((const Float4 *)&node.mBoundsMinX);
				Vec4 bounds_miny = Vec4::LoadFloat4Aligned((const Float4 *)&node.mBoundsMinY);
				Vec4 bounds_minz = Vec4::LoadFloat4Aligned((const Float4 *)&node.mBoundsMinZ);
				Vec4 bounds_maxx = Vec4::LoadFloat4Aligned((const Float4 *)&node.mBoundsMaxX);
				Vec4 bounds_maxy = Vec4::LoadFloat4Aligned((const Float4 *)&node.mBoundsMaxY);
				Vec4 bounds_maxz = Vec4::LoadFloat4Aligned((const Float4 *)&node.mBoundsMaxZ);

				// Load ids for 4 children
				UVec4 child_ids = UVec4::LoadInt4Aligned((const uint32 *)&node.mChildNodeID[0]);
...
void QuadTree::CollideSphere(Vec3Arg inCenter, float inRadius, CollideShapeBodyCollector &ioCollector, const ObjectLayerFilter &inObjectLayerFilter, const TrackingVector &inTracking) const
{
	...
	public:
		...
		MOSS_INLINE					Visitor(Vec3Arg inCenter, float inRadius, CollideShapeBodyCollector &ioCollector) :
			...
			mRadiusSq(Vec4::Replicate(Square(inRadius))),
			mCollector(ioCollector)
		{
		}
		...
		do
		{
			...
			if (child_node_id.IsBody())
			{
				...
				if (b1_id != b2_id)
				{
					...
					if (inObjectLayerPairFilter.ShouldCollide(body1.GetObjectLayer(), body2.GetObjectLayer())
						&& Body::FindCollidingPairsCanCollide(body1, body2)
			...
			else if (child_node_id.IsValid())
			{
				...
				if (num_results > 0)
				{
					...
					child_ids = UVec4::Sort4True(overlap, child_ids);
