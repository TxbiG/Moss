...
static const TStaticArray<Vec3, 192> sCapsuleTopTriangles = []() {
	...
	GetTrianglesContextVertexList::CreateHalfUnitSphereTop(verts, cCapsuleDetailLevel);
	return verts;
}();

static const TStaticArray<Vec3, 96> sCapsuleMiddleTriangles = []() {
	TStaticArray<Vec3, 96> verts;
	GetTrianglesContextVertexList::CreateUnitOpenCylinder(verts, cCapsuleDetailLevel);
	return verts;
}();

static const TStaticArray<Vec3, 192> sCapsuleBottomTriangles = []() {
	TStaticArray<Vec3, 192> verts;
	GetTrianglesContextVertexList::CreateHalfUnitSphereBottom(verts, cCapsuleDetailLevel);
	return verts;
}();

#ifndef MOSS_DEBUG_RENDERER
bool Shape::DrawSubmergedVolumes = false;
#endif // MOSS_DEBUG_RENDERER

ShapeFunctions ShapeFunctions::Registry[NumSubShapeTypes];
...
const TStaticArray<Vec3, 384> ConvexShape::UnitSphereTriangles = []() {
	const int level = 2;

	TStaticArray<Vec3, 384> verts;
	GetTrianglesContextVertexList::CreateHalfUnitSphereTop(verts, level);
	GetTrianglesContextVertexList::CreateHalfUnitSphereBottom(verts, level);
	return verts;
}();
...
{
	Vec3 half_extent = Vec3::Replicate(mRadius);
...
AABox SphereShape::GetWorldSpaceBounds(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale) const
{
	...
	Vec3 half_extent = Vec3::Replicate(scaled_radius);
...
public:
	...
	virtual Vec3	GetSupport(Vec3Arg inDirection) const override
	{
		...
		return len > 0.0f? (mRadius / len) * inDirection : Vec3::Zero();
	}
...
MassProperties SphereShape::GetMassProperties() const
{
	...
	p.mInertia = Mat44::Scale(inertia);
...
Vec3 SphereShape::GetSurfaceNormal(const SubShapeID &inSubShapeID, Vec3Arg inLocalSurfacePosition) const
{
	...
	return len != 0.0f? inLocalSurfacePosition / len : Vec3::AxisY();
}
...
void SphereShape::GetSubmergedVolume(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale, const Plane &inSurface, float &outTotalVolume, float &outSubmergedVolume, Vec3 &outCenterOfBuoyancy MOSS_IF_DEBUG_RENDERER(, RVec3Arg inBaseOffset)) const
{
	...
	else
	{
		...
		if (sDrawSubmergedVolumes)
		{
			...
			DebugRenderer::Instance->DrawPie(inBaseOffset + circle_center, circle_radius, inSurface.GetNormal(), inSurface.GetNormal().GetNormalizedPerpendicular(), -MOSS_PI, MOSS_PI, Color::Green, DebugRenderer::ECastShadow::Off);
		}
	#endif // MOSS_DEBUG_RENDERER
	}
	...
	if (sDrawSubmergedVolumes)
		DebugRenderer::Instance->DrawWireSphere(inBaseOffset + outCenterOfBuoyancy, 0.05f, Color::Red, 1);
#endif // MOSS_DEBUG_RENDERER
}
...
void SphereShape::Draw(DebugRenderer *inRenderer, RMat44Arg inCenterOfMassTransform, Vec3Arg inScale, ColorArg inColor, bool inUseMaterialColors, bool inDrawWireframe) const
{
	...
	inRenderer->DrawUnitSphere(inCenterOfMassTransform * Mat44::Scale(mRadius * inScale.Abs().GetX()), inUseMaterialColors? GetMaterial()->GetDebugColor() : inColor, DebugRenderer::ECastShadow::On, draw_mode);
}
#endif // MOSS_DEBUG_RENDERER

bool SphereShape::CastRay(const RayCast &inRay, const SubShapeIDCreator &inSubShapeIDCreator, RayCastResult &ioHit) const
{
	float fraction = RaySphere(inRay.mOrigin, inRay.mDirection, Vec3::Zero(), mRadius);
...
void SphereShape::CastRay(const RayCast &inRay, const RayCastSettings &inRayCastSettings, const SubShapeIDCreator &inSubShapeIDCreator, CastRayCollector &ioCollector, const ShapeFilter &inShapeFilter) const
{
	...
	int num_results = RaySphere(inRay.mOrigin, inRay.mDirection, Vec3::Zero(), mRadius, min_fraction, max_fraction);
	...
	for (CollideSoftBodyVertexIterator v = inVertices, sbv_end = inVertices + inNumVertices; v != sbv_end; ++v)
		if (v.GetInvMass() > 0.0f)
		{
			...
			if (v.UpdatePenetration(penetration))
			{
				...
				Vec3 normal = distance > 0.0f? delta / distance : Vec3::AxisY();
				Vec3 point = center + radius * normal;

				// Store collision
				v.SetCollision(Plane::FromPointAndNormal(point, normal), inCollidingShapeIndex);
			}
		}
}
...
{
	float scaled_radius = GetScaledRadius(inScale);
	new (&ioContext) GetTrianglesContextVertexList(inPositionCOM, inRotation, Vec3::One(), Mat44::Scale(scaled_radius), sUnitSphereTriangles.data(), sUnitSphereTriangles.size(), GetMaterial());
}
...
void SphereShape::Register()
{
	ShapeFunctions &f = ShapeFunctions::Get(EShapeSubType::Sphere);
	f.mConstruct = []() -> Shape * { return new SphereShape; };
	f.mColor = Color::Green;
}
...
ShapeSettings::ShapeResult TaperedCapsuleShapeSettings::Create() const
{
	if (mCachedResult.IsEmpty())
	{
		...
		if (IsValid() && IsSphere())
		{
			...
			{
				RotatedTranslatedShapeSettings rot_trans(Vec3(0, center, 0), Quat::Identity(), shape);
...
Vec3 TaperedCapsuleShape::GetSurfaceNormal(const SubShapeID &inSubShapeID, Vec3Arg inLocalSurfacePosition) const
{
	...
	else
	{
		...
		Vec3 perpendicular = Vec3(inLocalSurfacePosition.GetX(), 0, inLocalSurfacePosition.GetZ()).NormalizedOr(Vec3::AxisX());
...
AABox TaperedCapsuleShape::GetWorldSpaceBounds(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale) const
{
	...
	float scale_xz = abs_scale.GetX();
	float scale_y = inScale.GetY(); // The sign of y is important as it flips the tapered capsule
	Vec3 bottom_extent = Vec3::Replicate(scale_xz * mBottomRadius);
	Vec3 bottom_center = inCenterOfMassTransform * Vec3(0, scale_y * mBottomCenter, 0);
	Vec3 top_extent = Vec3::Replicate(scale_xz * mTopRadius);
	Vec3 top_center = inCenterOfMassTransform * Vec3(0, scale_y * mTopCenter, 0);
	Vec3 p1 = Vec3::Min(top_center - top_extent, bottom_center - bottom_extent);
	Vec3 p2 = Vec3::Max(top_center + top_extent, bottom_center + bottom_extent);
	return AABox(p1, p2);
}
...
void TaperedCapsuleShape::CollideSoftBodyVertices(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale, const CollideSoftBodyVertexIterator &inVertices, uint32 inNumVertices, int inCollidingShapeIndex) const
{
	...
	for (CollideSoftBodyVertexIterator v = inVertices, sbv_end = inVertices + inNumVertices; v != sbv_end; ++v)
		if (v.GetInvMass() > 0.0f)
		{
			...
			{
				// Top sphere
				normal = top_center_to_local_pos_len != 0.0f? top_center_to_local_pos / top_center_to_local_pos_len : Vec3::AxisY();
			...
			else
			{
				...
				{
					// Bottom sphere
					normal = bottom_center_to_local_pos_len != 0.0f? bottom_center_to_local_pos / bottom_center_to_local_pos_len : -Vec3::AxisY();
				}
				else
				{
					// Tapered cylinder
					normal = Vec3(local_pos.GetX(), 0, local_pos.GetZ()).NormalizedOr(Vec3::AxisX());
			...
			Plane plane = Plane::FromPointAndNormal(position, normal);
...
void TaperedCapsuleShape::Draw(DebugRenderer *inRenderer, RMat44Arg inCenterOfMassTransform, Vec3Arg inScale, ColorArg inColor, bool inUseMaterialColors, bool inDrawWireframe) const
{
	if (mGeometry == nullptr)
	{
		...
		const Support *support = GetSupportFunction(ESupportMode::IncludeConvexRadius, buffer, Vec3::One());
	...
	RMat44 world_transform = inCenterOfMassTransform * Mat44::Scale(scale);
...
void TaperedCapsuleShape::Register()
{
	ShapeFunctions &f = ShapeFunctions::Get(EShapeSubType::TaperedCapsule);
...
{
	return (Vec3(1, 0, 1) * inSurfacePosition).NormalizedOr(Vec3::AxisX());
}
...
MassProperties TaperedCylinderShape::GetMassProperties() const
{
	...
	p.mInertia = Mat44::Scale(Vec3(inertia_x, inertia_y, inertia_z));
	return p;
}
			...
			if (side_penetration < 0.0f || min_top_bottom_penetration < 0.0f)
			{
				...
				*/
					...
					&& !inside_top_radius) // Outside the top radius
				{
					...
					normal = (local_pos - point).NormalizedOr(Vec3::AxisY());
				}
...
void TaperedCylinderShape::GetTrianglesStart(GetTrianglesContext &ioContext, const AABox &inBox, Vec3Arg inPositionCOM, QuatArg inRotation, Vec3Arg inScale) const
{
	...
	TCSGetTrianglesContext *context = new (&ioContext) TCSGetTrianglesContext(Mat44::RotationTranslation(inRotation, inPositionCOM) * Mat44::Scale(scale));
	constexpr float cMinRadius = 1.0e-3f;
...
void TaperedCylinderShape::Register()
{
	ShapeFunctions &f = ShapeFunctions::Get(EShapeSubType::TaperedCylinder);
...
ShapeSettings::ShapeResult StaticCompoundShapeSettings::Create(TempAllocator &inTempAllocator) const
{
	if (mCachedResult.IsEmpty())
	{
		...
		else if (mSubShapes.size() == 1)
		{
			...
			if (s.mPosition == Vec3::Zero()
				&& s.mRotation == Quat::Identity())
			{
...
void StaticCompoundShape::Partition(uint32 *ioBodyIdx, AABox *ioBounds, int inNumber, int &outMidPoint)
{
	...
	Vec3 center_min = Vec3::Replicate(FLT_MAX);
	Vec3 center_max = Vec3::Replicate(-FLT_MAX);
...
void StaticCompoundShape::Partition4(uint32 *ioBodyIdx, AABox *ioBounds, int inBegin, int inEnd, int *outSplit)
{
	...
	Partition(body_idx, node_bounds, number, outSplit[2]);

	// Partition lower half
	Partition(body_idx, node_bounds, outSplit[2], outSplit[1]);

	// Partition upper half
	Partition(body_idx + outSplit[2], node_bounds + outSplit[2], number - outSplit[2], outSplit[3]);
	...
	CompoundShape(EShapeSubType::StaticCompound, inSettings, outResult)
{
	...
	for (uint32 i = 0; i < num_subshapes; ++i)
	{
		...
		Mat44 transform = Mat44::RotationTranslation(shape.GetRotation(), shape.GetPositionCOM());
		AABox shape_bounds = shape.mShape->GetWorldSpaceBounds(transform, Vec3::One());
...
void StaticCompoundShape::CastShapeVsCompound(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	MOSS_PROFILE_FUNCTION();
...
void StaticCompoundShape::CollideCompoundVsShape(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, const ShapeFilter &inShapeFilter)
{
	MOSS_PROFILE_FUNCTION();
...
void StaticCompoundShape::CollideShapeVsCompound(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, const ShapeFilter &inShapeFilter)
{
	MOSS_PROFILE_FUNCTION();
	...
	struct Visitor : public CollideShapeVsCompoundVisitor
	{
		...
		MOSS_INLINE bool ShouldVisitNode([[maybe_unused]] int inStackTop) const { return true; }
...
void StaticCompoundShape::Register()
{
	ShapeFunctions &f = ShapeFunctions::Get(EShapeSubType::StaticCompound);
	f.mConstruct = []() -> Shape * { return new StaticCompoundShape; };
	f.mColor = Color::Orange;

	for (EShapeSubType s : sAllSubShapeTypes) {
		CollisionDispatch::RegisterCollideShape(EShapeSubType::StaticCompound, s, sCollideCompoundVsShape);
		CollisionDispatch::RegisterCollideShape(s, EShapeSubType::StaticCompound, sCollideShapeVsCompound);
		CollisionDispatch::RegisterCastShape(s, EShapeSubType::StaticCompound, sCastShapeVsCompound);
	}
}
...
void Shape::CollectTransformedShapes(const AABox &inBox, Vec3Arg inPositionCOM, QuatArg inRotation, Vec3Arg inScale, const SubShapeIDCreator &inSubShapeIDCreator, TransformedShapeCollector &ioCollector, const ShapeFilter &inShapeFilter) const
{
	...
	TransformedShape ts(RVec3(inPositionCOM), inRotation, this, TransformedShape::GetBodyID(ioCollector.GetContext()), inSubShapeIDCreator);
	ts.SetShapeScale(inScale);
...
Shape::ShapeResult Shape::RestoreFromBinaryState(StreamIn &inStream)
{
	ShapeResult result;
	...
	Ref<Shape> shape = ShapeFunctions::Get(shape_sub_type).mConstruct();
...
Shape::ShapeResult Shape::RestoreWithChildren(StreamIn &inStream, IDToShapeMap &ioShapeMap, IDToMaterialMap &ioMaterialMap) {
	ShapeResult result;
...
Shape::ShapeResult Shape::ScaleShape(Vec3Arg inScale) const {
	const Vec3 unit_scale = Vec3::One();
	...
	Collector collector;
	TransformShape(Mat44::Scale(inScale) * Mat44::Translation(GetCenterOfMass()), collector);
...
void Shape::CollidePointUsingRayCast(const Shape &inShape, Vec3Arg inPoint, const SubShapeIDCreator &inSubShapeIDCreator, CollidePointCollector &ioCollector, const ShapeFilter &inShapeFilter)
{
	...
	if (bounds.Contains(inPoint))
	{
		...
		inShape.CastRay(RayCast { inPoint, 1.1f * bounds.GetSize().GetY() * Vec3::AxisY() }, settings, inSubShapeIDCreator, collector, inShapeFilter);

		// Odd amount of hits means inside
		if ((collector.mHitCount & 1) == 1)
			ioCollector.AddHit({ TransformedShape::GetBodyID(ioCollector.GetContext()), collector.mSubShapeID });
	}
}
...
public:
	...
	{
		float len = inDirection.Length();
		Vec3 radius = len > 0.0f? inDirection * (mRadius / len) : Vec3::Zero();
...
MassProperties CapsuleShape::GetMassProperties() const
{
	...
	// Set inertia
	p.mInertia = Mat44::Scale(Vec3(inertia_xz, inertia_y, inertia_xz));
...
Vec3 CapsuleShape::GetSurfaceNormal(const SubShapeID &inSubShapeID, Vec3Arg inLocalSurfacePosition) const
{
	...
	if (inLocalSurfacePosition.GetY() > mHalfHeightOfCylinder)
		...
		return Vec3(inLocalSurfacePosition.GetX(), 0, inLocalSurfacePosition.GetZ()).NormalizedOr(Vec3::AxisX());
}

AABox CapsuleShape::GetLocalBounds() const
{
	Vec3 extent = Vec3::Replicate(mRadius) + Vec3(0, mHalfHeightOfCylinder, 0);
...
AABox CapsuleShape::GetWorldSpaceBounds(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale) const
{
	...
	Vec3 abs_scale = inScale.Abs();
	float scale = abs_scale.GetX();
	Vec3 extent = Vec3::Replicate(scale * mRadius);
	...
	return AABox(Vec3::Min(p1, p2) - extent, Vec3::Max(p1, p2) + extent);
}
...
{
	DebugRenderer::EDrawMode draw_mode = inDrawWireframe? DebugRenderer::EDrawMode::Wireframe : DebugRenderer::EDrawMode::Solid;
	inRenderer->DrawCapsule(inCenterOfMassTransform * Mat44::Scale(inScale.Abs().GetX()), mHalfHeightOfCylinder, mRadius, inUseMaterialColors? GetMaterial()->GetDebugColor() : inColor, DebugRenderer::ECastShadow::On, draw_mode);
}
#endif // MOSS_DEBUG_RENDERER
	...
	for (CollideSoftBodyVertexIterator v = inVertices, sbv_end = inVertices + inNumVertices; v != sbv_end; ++v)
		if (v.GetInvMass() > 0.0f)
		{
			...
			if (abs(local_pos.GetY()) <= half_height_of_cylinder)
			{
				...
				{
					// Calculate contact point and normal
					normal = normal_length > 0.0f? normal / normal_length : Vec3::AxisX();
					Vec3 point = radius * normal;

					// Store collision
					v.SetCollision(Plane::FromPointAndNormal(point, normal).GetTransformed(inCenterOfMassTransform), inCollidingShapeIndex);
				}
			}
			else
			{
...
void CapsuleShape::GetTrianglesStart(GetTrianglesContext &ioContext, const AABox &inBox, Vec3Arg inPositionCOM, QuatArg inRotation, Vec3Arg inScale) const
{
	...
	Mat44 world_matrix = Mat44::RotationTranslation(inRotation, inPositionCOM) * Mat44::Scale(scale);
	...
	Mat44 middle_matrix = world_matrix * Mat44::Scale(Vec3(mRadius, mHalfHeightOfCylinder, mRadius));
...
void CapsuleShape::Register()
{
	ShapeFunctions &f = ShapeFunctions::Get(EShapeSubType::Capsule);
	...
	case ESupportMode::ExcludeConvexRadius:
		{
			...
			Vec3 convex_radius3 = Vec3::Replicate(convex_radius);
...
{
	DebugRenderer::EDrawMode draw_mode = inDrawWireframe? DebugRenderer::EDrawMode::Wireframe : DebugRenderer::EDrawMode::Solid;
	inRenderer->DrawBox(inCenterOfMassTransform * Mat44::Scale(inScale.Abs()), GetLocalBounds(), inUseMaterialColors? GetMaterial()->GetDebugColor() : inColor, DebugRenderer::ECastShadow::On, draw_mode);
}
#endif // MOSS_DEBUG_RENDERER
...
void BoxShape::CollidePoint(Vec3Arg inPoint, const SubShapeIDCreator &inSubShapeIDCreator, CollidePointCollector &ioCollector, const ShapeFilter &inShapeFilter) const
{
	...
	if (Vec3::LessOrEqual(inPoint.Abs(), mHalfExtent).TestAllXYZTrue())
		ioCollector.AddHit({ TransformedShape::GetBodyID(ioCollector.GetContext()), inSubShapeIDCreator.GetID() });
}
...
void BoxShape::CollideSoftBodyVertices(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale, const CollideSoftBodyVertexIterator &inVertices, uint32 inNumVertices, int inCollidingShapeIndex) const
{
	...
	for (CollideSoftBodyVertexIterator v = inVertices, sbv_end = inVertices + inNumVertices; v != sbv_end; ++v)
		if (v.GetInvMass() > 0.0f)
		{
			...
			Vec3 clamped_point = Vec3::Max(Vec3::Min(local_pos, half_extent), -half_extent);
			...
			if (clamped_point == local_pos)
			{
				...
				{
					// Calculate contact point and normal
					Vec3 possible_normals[] = { Vec3::AxisX(), Vec3::AxisY(), Vec3::AxisZ() };
			...
			else
			{
				...
				if (v.UpdatePenetration(penetration))
				{
					...
					// Store collision
					v.SetCollision(Plane::FromPointAndNormal(clamped_point, normal).GetTransformed(inCenterOfMassTransform), inCollidingShapeIndex);
				}
			}
		}
}
...
{
	new (&ioContext) GetTrianglesContextVertexList(inPositionCOM, inRotation, inScale, Mat44::Scale(mHalfExtent), sUnitBoxTriangles, std::size(sUnitBoxTriangles), GetMaterial());
}
...
Vec3 CompoundShape::GetSurfaceNormal(const SubShapeID &inSubShapeID, Vec3Arg inLocalSurfacePosition) const
{
	...
	Mat44 transform = Mat44::InverseRotationTranslation(shape.GetRotation(), shape.GetPositionCOM());
...
{
	for (const SubShape &shape : mSubShapes)
		shape.mShape->TransformShape(inCenterOfMassTransform * Mat44::RotationTranslation(shape.GetRotation(), shape.GetPositionCOM()), ioCollector);
}

void CompoundShape::CastCompoundVsShape(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	MOSS_PROFILE_FUNCTION();
	...
	for (int i = 0; i < n; ++i)
	{
		...
		CollisionDispatch::CastShapeVsShapeLocalSpace(shape_cast, inShapeCastSettings, inShape, inScale, inShapeFilter, inCenterOfMassTransform2, shape1_sub_shape_id, inSubShapeIDCreator2, ioCollector);
...
void CompoundShape::Register()
{
	for (EShapeSubType s1 : sCompoundSubShapeTypes)
		for (EShapeSubType s2 : sAllSubShapeTypes)
			CollisionDispatch::RegisterCastShape(s1, s2, sCastCompoundVsShape);
}

/*													*/

void ConvexShape::CollideConvexVsConvex(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, [[maybe_unused]] const ShapeFilter &inShapeFilter)
{
	MOSS_PROFILE_FUNCTION();
	...
	CollideShapeResult result(point1, point2, penetration_axis_world, penetration_depth, inSubShapeIDCreator1.GetID(), inSubShapeIDCreator2.GetID(), TransformedShape::GetBodyID(ioCollector.GetContext()));
...
bool ConvexShape::CastRay(const RayCast &inRay, const SubShapeIDCreator &inSubShapeIDCreator, RayCastResult &ioHit) const
{
	...
	// Create support function
	SupportBuffer buffer;
	const Support *support = GetSupportFunction(ConvexShape::ESupportMode::IncludeConvexRadius, buffer, Vec3::One());
		...
		if (inRayCastSettings.mBackFaceModeConvex == EBackFaceMode::CollideWithBackFaces && !ioCollector.ShouldEarlyOut())
		{
			...
			if (delta_fraction < 0.0f)
			{
					...
					&& inverted_hit.mFraction > 0.0f) // Ignore hits with fraction 0, this means the ray ends inside the object and we don't want to report it as a back facing hit
				{
					...
					inverted_hit.mBodyID = TransformedShape::GetBodyID(ioCollector.GetContext());
...
void ConvexShape::CastConvexVsConvex(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, [[maybe_unused]] const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	MOSS_PROFILE_FUNCTION();
			...
			|| contact_normal.Dot(inShapeCast.mDirection) > 0.0f)) // Test if backfacing
	{
		...
		ShapeCastResult result(fraction, contact_point_a, contact_point_b, contact_normal_world, false, inSubShapeIDCreator1.GetID(), inSubShapeIDCreator2.GetID(), TransformedShape::GetBodyID(ioCollector.GetContext()));
...
public:
		...
		mLocalToWorld(Mat44::RotationTranslation(inRotation, inPositionCOM) * Mat44::Scale(inScale)),
		mIsInsideOut(ScaleHelpers::IsInsideOut(inScale))
	{
		mSupport = inShape->GetSupportFunction(ESupportMode::IncludeConvexRadius, mSupportBuffer, Vec3::One());
	}
...
void ConvexShape::GetSubmergedVolume(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale, const Plane &inSurface, float &outTotalVolume, float &outSubmergedVolume, Vec3 &outCenterOfBuoyancy MOSS_IF_DEBUG_RENDERER(, RVec3Arg inBaseOffset)) const
{
	...
	PolyhedronSubmergedVolumeCalculator submerged_vol_calc(inCenterOfMassTransform * Mat44::Scale(extent), points, sizeof(Vec3), 8, inSurface, buffer MOSS_IF_DEBUG_RENDERER(, inBaseOffset));
...
void ConvexShape::DrawGetSupportFunction(DebugRenderer *inRenderer, RMat44Arg inCenterOfMassTransform, Vec3Arg inScale, ColorArg inColor, bool inDrawSupportDirection) const
{
	...
	if (inDrawSupportDirection)
	{
		...
		for (Vec3 v : Vec3::UnitSphere) {
			...
			inRenderer->DrawMarker(from, Color::White, 0.001f);
			inRenderer->DrawArrow(from, to, Color::White, 0.001f);
		}
	}
}
...
void ConvexShape::DrawGetSupportingFace(DebugRenderer *inRenderer, RMat44Arg inCenterOfMassTransform, Vec3Arg inScale) const
{
	...
	for (Vec3 v : Vec3::UnitSphere)
	{
		...
		SupportingFace face;
		GetSupportingFace(SubShapeID(), direction, inScale, Mat44::Identity(), face);
	...
	{
		Color color = Color::GetDistinctColor(color_it++);
		...
		Vec3 normal = face.size() >= 3? (face[2] - face[1]).Cross(face[0] - face[1]).NormalizedOr(Vec3::Zero()) : Vec3::Zero();
		Vec3 displacement = 0.001f * normal;

		// Transform face to world space and calculate center of mass
		Vec3 com_ls = Vec3::Zero();
		for (Vec3 &v : face)
		{
		...
		inRenderer->DrawWirePolygon(RMat44::Translation(inCenterOfMassTransform.GetTranslation()), face, color, face.size() >= 3? 0.001f : 0.0f);
		if (face.size() >= 3)
...
void ConvexShape::Register()
{
	for (EShapeSubType s1 : sConvexSubShapeTypes)
		for (EShapeSubType s2 : sConvexSubShapeTypes) {
			CollisionDispatch::RegisterCollideShape(s1, s2, sCollideConvexVsConvex);
			CollisionDispatch::RegisterCastShape(s1, s2, sCastConvexVsConvex);
		}
}
	...
	mConvexRadius(inSettings.mMaxConvexRadius)
{
	...
	Mat44 covariance_matrix = Mat44::Zero();
	...
	mInertia = Mat44::Identity() * (covariance_matrix(0, 0) + covariance_matrix(1, 1) + covariance_matrix(2, 2)) - covariance_matrix;
	...
	for (BuilderFace *builder_face : builder_faces)
	{
		...
		// Add plane
		Plane plane = Plane::FromPointAndNormal(builder_face->mCentroid - mCenterOfMass, builder_face->mNormal.Normalized());
		...
		for (const Point &point : mPoints)
			if (point.mNumFaces != 1) // If we have a single face, shifting back is easy and we don't need to reduce the convex radius
			{
				...
				else
				{
					...
					p3 = Plane::FromPointAndNormal(point.mPosition, p1.GetNormal().Cross(p2.GetNormal()));
		...
		if (ScaleHelpers::IsNotScaled(inScale))
		{
			...
			for (const Point &point : mPoints)
			{
				...
				else
				{
					...
					if (!Plane::IntersectPlanes(p1, p2, p3, new_point))
					{
		...
		else
		{
			...
			for (const Point &point : mPoints)
			{
				...
				else
				{
					...
					// Get first two planes and offset inwards by convex radius
					Plane p1 = Plane::FromPointAndNormal(pos, n1).Offset(-convex_radius);
					Plane p2 = Plane::FromPointAndNormal(pos, n2).Offset(-convex_radius);
					Plane p3;
...
void ConvexHullShape::GetSubmergedVolume(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale, const Plane &inSurface, float &outTotalVolume, float &outSubmergedVolume, Vec3 &outCenterOfBuoyancy MOSS_IF_DEBUG_RENDERER(, RVec3Arg inBaseOffset)) const
{
	...
	PolyhedronSubmergedVolumeCalculator submerged_vol_calc(inCenterOfMassTransform * Mat44::Scale(inScale), &mPoints[0].mPosition, sizeof(Point), num_points, inSurface, buffer MOSS_IF_DEBUG_RENDERER(, inBaseOffset));
...
void ConvexHullShape::Draw(DebugRenderer *inRenderer, RMat44Arg inCenterOfMassTransform, Vec3Arg inScale, ColorArg inColor, bool inUseMaterialColors, bool inDrawWireframe) const
{
	if (mGeometry == nullptr)
	{
		...
		for (const Face &f : mFaces)
		{
			...
			triangles.push_back({ v0, v1, v2, Color::White, v0, uv_direction });

			// Draw any other triangles in this polygon
			for (const uint8 *v = first_vtx + 3; v < end_vtx; ++v)
				triangles.push_back({ v0, mPoints[*(v - 1)].mPosition, mPoints[*v].mPosition, Color::White, v0, uv_direction });
		}
	...
	if (sDrawFaceOutlines)
		for (const Face &f : mFaces)
		{
			...
			// Draw edges of face
			inRenderer->DrawLine(transform * mPoints[*(end_vtx - 1)].mPosition, transform * mPoints[*first_vtx].mPosition, Color::Grey);
			for (const uint8 *v = first_vtx + 1; v < end_vtx; ++v)
				inRenderer->DrawLine(transform * mPoints[*(v - 1)].mPosition, transform * mPoints[*v].mPosition, Color::Grey);
		}
}
...
void ConvexHullShape::DrawShrunkShape(DebugRenderer *inRenderer, RMat44Arg inCenterOfMassTransform, Vec3Arg inScale) const
{
	...
	RMat44 transform = inCenterOfMassTransform * Mat44::Scale(inScale);
	...
	for (int p = 0; p < (int)mPoints.size(); ++p)
	{
		...
		inRenderer->DrawLine(position, shrunk_point, Color::Green);

		// Draw face normals that are contributing
		for (int i = 0; i < point.mNumFaces; ++i)
			inRenderer->DrawLine(position, position + 0.1f * mPlanes[point.mFaces[i]].GetNormal(), Color::Yellow);

		// Draw point index
		inRenderer->DrawText3D(position, ConvertToString(p), Color::White, 0.1f);
	}
}
#endif // MOSS_DEBUG_RENDERER
	...
	for (CollideSoftBodyVertexIterator v = inVertices, sbv_end = inVertices + inNumVertices; v != sbv_end; ++v)
		if (v.GetInvMass() > 0.0f)
		{
			...
			if (v.UpdatePenetration(penetration))
			{
				...
				Plane plane = Plane::FromPointAndNormal(closest_point, normal);
...
void ConvexHullShape::GetTrianglesStart(GetTrianglesContext &ioContext, const AABox &inBox, Vec3Arg inPositionCOM, QuatArg inRotation, Vec3Arg inScale) const
{
	...
	new (&ioContext) CHSGetTrianglesContext(Mat44::RotationTranslation(inRotation, inPositionCOM) * Mat44::Scale(inScale), ScaleHelpers::IsInsideOut(inScale));
}
...
void ConvexHullShape::Register()
{
	ShapeFunctions &f = ShapeFunctions::Get(EShapeSubType::ConvexHull);
...
Vec3 CylinderShape::GetSurfaceNormal(const SubShapeID &inSubShapeID, Vec3Arg inLocalSurfacePosition) const
{
	...
	if (distance_to_curved_surface < distance_to_top_or_bottom)
		...
		return inLocalSurfacePosition.GetY() > 0.0f? Vec3::AxisY() : -Vec3::AxisY();
}
...
{
	DebugRenderer::EDrawMode draw_mode = inDrawWireframe? DebugRenderer::EDrawMode::Wireframe : DebugRenderer::EDrawMode::Solid;
	inRenderer->DrawCylinder(inCenterOfMassTransform * Mat44::Scale(inScale.Abs()), mHalfHeight, mRadius, inUseMaterialColors? GetMaterial()->GetDebugColor() : inColor, DebugRenderer::ECastShadow::On, draw_mode);
}
#endif // MOSS_DEBUG_RENDERER
...
void CylinderShape::CollideSoftBodyVertices(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale, const CollideSoftBodyVertexIterator &inVertices, uint32 inNumVertices, int inCollidingShapeIndex) const
{
	...
	for (CollideSoftBodyVertexIterator v = inVertices, sbv_end = inVertices + inNumVertices; v != sbv_end; ++v)
		if (v.GetInvMass() > 0.0f)
		{
			...
			{
				// Side surface is closest
				normal = side_normal_length > 0.0f? side_normal / side_normal_length : Vec3::AxisX();
				point = radius * normal;
			}
			else
			{
...
void CylinderShape::Register()
{
	ShapeFunctions &f = ShapeFunctions::Get(EShapeSubType::Cylinder);
...
MassProperties EmptyShape::GetMassProperties() const
{
	...
	mass_properties.mInertia = Mat44::Identity();
...
void EmptyShape::Register()
{
	...
	{
		CollisionDispatch::RegisterCollideShape(EShapeSubType::Empty, s, collide_empty);
		CollisionDispatch::RegisterCollideShape(s, EShapeSubType::Empty, collide_empty);

		CollisionDispatch::RegisterCastShape(EShapeSubType::Empty, s, cast_empty);
		CollisionDispatch::RegisterCastShape(s, EShapeSubType::Empty, cast_empty);
	}
}
...
void HeightFieldShape::Draw(DebugRenderer *inRenderer, RMat44Arg inCenterOfMassTransform, Vec3Arg inScale, ColorArg inColor, bool inUseMaterialColors, bool inDrawWireframe) const
{
	...
	if (mGeometry.empty())
	{
		...
		for (uint32 by = 0; by < mSampleCount; by += block_size)
			for (uint32 bx = 0; bx < mSampleCount; bx += block_size)
			{
				...
				for (uint32 y = by, max_y = min(by + block_size, mSampleCount - 1); y < max_y; ++y)
					for (uint32 x = bx, max_x = min(bx + block_size, mSampleCount - 1); x < max_x; ++x)
						if (!IsNoCollision(x, y) && !IsNoCollision(x + 1, y + 1))
						{
							Vec3 x1y1 = GetPosition(x, y);
							Vec3 x2y2 = GetPosition(x + 1, y + 1);
							Color color = inUseMaterialColors? GetMaterial(x, y)->GetDebugColor() : Color::White;
				...
				// Create batch
				if (num_triangles > 0)
					mGeometry.push_back(new DebugRenderer::Geometry(inRenderer->CreateTriangleBatch(triangles), DebugRenderer::CalculateBounds(&triangles[0].mV[0], int(3 * num_triangles))));
			}
	}
	...
	if (sDrawTriangleOutlines)
	{
		struct Visitor
		{
			...
			{
				UVec4 valid = Vec4::LessOrEqual(inBoundsMinY, inBoundsMaxY);
			...
			MOSS_INLINE void			VisitTriangle(uint32 inX, uint32 inY, uint32 inTriangle, Vec3Arg inV0, Vec3Arg inV1, Vec3Arg inV2) const
			{
				...
				for (uint32 edge_idx = 0; edge_idx < 3; ++edge_idx)
				{
					...
					if (active_edges & (1 << edge_idx))
						mRenderer->DrawArrow(v1, v2, Color::Green, 0.01f);
					else
						mRenderer->DrawLine(v1, v2, Color::Grey);
				}
			}
...
public:
	...
	MOSS_INLINE void				WalkHeightField(Visitor &ioVisitor)
	{
		...
		MOSS_IF_ENABLE_ASSERTS(UVec4 dummy = UVec4::Replicate(0);)
		MOSS_ASSERT(ioVisitor.VisitRangeBlock(Vec4::Replicate(-1.0e6f), Vec4::Replicate(1.0e6f), Vec4::Replicate(-1.0e6f), Vec4::Replicate(1.0e6f), Vec4::Replicate(-1.0e6f), Vec4::Replicate(1.0e6f), dummy, 0) == 0);

		// Precalculate values relating to sample count
		uint32 sample_count = mShape->mSampleCount;
		UVec4 sample_count_min_1 = UVec4::Replicate(sample_count - 1);
		...
		do
		{
			...
			if (level >= max_level)
			{
				...
				for (int block = 0; block < 4; ++block)
				{
					...
					Vec3 value_min = Vec3::Replicate(cLargeFloat);
					Vec3 value_max = Vec3::Replicate(-cLargeFloat);
					...
					for (uint32 block_y = 0; block_y < size_y_plus_1; ++block_y)
					{
						for (uint32 block_x = 0; block_x < size_x_plus_1; ++block_x)
						{
							if (!*src_no_collision)
							{
								value_min = Vec3::Min(value_min, *src_vertex);
								value_max = Vec3::Max(value_max, *src_vertex);
							}
							++src_vertex;
							++src_no_collision;
						}
				...
				for (int block = 0; block < 4; ++block)
				{
					...
					if (bounds.IsValid())
						DebugRenderer::Instance->DrawWireBox(bounds, Color::Yellow);
				}
					...
					for (uint32 v_y = block_min_y; v_y < block_max_y; ++v_y)
						for (uint32 v_x = block_min_x; v_x < block_max_x; ++v_x)
						{
							...
							if (!start_no_collision[0] && !start_no_collision[block_size_plus_1 + 1])
							{
								...
								#ifdef MOSS_DEBUG_HEIGHT_FIELD
									DebugRenderer::Instance->DrawWireTriangle(RVec3(v0), RVec3(v1), RVec3(v2), Color::White);
								#endif
			...
			else
			{
				...
				UVec4 block = UVec4::LoadInt4Aligned(reinterpret_cast<const uint32 *>(&mShape->mRangeBlocks[offset]));
				...
				UVec4 internal_cell_size = UVec4::Replicate(block_size << (max_level - level - 1)); // subtract 1 from level because we have an internal grid of 2x2

				// Calculate min/max x and z
				UVec4 two_x = UVec4::Replicate(2 * x); // multiply by two because we have an internal grid of 2x2
				Vec4 bounds_minx = ox + sx * (internal_cell_size * (two_x + UVec4(0, 1, 0, 1))).ToFloat();
				Vec4 bounds_maxx = ox + sx * UVec4::Min(internal_cell_size * (two_x + UVec4(1, 2, 1, 2)), sample_count_min_1).ToFloat();

				UVec4 two_y = UVec4::Replicate(2 * y);
				Vec4 bounds_minz = oz + sz * (internal_cell_size * (two_y + UVec4(0, 0, 1, 1))).ToFloat();
				Vec4 bounds_maxz = oz + sz * UVec4::Min(internal_cell_size * (two_y + UVec4(1, 1, 2, 2)), sample_count_min_1).ToFloat();

				// Calculate properties of child blocks
				UVec4 properties = UVec4::Replicate(((level + 1) << cLevelShift) + (y << (cNumBitsXY + 1)) + (x << 1)) + UVec4(0, 1, 1 << cNumBitsXY, (1 << cNumBitsXY) + 1);
				...
				for (int i = 0; i < 4; ++i)
				{
					...
					if (b.IsValid())
						DebugRenderer::Instance->DrawWireBox(b, Color::Green);
				}
			#endif
...
void HeightFieldShape::CastRay(const RayCast &inRay, const RayCastSettings &inRayCastSettings, const SubShapeIDCreator &inSubShapeIDCreator, CastRayCollector &ioCollector, const ShapeFilter &inShapeFilter) const
{
	...
	struct Visitor
	{
		...
		MOSS_INLINE void			VisitTriangle(uint32 inX, uint32 inY, uint32 inTriangle, Vec3Arg inV0, Vec3Arg inV1, Vec3Arg inV2) const
		{
			...
			{
				RayCastResult hit;
				hit.mBodyID = TransformedShape::GetBodyID(mCollector.GetContext());
...
void HeightFieldShape::CollideSoftBodyVertices(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale, const CollideSoftBodyVertexIterator &inVertices, uint32 inNumVertices, int inCollidingShapeIndex) const
{
	...
	struct Visitor : public CollideSoftBodyVerticesVsTriangles
	{
		...
		MOSS_INLINE int	VisitRangeBlock(Vec4Arg inBoundsMinX, Vec4Arg inBoundsMinY, Vec4Arg inBoundsMinZ, Vec4Arg inBoundsMaxX, Vec4Arg inBoundsMaxY, Vec4Arg inBoundsMaxZ, UVec4 &ioProperties, int inStackTop)
		{
			...
			dist_sq = Vec4::Select(Vec4::Replicate(FLT_MAX), dist_sq, Vec4::LessOrEqual(inBoundsMinY, inBoundsMaxY));
...
void HeightFieldShape::CastConvexVsHeightField(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, [[maybe_unused]] const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	MOSS_PROFILE_FUNCTION();
	...
	struct Visitor : public CastConvexVsTriangles
	{
		...
		MOSS_INLINE int				VisitRangeBlock(Vec4Arg inBoundsMinX, Vec4Arg inBoundsMinY, Vec4Arg inBoundsMinZ, Vec4Arg inBoundsMaxX, Vec4Arg inBoundsMaxY, Vec4Arg inBoundsMaxZ, UVec4 &ioProperties, int inStackTop)
		{
			...
			// Clear distance for invalid bounds
			distance = Vec4::Select(Vec4::Replicate(FLT_MAX), distance, Vec4::LessOrEqual(inBoundsMinY, inBoundsMaxY));
...
void HeightFieldShape::CastSphereVsHeightField(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, [[maybe_unused]] const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	MOSS_PROFILE_FUNCTION();
	...
	struct Visitor : public CastSphereVsTriangles
	{
		...
		MOSS_INLINE int				VisitRangeBlock(Vec4Arg inBoundsMinX, Vec4Arg inBoundsMinY, Vec4Arg inBoundsMinZ, Vec4Arg inBoundsMaxX, Vec4Arg inBoundsMaxY, Vec4Arg inBoundsMaxZ, UVec4 &ioProperties, int inStackTop)
		{
			...
			AABox4EnlargeWithExtent(Vec3::Replicate(mRadius), bounds_min_x, bounds_min_y, bounds_min_z, bounds_max_x, bounds_max_y, bounds_max_z);
...
struct HeightFieldShape::HSGetTrianglesContext
{
		...
		mDecodeCtx(inShape),
		mShape(inShape),
		mLocalBox(Mat44::InverseRotationTranslation(inRotation, inPositionCOM), inBox),
	...
	int		VisitRangeBlock(Vec4Arg inBoundsMinX, Vec4Arg inBoundsMinY, Vec4Arg inBoundsMinZ, Vec4Arg inBoundsMaxX, Vec4Arg inBoundsMaxY, Vec4Arg inBoundsMaxZ, UVec4 &ioProperties, [[maybe_unused]] int inStackTop) const
	{
		...
		collides = UVec4::And(collides, Vec4::LessOrEqual(inBoundsMinY, inBoundsMaxY));
...
void HeightFieldShape::CollideConvexVsHeightField(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, [[maybe_unused]] const ShapeFilter &inShapeFilter)
{
	MOSS_PROFILE_FUNCTION();
...
void HeightFieldShape::CollideSphereVsHeightField(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, [[maybe_unused]] const ShapeFilter &inShapeFilter)
{
	MOSS_PROFILE_FUNCTION();
...
void HeightFieldShape::Register()
{
	ShapeFunctions &f = ShapeFunctions::Get(EShapeSubType::HeightField);
	f.mConstruct = []() -> Shape * { return new HeightFieldShape; };
	f.mColor = Color::Purple;

	for (EShapeSubType s : sConvexSubShapeTypes)
	{
		CollisionDispatch::RegisterCollideShape(s, EShapeSubType::HeightField, sCollideConvexVsHeightField);
		CollisionDispatch::RegisterCastShape(s, EShapeSubType::HeightField, sCastConvexVsHeightField);

		CollisionDispatch::RegisterCastShape(EShapeSubType::HeightField, s, CollisionDispatch::sReversedCastShape);
		CollisionDispatch::RegisterCollideShape(EShapeSubType::HeightField, s, CollisionDispatch::sReversedCollideShape);
	}

	// Specialized collision functions
	CollisionDispatch::RegisterCollideShape(EShapeSubType::Sphere, EShapeSubType::HeightField, sCollideSphereVsHeightField);
	CollisionDispatch::RegisterCastShape(EShapeSubType::Sphere, EShapeSubType::HeightField, sCastSphereVsHeightField);
}
...
void MeshShape::FindActiveEdges(const MeshShapeSettings &inSettings, IndexedTriangleList &ioIndices) {
	...
	for (const EdgeToTriangle::value_type &edge : edge_to_triangle)
	{
		...
		else if (edge.second.mNumTriangles == 2)
		{
			...
			Plane triangle1_plane = Plane::FromPointsCCW(triangle1_e1, triangle1_e2, triangle1_op);
			...
			Plane triangle2_plane = Plane::FromPointsCCW(triangle2_e1, triangle2_e2, triangle2_op);
...
MassProperties MeshShape::GetMassProperties() const
{
	...
	// BodyCreationSettings::mMassPropertiesOverride.SetMassAndInertiaOfSolidBox(Vec3::One(), 1000.0f);
	//
...
void MeshShape::DecodeSubShapeID(const SubShapeID &inSubShapeID, const void *&outTriangleBlock, uint32 &outTriangleIndex) const
{
	...
	uint32 block_id = inSubShapeID.PopID(NodeCodec::DecodingContext::TriangleBlockIDBits(sGetNodeHeader(mTree)), triangle_idx_subshape_id);
	outTriangleBlock = NodeCodec::DecodingContext::GetTriangleBlockStart(&mTree[0], block_id);
...
uint32 MeshShape::GetMaterialIndex(const SubShapeID &inSubShapeID) const
{
	...
	// Fetch the flags
	uint8 flags = TriangleCodec::DecodingContext::GetFlags(block_start, triangle_idx);
...
void MeshShape::Draw(DebugRenderer *inRenderer, RMat44Arg inCenterOfMassTransform, Vec3Arg inScale, ColorArg inColor, bool inUseMaterialColors, bool inDrawWireframe) const
{
	...
	if (mGeometry == nullptr)
	{
		struct Visitor
		{
			...
			MOSS_INLINE int		VisitNodes(Vec4Arg inBoundsMinX, Vec4Arg inBoundsMinY, Vec4Arg inBoundsMinZ, Vec4Arg inBoundsMaxX, Vec4Arg inBoundsMaxY, Vec4Arg inBoundsMaxZ, UVec4 &ioProperties, int inStackTop)
			{
				UVec4 valid = UVec4::Or(UVec4::Or(Vec4::Less(inBoundsMinX, inBoundsMaxX), Vec4::Less(inBoundsMinY, inBoundsMaxY)), Vec4::Less(inBoundsMinZ, inBoundsMaxZ));
				return CountAndSortTrues(valid, ioProperties);
			}
			...
			MOSS_INLINE void		VisitTriangles(const TriangleCodec::DecodingContext &ioContext, const void *inTriangles, int inNumTriangles, [[maybe_unused]] uint32 inTriangleBlockID)
			{
				...
				if (mDrawTriangleGroups || !mUseMaterialColors || mMaterials.empty())
				{
					...
					Color color = mDrawTriangleGroups? Color::GetDistinctColor(mColorIdx++) : (mUseMaterialColors? PhysicsMaterial::Default->GetDebugColor() : Color::sWhite);
				...
				else
				{
					// Per triangle color
					uint8 flags[MaxTrianglesPerLeaf];
					TriangleCodec::DecodingContext::GetFlags(inTriangles, inNumTriangles, flags);
	...
	// Draw the geometry
	inRenderer->DrawGeometry(inCenterOfMassTransform * Mat44::Scale(inScale), inColor, mGeometry, cull_mode, DebugRenderer::ECastShadow::On, draw_mode);

	if (sDrawTriangleOutlines) {
		struct Visitor {
			MOSS_INLINE Visitor(DebugRenderer *inRenderer, RMat44Arg inTransform) : mRenderer(inRenderer), mTransform(inTransform) { }
...
bool MeshShape::CastRay(const RayCast &inRay, const SubShapeIDCreator &inSubShapeIDCreator, RayCastResult &ioHit) const
{
	...
	visitor.mTriangleBlockIDBits = NodeCodec::DecodingContext::TriangleBlockIDBits(sGetNodeHeader(mTree));
...
void MeshShape::CastConvexVsMesh(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, [[maybe_unused]] const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	MOSS_PROFILE_FUNCTION();
...
void MeshShape::CastSphereVsMesh(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, [[maybe_unused]] const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	MOSS_PROFILE_FUNCTION();
	...
	MOSS_INLINE void	VisitTriangles(const TriangleCodec::DecodingContext &ioContext, const void *inTriangles, int inNumTriangles, [[maybe_unused]] uint32 inTriangleBlockID)
	{
		...
		if (mMaterials != nullptr)
		{
			if (mShape->mMaterials.empty())
			{
				...
				const PhysicsMaterial *default_material = PhysicsMaterial::Default;
...
void MeshShape::CollideConvexVsMesh(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, [[maybe_unused]] const ShapeFilter &inShapeFilter)
{
	MOSS_PROFILE_FUNCTION();
...
void MeshShape::CollideSphereVsMesh(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, [[maybe_unused]] const ShapeFilter &inShapeFilter)
{
	MOSS_PROFILE_FUNCTION();
...
void MeshShape::Register()
{
	ShapeFunctions &f = ShapeFunctions::Get(EShapeSubType::Mesh);
	f.mConstruct = []() -> Shape * { return new MeshShape; };
	f.mColor = Color::Red;

	for (EShapeSubType s : sConvexSubShapeTypes)
	{
		CollisionDispatch::RegisterCollideShape(s, EShapeSubType::Mesh, sCollideConvexVsMesh);
		CollisionDispatch::RegisterCastShape(s, EShapeSubType::Mesh, sCastConvexVsMesh);

		CollisionDispatch::RegisterCastShape(EShapeSubType::Mesh, s, CollisionDispatch::ReversedCastShape);
		CollisionDispatch::RegisterCollideShape(EShapeSubType::Mesh, s, CollisionDispatch::ReversedCollideShape);
	}

	// Specialized collision functions
	CollisionDispatch::RegisterCollideShape(EShapeSubType::Sphere, EShapeSubType::Mesh, sCollideSphereVsMesh);
	CollisionDispatch::RegisterCastShape(EShapeSubType::Sphere, EShapeSubType::Mesh, sCastSphereVsMesh);
}
...
void MutableCompoundShape::CalculateLocalBounds()
{
	...
	if (num_blocks > 0)
	{
		...
		{
			min_x = Vec4::Min(min_x, bounds->mMinX);
			min_y = Vec4::Min(min_y, bounds->mMinY);
			min_z = Vec4::Min(min_z, bounds->mMinZ);
			max_x = Vec4::Max(max_x, bounds->mMaxX);
			max_y = Vec4::Max(max_y, bounds->mMaxY);
			max_z = Vec4::Max(max_z, bounds->mMaxZ);
		}
	...
	else
	{
		...
		mLocalBounds.mMin = mLocalBounds.mMax = Vec3::Zero();
	}
	...
	for (uint32 sub_shape_idx_start = inStartIdx & ~uint32(3), sub_shape_idx_end = inStartIdx + inNumber; sub_shape_idx_start < sub_shape_idx_end; sub_shape_idx_start += 4)
	{
		...
		for (uint32 col = 0; col < 4; ++col)
		{
			...
			if (sub_shape_idx < mSubShapes.size()) // else reuse sub_shape_bounds from previous iteration
			{
				...
				// Transform the shape's bounds into our local space
				Mat44 transform = Mat44::RotationTranslation(sub_shape.GetRotation(), sub_shape.GetPositionCOM());

				// Get the bounding box
				sub_shape_bounds = sub_shape.mShape->GetWorldSpaceBounds(transform, Vec3::One());
			}
...
bool MutableCompoundShape::CastRay(const RayCast &inRay, const SubShapeIDCreator &inSubShapeIDCreator, RayCastResult &ioHit) const
{
	...
	struct Visitor : public CastRayVisitor
	{
		...
		{
			UVec4 closer = Vec4::Less(inResult, Vec4::Replicate(mHit.mFraction));
...
void MutableCompoundShape::CastRay(const RayCast &inRay, const RayCastSettings &inRayCastSettings, const SubShapeIDCreator &inSubShapeIDCreator, CastRayCollector &ioCollector, const ShapeFilter &inShapeFilter) const
{
	...
	struct Visitor : public CastRayVisitorCollector
	{
		...
		MOSS_INLINE bool		ShouldVisitBlock(Vec4Arg inResult) const
		{
			UVec4 closer = Vec4::Less(inResult, Vec4::Replicate(mCollector.GetEarlyOutFraction()));
			return closer.TestAnyTrue();
		}
...
void MutableCompoundShape::CastShapeVsCompound(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	...
	struct Visitor : public CastShapeVisitor
	{
		...
		MOSS_INLINE bool		ShouldVisitBlock(Vec4Arg inResult) const
		{
			UVec4 closer = Vec4::Less(inResult, Vec4::Replicate(mCollector.GetPositiveEarlyOutFraction()));
			return closer.TestAnyTrue();
		}
...
void MutableCompoundShape::CollideCompoundVsShape(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, const ShapeFilter &inShapeFilter)
{
	MOSS_PROFILE_FUNCTION();
...
void MutableCompoundShape::CollideShapeVsCompound(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, const ShapeFilter &inShapeFilter)
{
	MOSS_PROFILE_FUNCTION();
...
void MutableCompoundShape::Register()
{
	ShapeFunctions &f = ShapeFunctions::Get(EShapeSubType::MutableCompound);
	f.mConstruct = []() -> Shape * { return new MutableCompoundShape; };
	f.mColor = Color::DarkOrange;

	for (EShapeSubType s : sAllSubShapeTypes)
	{
		CollisionDispatch::RegisterCollideShape(EShapeSubType::MutableCompound, s, sCollideCompoundVsShape);
		CollisionDispatch::RegisterCollideShape(s, EShapeSubType::MutableCompound, sCollideShapeVsCompound);
		CollisionDispatch::RegisterCastShape(s, EShapeSubType::MutableCompound, sCastShapeVsCompound);
	}
}
...
MassProperties RotatedTranslatedShape::GetMassProperties() const
{
	...
	p.Rotate(Mat44::Rotation(mRotation));
	return p;
}

AABox RotatedTranslatedShape::GetLocalBounds() const
{
	return mInnerShape->GetLocalBounds().Transformed(Mat44::Rotation(mRotation));
}

AABox RotatedTranslatedShape::GetWorldSpaceBounds(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale) const
{
	Mat44 transform = inCenterOfMassTransform * Mat44::Rotation(mRotation);
...
{
	// Transform surface position to local space and pass call on
	Mat44 transform = Mat44::Rotation(mRotation.Conjugated());
...
{
	Mat44 transform = Mat44::Rotation(mRotation);
...
{
	mInnerShape->Draw(inRenderer, inCenterOfMassTransform * Mat44::Rotation(mRotation), TransformScale(inScale), inColor, inUseMaterialColors, inDrawWireframe);
}
...
{
	mInnerShape->DrawGetSupportFunction(inRenderer, inCenterOfMassTransform * Mat44::Rotation(mRotation), TransformScale(inScale), inColor, inDrawSupportDirection);
}
...
{
	mInnerShape->DrawGetSupportingFace(inRenderer, inCenterOfMassTransform * Mat44::Rotation(mRotation), TransformScale(inScale));
}
#endif // MOSS_DEBUG_RENDERER
...
{
	mInnerShape->TransformShape(inCenterOfMassTransform * Mat44::Rotation(mRotation), ioCollector);
}

void RotatedTranslatedShape::CollideRotatedTranslatedVsShape(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, const ShapeFilter &inShapeFilter)
{
	...
	Mat44 transform1 = inCenterOfMassTransform1 * Mat44::Rotation(shape1->mRotation);

	CollisionDispatch::CollideShapeVsShape(shape1->mInnerShape, inShape2, shape1->TransformScale(inScale1), inScale2, transform1, inCenterOfMassTransform2, inSubShapeIDCreator1, inSubShapeIDCreator2, inCollideShapeSettings, ioCollector, inShapeFilter);
}

void RotatedTranslatedShape::CollideShapeVsRotatedTranslated(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, const ShapeFilter &inShapeFilter)
{
	...
	Mat44 transform2 = inCenterOfMassTransform2 * Mat44::Rotation(shape2->mRotation);

	CollisionDispatch::CollideShapeVsShape(inShape1, shape2->mInnerShape, inScale1, shape2->TransformScale(inScale2), inCenterOfMassTransform1, transform2, inSubShapeIDCreator1, inSubShapeIDCreator2, inCollideShapeSettings, ioCollector, inShapeFilter);
}

void RotatedTranslatedShape::CollideRotatedTranslatedVsRotatedTranslated(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, const ShapeFilter &inShapeFilter)
{
	MOSS_ASSERT(inShape1->GetSubType() == EShapeSubType::RotatedTranslated);
	const RotatedTranslatedShape *shape1 = static_cast<const RotatedTranslatedShape *>(inShape1);
	MOSS_ASSERT(inShape2->GetSubType() == EShapeSubType::RotatedTranslated);
	...
	CollisionDispatch::CollideShapeVsShape(shape1->mInnerShape, shape2->mInnerShape, shape1->TransformScale(inScale1), shape2->TransformScale(inScale2), transform1, transform2, inSubShapeIDCreator1, inSubShapeIDCreator2, inCollideShapeSettings, ioCollector, inShapeFilter);
}

void RotatedTranslatedShape::CastRotatedTranslatedVsShape(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	...
	// Transform the shape cast and update the shape
	Mat44 transform = inShapeCast.mCenterOfMassStart * Mat44::Rotation(shape1->mRotation);
	...
	CollisionDispatch::CastShapeVsShapeLocalSpace(shape_cast, inShapeCastSettings, inShape, inScale, inShapeFilter, inCenterOfMassTransform2, inSubShapeIDCreator1, inSubShapeIDCreator2, ioCollector);
}

void RotatedTranslatedShape::CastShapeVsRotatedTranslated(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	...
	Mat44 local_transform = Mat44::Rotation(shape->mRotation);

	// Transform the shape cast
	ShapeCast shape_cast = inShapeCast.PostTransformed(local_transform.Transposed3x3());

	CollisionDispatch::CastShapeVsShapeLocalSpace(shape_cast, inShapeCastSettings, shape->mInnerShape, shape->TransformScale(inScale), inShapeFilter, inCenterOfMassTransform2 * local_transform, inSubShapeIDCreator1, inSubShapeIDCreator2, ioCollector);
}

void RotatedTranslatedShape::CastRotatedTranslatedVsRotatedTranslated(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	MOSS_ASSERT(inShapeCast.mShape->GetSubType() == EShapeSubType::RotatedTranslated);
	const RotatedTranslatedShape *shape1 = static_cast<const RotatedTranslatedShape *>(inShapeCast.mShape);
	MOSS_ASSERT(inShape->GetSubType() == EShapeSubType::RotatedTranslated);
	...
	Mat44 local_transform2 = Mat44::Rotation(shape2->mRotation);
	Mat44 local_transform2_transposed = local_transform2.Transposed3x3();

	// Transform the shape cast and update the shape
	Mat44 transform = (local_transform2_transposed * inShapeCast.mCenterOfMassStart) * Mat44::Rotation(shape1->mRotation);
	Vec3 scale = shape1->TransformScale(inShapeCast.mScale);
	ShapeCast shape_cast(shape1->mInnerShape, scale, transform, local_transform2_transposed.Multiply3x3(inShapeCast.mDirection));

	CollisionDispatch::CastShapeVsShapeLocalSpace(shape_cast, inShapeCastSettings, shape2->mInnerShape, shape2->TransformScale(inScale), inShapeFilter, inCenterOfMassTransform2 * local_transform2, inSubShapeIDCreator1, inSubShapeIDCreator2, ioCollector);
}
...
void RotatedTranslatedShape::Register()
{
	ShapeFunctions &f = ShapeFunctions::Get(EShapeSubType::RotatedTranslated);
	f.mConstruct = []() -> Shape * { return new RotatedTranslatedShape; };
	f.mColor = Color::Blue;

	for (EShapeSubType s : sAllSubShapeTypes)
	{
		CollisionDispatch::RegisterCollideShape(EShapeSubType::RotatedTranslated, s, sCollideRotatedTranslatedVsShape);
		CollisionDispatch::RegisterCollideShape(s, EShapeSubType::RotatedTranslated, sCollideShapeVsRotatedTranslated);
		CollisionDispatch::RegisterCastShape(EShapeSubType::RotatedTranslated, s, sCastRotatedTranslatedVsShape);
		CollisionDispatch::RegisterCastShape(s, EShapeSubType::RotatedTranslated, sCastShapeVsRotatedTranslated);
	}

	CollisionDispatch::RegisterCollideShape(EShapeSubType::RotatedTranslated, EShapeSubType::RotatedTranslated, sCollideRotatedTranslatedVsRotatedTranslated);
	CollisionDispatch::RegisterCastShape(EShapeSubType::RotatedTranslated, EShapeSubType::RotatedTranslated, sCastRotatedTranslatedVsRotatedTranslated);
}
...
{
	outPerp1 = inNormal.Cross(Vec3::AxisY()).NormalizedOr(Vec3::AxisX());
...
void PlaneShape::CastConvexVsPlane(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, [[maybe_unused]] const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	MOSS_PROFILE_FUNCTION();
	...
	ShapeCastResult result(fraction, point1, point2, penetration_axis_world, back_facing, inSubShapeIDCreator1.GetID(), inSubShapeIDCreator2.GetID(), TransformedShape::GetBodyID(ioCollector.GetContext()));
...
void PlaneShape::CollideConvexVsPlane(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, [[maybe_unused]] const ShapeFilter &inShapeFilter)
{
	MOSS_PROFILE_FUNCTION();
...
void PlaneShape::Register()
{
	ShapeFunctions &f = ShapeFunctions::Get(EShapeSubType::Plane);
	f.mConstruct = []() -> Shape * { return new PlaneShape; };
	f.mColor = Color::DarkRed;

	for (EShapeSubType s : sConvexSubShapeTypes)
	{
		CollisionDispatch::RegisterCollideShape(s, EShapeSubType::Plane, sCollideConvexVsPlane);
		CollisionDispatch::RegisterCastShape(s, EShapeSubType::Plane, sCastConvexVsPlane);

		CollisionDispatch::RegisterCastShape(EShapeSubType::Plane, s, CollisionDispatch::ReversedCastShape);
		CollisionDispatch::RegisterCollideShape(EShapeSubType::Plane, s, CollisionDispatch::ReversedCollideShape);
	}
}
...
{
	mInnerShape->TransformShape(inCenterOfMassTransform * Mat44::Scale(mScale), ioCollector);
}
...
void ScaledShape::CollideScaledVsShape(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, const ShapeFilter &inShapeFilter)
{
	MOSS_ASSERT(inShape1->GetSubType() == EShapeSubType::Scaled);
	const ScaledShape *shape1 = static_cast<const ScaledShape *>(inShape1);

	CollisionDispatch::CollideShapeVsShape(shape1->GetInnerShape(), inShape2, inScale1 * shape1->GetScale(), inScale2, inCenterOfMassTransform1, inCenterOfMassTransform2, inSubShapeIDCreator1, inSubShapeIDCreator2, inCollideShapeSettings, ioCollector, inShapeFilter);
}

void ScaledShape::CollideShapeVsScaled(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, const ShapeFilter &inShapeFilter)
{
	MOSS_ASSERT(inShape2->GetSubType() == EShapeSubType::Scaled);
	const ScaledShape *shape2 = static_cast<const ScaledShape *>(inShape2);

	CollisionDispatch::CollideShapeVsShape(inShape1, shape2->GetInnerShape(), inScale1, inScale2 * shape2->GetScale(), inCenterOfMassTransform1, inCenterOfMassTransform2, inSubShapeIDCreator1, inSubShapeIDCreator2, inCollideShapeSettings, ioCollector, inShapeFilter);
}

void ScaledShape::CastScaledVsShape(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	...
	CollisionDispatch::CastShapeVsShapeLocalSpace(scaled_cast, inShapeCastSettings, inShape, inScale, inShapeFilter, inCenterOfMassTransform2, inSubShapeIDCreator1, inSubShapeIDCreator2, ioCollector);
}

void ScaledShape::CastShapeVsScaled(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	MOSS_ASSERT(inShape->GetSubType() == EShapeSubType::Scaled);
	const ScaledShape *shape = static_cast<const ScaledShape *>(inShape);

	CollisionDispatch::CastShapeVsShapeLocalSpace(inShapeCast, inShapeCastSettings, shape->mInnerShape, inScale * shape->mScale, inShapeFilter, inCenterOfMassTransform2, inSubShapeIDCreator1, inSubShapeIDCreator2, ioCollector);
}

void ScaledShape::Register()
{
	ShapeFunctions &f = ShapeFunctions::Get(EShapeSubType::Scaled);
	f.mConstruct = []() -> Shape * { return new ScaledShape; };
	f.mColor = Color::Yellow;

	for (EShapeSubType s : sAllSubShapeTypes)
	{
		CollisionDispatch::RegisterCollideShape(EShapeSubType::Scaled, s, sCollideScaledVsShape);
		CollisionDispatch::RegisterCollideShape(s, EShapeSubType::Scaled, sCollideShapeVsScaled);
		CollisionDispatch::RegisterCastShape(EShapeSubType::Scaled, s, sCastScaledVsShape);
		CollisionDispatch::RegisterCastShape(s, EShapeSubType::Scaled, sCastShapeVsScaled);
	}
}
...
void OffsetCenterOfMassShape::CollideOffsetCenterOfMassVsShape(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, const ShapeFilter &inShapeFilter)
{
	...
	CollisionDispatch::CollideShapeVsShape(shape1->mInnerShape, inShape2, inScale1, inScale2, inCenterOfMassTransform1.PreTranslated(-inScale1 * shape1->mOffset), inCenterOfMassTransform2, inSubShapeIDCreator1, inSubShapeIDCreator2, inCollideShapeSettings, ioCollector, inShapeFilter);
}

void OffsetCenterOfMassShape::CollideShapeVsOffsetCenterOfMass(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, const ShapeFilter &inShapeFilter)
{
	...
	CollisionDispatch::CollideShapeVsShape(inShape1, shape2->mInnerShape, inScale1, inScale2, inCenterOfMassTransform1, inCenterOfMassTransform2.PreTranslated(-inScale2 * shape2->mOffset), inSubShapeIDCreator1, inSubShapeIDCreator2, inCollideShapeSettings, ioCollector, inShapeFilter);
}

void OffsetCenterOfMassShape::CastOffsetCenterOfMassVsShape(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
...
void OffsetCenterOfMassShape::CastShapeVsOffsetCenterOfMass(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	...
	// Transform the shape cast
	ShapeCast shape_cast = inShapeCast.PostTransformed(Mat44::Translation(inScale * shape->mOffset));

	CollisionDispatch::CastShapeVsShapeLocalSpace(shape_cast, inShapeCastSettings, shape->mInnerShape, inScale, inShapeFilter, inCenterOfMassTransform2.PreTranslated(-inScale * shape->mOffset), inSubShapeIDCreator1, inSubShapeIDCreator2, ioCollector);
}
...
void OffsetCenterOfMassShape::Register() {
	ShapeFunctions &f = ShapeFunctions::Get(EShapeSubType::OffsetCenterOfMass);
	f.mConstruct = []() -> Shape* { return new OffsetCenterOfMassShape; };
	f.mColor = Color::Cyan;

	for (EShapeSubType s : sAllSubShapeTypes)
	{
		CollisionDispatch::RegisterCollideShape(EShapeSubType::OffsetCenterOfMass, s, sCollideOffsetCenterOfMassVsShape);
		CollisionDispatch::RegisterCollideShape(s, EShapeSubType::OffsetCenterOfMass, sCollideShapeVsOffsetCenterOfMass);
		CollisionDispatch::RegisterCastShape(EShapeSubType::OffsetCenterOfMass, s, sCastOffsetCenterOfMassVsShape);
		CollisionDispatch::RegisterCastShape(s, EShapeSubType::OffsetCenterOfMass, sCastShapeVsOffsetCenterOfMass);
	}
}
