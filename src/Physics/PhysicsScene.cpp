...
#include <Moss/Physics/PhysicsScene.h>
...
{
	MOSS_ADD_ATTRIBUTE(PhysicsScene, mBodies)
	MOSS_ADD_ATTRIBUTE(PhysicsScene, mConstraints)
	MOSS_ADD_ATTRIBUTE(PhysicsScene, mSoftBodies)
}

MOSS_IMPLEMENT_SERIALIZABLE_NON_VIRTUAL(PhysicsScene::ConnectedConstraint)
{
	MOSS_ADD_ATTRIBUTE(PhysicsScene::ConnectedConstraint, mSettings)
	MOSS_ADD_ATTRIBUTE(PhysicsScene::ConnectedConstraint, mBody1)
	MOSS_ADD_ATTRIBUTE(PhysicsScene::ConnectedConstraint, mBody2)
}
...
PhysicsScene::PhysicsSceneResult PhysicsScene::RestoreFromBinaryState(StreamIn &inStream)
{
	...
	{
		ConstraintSettings::ConstraintResult c_result = ConstraintSettings::RestoreFromBinaryState(inStream);
	...
	{
		// Read creation settings
		SoftBodyCreationSettings::SBCSResult sbcs_result = SoftBodyCreationSettings::RestoreWithChildren(inStream, id_to_settings, id_to_material, id_to_group_filter);
...
void PhysicsScene::FromPhysicsSystem(const PhysicsSystem *inSystem)
{
	...
	using BodyIDToIdxMap = TMap<BodyID, uint32>;
	...
	for (const Constraint *c : constraints)
		if (c->GetType() == EConstraintType::TwoBodyConstraint)
		{
			...
			MOSS_ASSERT(b1 != body_id_to_idx.end() && b2 != body_id_to_idx.end());
