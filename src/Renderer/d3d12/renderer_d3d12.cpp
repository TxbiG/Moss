...
typedef HRESULT Moss_Result;
void Moss_GPUFatalError(Moss_Result inHResult) { if (FAILED(inHResult)) { string message = system_category().message(inHResult); MOSS_FATAL("DirectX error returned: %s (%s)", ConvertToString(inHResult).c_str(), message.c_str()); } }
