...
#define ENET_CONTEXT_ENCODE_EXCLUDE(context, value_, under, total, minimum)                 \
    ENET_CONTEXT_WALK(context, {                                                            \
        if (node -> value != value_) {                                                      \
            uint16 parentCount = rangeCoder -> symbols [node -> parent].count + minimum;    \
            if (node -> value < value_) under -= parentCount; total -= parentCount;         \
        }})

typedef struct _ENetExclude { uint8 value; uint16 under; } ENetExclude;
...
#define ENET_CONTEXT_DECODE(context, symbol_, code, value_, under_, count_, update, minimum, createRoot, visitNode, createRight, createLeft) {  \
    under_ = 0;                                                                                                                                 \
    count_ = minimum;                                                                                                                           \
    if (! (context) -> symbols) { createRoot; }                                                                                                 \
    else {                                                                                                                                      \
        ENetSymbol * node = (context) + (context) -> symbols;                                                                                   \
        for (;;) {                                                                                                                              \
            uint16 after = under_ + node -> under + (node -> value + 1)*minimum, before = node -> count + minimum;                              \
            visitNode;                                                                                                                          \
            if (code >= after) {                                                                                                                \
                under_ += node -> under;                                                                                                        \
                if (node -> right) { node += node -> right; continue; }                                                                         \
...
#define ENET_CONTEXT_ROOT_DECODE(context, symbol_, code, value_, under_, count_, update, minimum, exclude) ENET_CONTEXT_DECODE(context, symbol_, code, value_, under_, count_, update, minimum, {   \
        value_ = code / minimum;                                                                                                                                                                    \
...
typedef struct _ENetSymbol {
    uint8 value; uint8 count; uint16 under; uint16 left, right; /* binary indexed tree of symbols */
    uint16 symbols; uint16 escapes; uint16 total; uint16 parent;/* context defined by this symbol */
} ENetSymbol;
...
static uint16 enet_symbol_rescale (ENetSymbol * symbol)
{
    uint16 total = 0;
    for (;;)
    {
...
size_t enet_range_coder_compress (void * context, const ENetBuffer * inBuffers, size_t inBufferCount, size_t inLimit, uint8 * outData, size_t outLimit) {
    ...
    inData = (const uint8 *) inBuffers -> data;
...
#endif
        uint8 value;
        uint16 count, under, * parent = & predicted, total;
        if (inData >= inEnd)
        {
            if (inBufferCount <= 0)
              break;
...
size_t enet_range_coder_decompress (void* context, const uint8* inData, size_t inLimit, uint8* outData, size_t outLimit) {
    ...
    for (;;)
    {
...
#endif
        uint8 value = 0;
        uint16 code, under, count, bottom, * parent = & predicted, total;
                ...
                subcontext = & rangeCoder -> symbols [subcontext -> parent])
        {
            ...
            total = subcontext -> total;
#ifdef ENET_CONTEXT_EXCLUSION
            if (childContext -> total > 0) ENET_CONTEXT_DECODE_EXCLUDE (childContext, total, 0); 
#endif
...
#ifdef ENET_CONTEXT_EXCLUSION
            if (childContext->total > 0) { ENET_CONTEXT_TRY_DECODE (subcontext, symbol, code, value, under, count, ENET_SUBCONTEXT_SYMBOL_DELTA, 0, ENET_CONTEXT_EXCLUDED); }
            else
#endif
            {
        ...
        total = root -> total;
#ifdef ENET_CONTEXT_EXCLUSION
        if (childContext -> total > 0) ENET_CONTEXT_DECODE_EXCLUDE (childContext, total, ENET_CONTEXT_SYMBOL_MINIMUM);  
#endif
        code = ENET_RANGE_CODER_READ (total);
        ...
        code -= root -> escapes;
#ifdef ENET_CONTEXT_EXCLUSION
        if (childContext -> total > 0) { ENET_CONTEXT_ROOT_DECODE (root, symbol, code, value, under, count, ENET_CONTEXT_SYMBOL_DELTA, ENET_CONTEXT_SYMBOL_MINIMUM, ENET_CONTEXT_EXCLUDED); }
        else
#endif
