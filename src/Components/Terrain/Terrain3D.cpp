...
static constexpr uint32_t MAX_LODS = 4;
...
void TerrainChunk::UpdateLOD(float distanceToCamera) {
    uint32_t newLOD;

    if (distance < 150.0f)      newLOD = 0;
    else if (distance < 400.0f) newLOD = 1;
    else if (distance < 800.0f) newLOD = 2;
    else                        newLOD = 3;
