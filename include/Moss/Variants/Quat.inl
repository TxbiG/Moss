...
Quat Quat::sRandom(Random &inRandom)
{
	...
	std::uniform_real_distribution<float> zero_to_two_pi(0.0f, 2.0f * MOSS_PI);
	Vec4 s, c;
...
Quat Quat::GetTwist(const Vec3 inAxis) const
{
...
Vec3 Quat::InverseRotate(const Vec3 inValue) const
{
	MOSS_ASSERT(IsNormalized());
