...
public:
	...
	explicit			TStaticArray(std::initializer_list<T> inList)
	{
		MOSS_ASSERT(inList.size() <= N);
		for (const T &v : inList)
			new (reinterpret_cast<T *>(&mElements[mSize++])) T(v);
	}

	/// Copy constructor
	TStaticArray(const TStaticArray<T, N> &inRHS)
	{
	...
	/// Assignment operator
	TStaticArray<T, N> &	operator = (const TStaticArray<T, N> &inRHS)
	{
