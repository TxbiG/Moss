...
struct Basis {
	...
	void rotate_local(const Vec3 &p_axis, real_t p_angle);
	Basis rotated_local(const Vec3 &p_axis, real_t p_angle) const;

	void rotate(const Vec3 &p_euler, EulerOrder p_order = EulerOrder::YXZ);
	Basis rotated(const Vec3 &p_euler, EulerOrder p_order = EulerOrder::YXZ) const;

	void rotate(const Quat &p_quat);
	Basis rotated(const Quat &p_quat) const;

	Vec3 get_euler_normalized(EulerOrder p_order = EulerOrder::YXZ) const;
	void get_rotation_axis_angle(Vec3 &p_axis, real_t &p_angle) const;
	void get_rotation_axis_angle_local(Vec3 &p_axis, real_t &p_angle) const;
	Quat get_rotation_quat() const;

	void rotate_to_align(Vec3 p_start_direction, Vec3 p_end_direction);

	Vec3 rotref_posscale_decomposition(Basis &rotref) const;

	Vec3 get_euler(EulerOrder p_order = EulerOrder::YXZ) const;
	void set_euler(const Vec3 &p_euler, EulerOrder p_order = EulerOrder::YXZ);
	static Basis from_euler(const Vec3 &p_euler, EulerOrder p_order = EulerOrder::YXZ) {
		Basis b;
	...
	void get_axis_angle(Vec3 &r_axis, real_t &r_angle) const;
	void set_axis_angle(const Vec3 &p_axis, real_t p_angle);

	void scale(const Vec3 &p_scale);
	Basis scaled(const Vec3 &p_scale) const;

	void scale_local(const Vec3 &p_scale);
	Basis scaled_local(const Vec3 &p_scale) const;

	void scale_orthogonal(const Vec3 &p_scale);
	Basis scaled_orthogonal(const Vec3 &p_scale) const;
	real_t get_uniform_scale() const;

	Vec3 get_scale() const;
	Vec3 get_scale_abs() const;
	Vec3 get_scale_global() const;

	void set_axis_angle_scale(const Vec3 &p_axis, real_t p_angle, const Vec3 &p_scale);
	void set_euler_scale(const Vec3 &p_euler, const Vec3 &p_scale, EulerOrder p_order = EulerOrder::YXZ);
...
Vec3 Basis::xform(const Vec3 &p_vector) const {
	return Vec3(
			rows[0].dot(p_vector),
			rows[1].dot(p_vector),
			rows[2].dot(p_vector));
...
Vec3 Basis::xform_inv(const Vec3 &p_vector) const {
	return Vec3(
