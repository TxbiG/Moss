...
/// or in vector notation: \f$q = [w, v] = [w, x, y, z]\f$
/// Where:
/// w = the real part
...
class [[nodiscard]] alignas(MOSS_VECTOR_ALIGNMENT) Quat
{
public:
	...
	inline bool					IsClose(const Quat inRHS, float inMaxDistSq = 1.0e-12f) const		{ return mValue.IsClose(inRHS.mValue, inMaxDistSq); }
	...
	MOSS_INLINE float			GetX() const													{ return mValue.GetX(); }

	/// Get Y component (imaginary part j)
	MOSS_INLINE float			GetY() const													{ return mValue.GetY(); }

	/// Get Z component (imaginary part k)
	MOSS_INLINE float			GetZ() const													{ return mValue.GetZ(); }

	/// Get W component (real part)
	MOSS_INLINE float			GetW() const													{ return mValue.GetW(); }

	/// Get the imaginary part of the quaternion
	MOSS_INLINE Vec3			GetXYZ() const												{ return Vec3(mValue); }

	/// Get the quaternion as a Vec4
	MOSS_INLINE Vec4			GetXYZW() const												{ return mValue; }

	/// Set individual components
	MOSS_INLINE void			SetX(float inX)													{ mValue.SetX(inX); }
	MOSS_INLINE void			SetY(float inY)													{ mValue.SetY(inY); }
	MOSS_INLINE void			SetZ(float inZ)													{ mValue.SetZ(inZ); }
	MOSS_INLINE void			SetW(float inW)													{ mValue.SetW(inW); }

	/// Set all components
	MOSS_INLINE void			Set(float inX, float inY, float inZ, float inW)					{ mValue.Set(inX, inY, inZ, inW); }
	...
	MOSS_INLINE float			LengthSq() const												{ return mValue.LengthSq(); }

	/// Length of quaternion.
	/// @return Length of quaternion (\f$|v|\f$)
	MOSS_INLINE float			Length() const													{ return mValue.Length(); }

	/// Normalize the quaternion (make it length 1)
	MOSS_INLINE Quat			Normalized() const												{ return Quat(mValue.Normalized()); }
	...
	inline bool					operator == (const Quat inRHS) const							{ return mValue == inRHS.mValue; }

	/// Check if two quaternions are different
	inline bool					operator != (const Quat inRHS) const							{ return mValue != inRHS.mValue; }
	MOSS_INLINE void			operator += (const Quat inRHS)									{ mValue += inRHS.mValue; }
	MOSS_INLINE void			operator -= (const Quat inRHS)									{ mValue -= inRHS.mValue; }
	MOSS_INLINE void			operator *= (float inValue)										{ mValue *= inValue; }
	MOSS_INLINE void			operator /= (float inValue)										{ mValue /= inValue; }
	MOSS_INLINE Quat			operator - () const												{ return Quat(-mValue); }
	MOSS_INLINE Quat			operator + (const Quat inRHS) const								{ return Quat(mValue + inRHS.mValue); }
	MOSS_INLINE Quat			operator - (const Quat inRHS) const								{ return Quat(mValue - inRHS.mValue); }
	MOSS_INLINE Quat			operator * (const Quat inRHS) const;
	MOSS_INLINE Quat			operator * (float inValue) const								{ return Quat(mValue * inValue); }
	inline friend Quat			operator * (float inValue, const Quat inRHS)					{ return Quat(inRHS.mValue * inValue); }
	MOSS_INLINE Quat			operator / (float inValue) const								{ return Quat(mValue / inValue); }

	///@}

	/// Rotate a vector by this quaternion
	MOSS_INLINE Vec3				operator * (const Vec3  inValue) const;

	/// Rotate a vector by the inverse of this quaternion
	MOSS_INLINE Vec3				InverseRotate(const Vec3 inValue) const;

	/// Rotate a the vector (1, 0, 0) with this quaternion
	MOSS_INLINE Vec3		RotateAxisX() const;

	/// Rotate a the vector (0, 1, 0) with this quaternion
	MOSS_INLINE Vec3		RotateAxisY() const;

	/// Rotate a the vector (0, 0, 1) with this quaternion
	MOSS_INLINE Vec3				RotateAxisZ() const;
	...
	MOSS_INLINE float				GetRotationAngle(const Vec3 inAxis) const							{ return GetW() == 0.0f? MOSS_PI : 2.0f * ATan(GetXYZ().Dot(inAxis) / GetW()); }
	...
	MOSS_INLINE Quat				GetTwist(const Vec3 inAxis) const;
	...
	MOSS_INLINE void				GetSwingTwist(Quat &outSwing, Quat &outTwist) const;
	...
	MOSS_INLINE Quat				LERP(const Quat inDestination, float inFraction) const;
	...
	/// To String
	friend ostream &				operator << (ostream &inStream, const Quat inQ)					{ inStream << inQ.mValue; return inStream; }
