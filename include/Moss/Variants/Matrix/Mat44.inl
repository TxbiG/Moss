...
Mat44::Mat44(const Vec4  inC1, const Vec4  inC2, const Vec4 inC3, const Vec3 inC4) : mCol { inC1, inC2, inC3, Vec4(inC4, 1.0f) } { }
...
Mat44 Mat44::sTranslation(const Vec3 inV)
{
    ...
    return Mat44(
        Vec4(1, 0, 0, 0),         // 1st column
        Vec4(0, 1, 0, 0),         // 2nd column
        Vec4(0, 0, 1, 0),         // 3rd column
        Vec4(inV.GetX(), inV.GetY(), inV.GetZ(), 1) // 4th column (translation)
    );
#else
...
Mat44 Mat44::sLookAt(const Vec3 position, const Vec3 target, const Vec3 up) {
    ...
    return Mat44(
        Vec4(r.GetX(), u.GetX(), f.GetX(), 0),
        Vec4(r.GetY(), u.GetY(), f.GetY(), 0),
        Vec4(r.GetZ(), u.GetZ(), f.GetZ(), 0),
        Vec4(-r.Dot(position), -u.Dot(position), -f.Dot(position), 1)
    );
}
...
#ifdef MOSS_USE_OPENGL
    // Right-handed (OpenGL): Z range [-1, 1], camera looks down -Z
    float zRange = near - far;
    float zz = (far + near) / zRange;
    float zw = (2.0f * far * near) / zRange;

    return Mat44(
        Vec4(xScale, 0.0f,   0.0f,  0.0f),
        Vec4(0.0f,   yScale, 0.0f,  0.0f),
        Vec4(0.0f,   0.0f,   zz,   -1.0f),
        Vec4(0.0f,   0.0f,   zw,    0.0f)
    );

#else
    // Left-handed (Vulkan, DirectX): Z range [0, 1], camera looks down +Z
    float zRange = far - near;
    float zz = far / zRange;
    float zw = (-near * far) / zRange;
...
{
	MOSS_ASSERT(mCol[0][3] == 0.0f);
	MOSS_ASSERT(mCol[1][3] == 0.0f);
	MOSS_ASSERT(mCol[2][3] == 0.0f);
...
Mat44 Mat44::Inversed() const
{
	...
	Type tmp1 = MOSS_NEON_SHUFFLE_F32x4(mCol[0].mValue, mCol[1].mValue, 0, 1, 4, 5);
	Type row1 = MOSS_NEON_SHUFFLE_F32x4(mCol[2].mValue, mCol[3].mValue, 0, 1, 4, 5);
	Type row0 = MOSS_NEON_SHUFFLE_F32x4(tmp1, row1, 0, 2, 4, 6);
	row1 = MOSS_NEON_SHUFFLE_F32x4(row1, tmp1, 1, 3, 5, 7);
	tmp1 = MOSS_NEON_SHUFFLE_F32x4(mCol[0].mValue, mCol[1].mValue, 2, 3, 6, 7);
	Type row3 = MOSS_NEON_SHUFFLE_F32x4(mCol[2].mValue, mCol[3].mValue, 2, 3, 6, 7);
	Type row2 = MOSS_NEON_SHUFFLE_F32x4(tmp1, row3, 0, 2, 4, 6);
	row3 = MOSS_NEON_SHUFFLE_F32x4(row3, tmp1, 1, 3, 5, 7);

	tmp1 = vmulq_f32(row2, row3);
	tmp1 = MOSS_NEON_SHUFFLE_F32x4(tmp1, tmp1, 1, 0, 3, 2);
	Type minor0 = vmulq_f32(row1, tmp1);
	Type minor1 = vmulq_f32(row0, tmp1);
	tmp1 = MOSS_NEON_SHUFFLE_F32x4(tmp1, tmp1, 2, 3, 0, 1);
	minor0 = vsubq_f32(vmulq_f32(row1, tmp1), minor0);
	minor1 = vsubq_f32(vmulq_f32(row0, tmp1), minor1);
	minor1 = MOSS_NEON_SHUFFLE_F32x4(minor1, minor1, 2, 3, 0, 1);
	...
	minor3 = MOSS_NEON_SHUFFLE_F32x4(minor3, minor3, 2, 3, 0, 1);

	tmp1 = MOSS_NEON_SHUFFLE_F32x4(row1, row1, 2, 3, 0, 1);
	tmp1 = vmulq_f32(tmp1, row3);
	tmp1 = MOSS_NEON_SHUFFLE_F32x4(tmp1, tmp1, 1, 0, 3, 2);
	row2 = MOSS_NEON_SHUFFLE_F32x4(row2, row2, 2, 3, 0, 1);
	...
	minor2 = MOSS_NEON_SHUFFLE_F32x4(minor2, minor2, 2, 3, 0, 1);
	...
	return result;
#else
	float m00 = MOSS_EL(0, 0), m10 = MOSS_EL(1, 0), m20 = MOSS_EL(2, 0), m30 = MOSS_EL(3, 0);
	float m01 = MOSS_EL(0, 1), m11 = MOSS_EL(1, 1), m21 = MOSS_EL(2, 1), m31 = MOSS_EL(3, 1);
	float m02 = MOSS_EL(0, 2), m12 = MOSS_EL(1, 2), m22 = MOSS_EL(2, 2), m32 = MOSS_EL(3, 2);
	float m03 = MOSS_EL(0, 3), m13 = MOSS_EL(1, 3), m23 = MOSS_EL(2, 3), m33 = MOSS_EL(3, 3);
...
{
	return Mat44(
		Vec4(MOSS_EL(1, 1), MOSS_EL(1, 2), MOSS_EL(1, 0), 0) * Vec4(MOSS_EL(2, 2), MOSS_EL(2, 0), MOSS_EL(2, 1), 0)
			- Vec4(MOSS_EL(1, 2), MOSS_EL(1, 0), MOSS_EL(1, 1), 0) * Vec4(MOSS_EL(2, 1), MOSS_EL(2, 2), MOSS_EL(2, 0), 0),
		Vec4(MOSS_EL(0, 2), MOSS_EL(0, 0), MOSS_EL(0, 1), 0) * Vec4(MOSS_EL(2, 1), MOSS_EL(2, 2), MOSS_EL(2, 0), 0)
			- Vec4(MOSS_EL(0, 1), MOSS_EL(0, 2), MOSS_EL(0, 0), 0) * Vec4(MOSS_EL(2, 2), MOSS_EL(2, 0), MOSS_EL(2, 1), 0),
		Vec4(MOSS_EL(0, 1), MOSS_EL(0, 2), MOSS_EL(0, 0), 0) * Vec4(MOSS_EL(1, 2), MOSS_EL(1, 0), MOSS_EL(1, 1), 0)
			- Vec4(MOSS_EL(0, 2), MOSS_EL(0, 0), MOSS_EL(0, 1), 0) * Vec4(MOSS_EL(1, 1), MOSS_EL(1, 2), MOSS_EL(1, 0), 0),
		Vec4(0, 0, 0, 1));
}
...
Mat44 Mat44::Inversed3x3() const
{
	...
	return Mat44(
		(Vec4(MOSS_EL(1, 1), MOSS_EL(1, 2), MOSS_EL(1, 0), 0) * Vec4(MOSS_EL(2, 2), MOSS_EL(2, 0), MOSS_EL(2, 1), 0)
			- Vec4(MOSS_EL(1, 2), MOSS_EL(1, 0), MOSS_EL(1, 1), 0) * Vec4(MOSS_EL(2, 1), MOSS_EL(2, 2), MOSS_EL(2, 0), 0)) / det,
		(Vec4(MOSS_EL(0, 2), MOSS_EL(0, 0), MOSS_EL(0, 1), 0) * Vec4(MOSS_EL(2, 1), MOSS_EL(2, 2), MOSS_EL(2, 0), 0)
			- Vec4(MOSS_EL(0, 1), MOSS_EL(0, 2), MOSS_EL(0, 0), 0) * Vec4(MOSS_EL(2, 2), MOSS_EL(2, 0), MOSS_EL(2, 1), 0)) / det,
		(Vec4(MOSS_EL(0, 1), MOSS_EL(0, 2), MOSS_EL(0, 0), 0) * Vec4(MOSS_EL(1, 2), MOSS_EL(1, 0), MOSS_EL(1, 1), 0)
			- Vec4(MOSS_EL(0, 2), MOSS_EL(0, 0), MOSS_EL(0, 1), 0) * Vec4(MOSS_EL(1, 1), MOSS_EL(1, 2), MOSS_EL(1, 0), 0)) / det,
		Vec4(0, 0, 0, 1));
}

bool Mat44::SetInversed3x3(const Mat44 inM)
{
...
void Mat44::SetRotation(const Mat44 inRotation)
{
