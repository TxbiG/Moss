...
class [[nodiscard]] alignas(MOSS_VECTOR_ALIGNMENT) Mat4x3
{
public:
	...
	/// Identity matrix
	static MOSS_INLINE Mat4x3	Identity() { return Mat44(Vec4(1, 0, 0, 0),
														   Vec4(0, 1, 0, 0),
														   Vec4(0, 0, 1, 0)); }
	...
	friend MOSS_INLINE Mat4x3	operator * (float inV, const Mat4x3  inM)					{ return inM * inV; }

	/// Multiply matrix with float
	MOSS_INLINE Mat4x3 &		operator *= (float inV) {
		for (int c = 0; c < 3; ++c) { mCol[c] *= inV; }
		return *this;
	}

	/// Per element addition of matrix
	MOSS_INLINE Mat4x3			operator + (const Mat4x3  inM) const {
		Mat44 result;
	...
	/// Negate
	MOSS_INLINE Mat4x3			operator - () const {
		Mat44 result;
	...
	Mat4x3 &					operator = (const Mat4x3 &inM2) = default;
	
	/// Per element addition of matrix
	MOSS_INLINE Mat4x3 &		operator += (const Mat4x3  inM) {
		for (int c = 0; c < 3; ++c) { mCol[c] += inM.mCol[c]; }
		return *this;
	}

#ifndef MOSS_DOUBLE_PRECISION
	/// In single precision mode just return the matrix itself
	MOSS_INLINE Mat4x3			ToMat4x3() const											{ return *this; }
#endif // !MOSS_DOUBLE_PRECISION

	/// To String
	friend ostream &			operator << (ostream &inStream, const Mat4x3  inM)
	{
