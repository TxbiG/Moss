...
public:
	...
	MOSS_INLINE Vec3			GetColumn3(uint32_t inCol) const						{ MOSS_ASSERT(inCol < 3); return Vec3(mCol[inCol]); }
	MOSS_INLINE void			SetColumn3(uint32_t inCol, Vec3Arg inV)					{ MOSS_ASSERT(inCol < 3); mCol[inCol] = Vec4(inV, 0.0f); }
	MOSS_INLINE Vec4			GetColumn4(uint32_t inCol) const						{ MOSS_ASSERT(inCol < 3); return mCol[inCol]; }
	MOSS_INLINE void			SetColumn4(uint32_t inCol, Vec4Arg inV)					{ MOSS_ASSERT(inCol < 3); mCol[inCol] = inV; }
