...
public:
	...
	MOSS_INLINE					iVec3(Type inRHS) : mValue(inRHS)				{ CheckW(); }

	/// Load 3 floats from memory
	explicit MOSS_INLINE		iVec3(const Float2 &inV);
	...
	MOSS_INLINE iVec3			operator * (Vec2Arg inV2) const;
	...
	/// Multiply vector with float
	MOSS_INLINE iVec3 &			operator *= (float inV2);

	/// Multiply vector with vector
	MOSS_INLINE iVec3 &			operator *= (Vec2Arg inV2);
	...
	/// Negate
	MOSS_INLINE iVec3			operator - () const;
	...
	MOSS_INLINE iVec3			Swizzle() const;
	...
	MOSS_INLINE iVec3			Abs() const;

	/// Reciprocal vector (1 / value) for each of the components
	MOSS_INLINE iVec3			Reciprocal() const;

	/// Cross product
	MOSS_INLINE iVec3			Cross(Vec2Arg inV2) const;
	MOSS_INLINE float			Cross(Vec2Arg a, Vec2Arg b) const;
	MOSS_INLINE iVec3			Cross(float radian) const;

	/// Dot product, returns the dot product in X, Y and Z components
	MOSS_INLINE iVec3			DotV(Vec2Arg inV2) const;
	...
	/// Normalize vector
	MOSS_INLINE iVec3			Normalized() const;

	MOSS_INLINE bool 			IsNormalized(iVec3 a);

	MOSS_INLINE iVec3			Lerp(iVec3 a, iVec3 b, float t) const;

	MOSS_INLINE float			Distance(iVec3 a) const;

	/// Normalize vector or return inZeroValue if the length of the vector is zero
	MOSS_INLINE iVec3			NormalizedOr(Vec2Arg inZeroValue) const;
	...
	MOSS_INLINE iVec3			Sqrt() const;

	/// Get normalized vector that is perpendicular to this vector
	MOSS_INLINE iVec3			GetNormalizedPerpendicular() const;

	/// Get vector that contains the sign of each element (returns 1.0f if positive, -1.0f if negative)
	MOSS_INLINE iVec3			GetSign() const;
...
static_assert(std::is_trivial<iVec3>(), "Is supposed to be a trivial type!");
