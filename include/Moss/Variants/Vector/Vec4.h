...
public:
	...
	static MOSS_INLINE Vec4		Zero();

	/// Vector with all ones
	static MOSS_INLINE Vec4		One();

	/// Vector with all NaN's
	static MOSS_INLINE Vec4		NaN();

	/// Replicate inV across all components
	static MOSS_INLINE Vec4		Replicate(float inV);

	/// Load 4 floats from memory
	static MOSS_INLINE Vec4		LoadFloat4(const Float4 *inV);

	/// Load 4 floats from memory, 16 bytes aligned
	static MOSS_INLINE Vec4		LoadFloat4Aligned(const Float4 *inV);

	/// Gather 4 floats from memory at inBase + inOffsets[i] * Scale
	template <const int Scale>
	static MOSS_INLINE Vec4		GatherFloat4(const float *inBase, const UVec4 inOffsets);

	/// Return the minimum value of each of the components
	static MOSS_INLINE Vec4		Min(const Vec4 inV1, const Vec4 inV2);

	/// Return the maximum of each of the components
	static MOSS_INLINE Vec4		Max(const Vec4 inV1, const Vec4 inV2);

	/// Equals (component wise)
	static MOSS_INLINE UVec4	Equals(const Vec4 inV1, const Vec4 inV2);

	/// Less than (component wise)
	static MOSS_INLINE UVec4	Less(const Vec4 inV1, const Vec4 inV2);

	/// Less than or equal (component wise)
	static MOSS_INLINE UVec4	LessOrEqual(const Vec4 inV1, const Vec4 inV2);

	/// Greater than (component wise)
	static MOSS_INLINE UVec4	Greater(const Vec4 inV1, const Vec4 inV2);

	/// Greater than or equal (component wise)
	static MOSS_INLINE UVec4	GreaterOrEqual(const Vec4 inV1, const Vec4 inV2);

	/// Calculates inMul1 * inMul2 + inAdd
	static MOSS_INLINE Vec4		FusedMultiplyAdd(const Vec4 inMul1, const Vec4 inMul2, const Vec4 inAdd);

	/// Component wise select, returns inNotSet when highest bit of inControl = 0 and inSet when highest bit of inControl = 1
	static MOSS_INLINE Vec4		Select(const Vec4 inNotSet, const Vec4 inSet, const UVec4 inControl);

	/// Logical or (component wise)
	static MOSS_INLINE Vec4		Or(const Vec4 inV1, const Vec4 inV2);

	/// Logical xor (component wise)
	static MOSS_INLINE Vec4		Xor(const Vec4 inV1, const Vec4 inV2);

	/// Logical and (component wise)
	static MOSS_INLINE Vec4		And(const Vec4 inV1, const Vec4 inV2);
	...
	static MOSS_INLINE void		Sort4(Vec4 &ioValue, UVec4 &ioIndex);

	/// Reverse sort the four elements of ioValue (highest first) and sort ioIndex at the same time.
	/// Based on a sorting network: http://en.wikipedia.org/wiki/Sorting_network
	static MOSS_INLINE void		Sort4Reverse(Vec4 &ioValue, UVec4 &ioIndex);
	...
	inline static Vec4			ATan2(const Vec4 inY, const Vec4 inX);
