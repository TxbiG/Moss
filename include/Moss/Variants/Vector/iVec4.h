...
class [[nodiscard]] alignas(MOSS_VECTOR_ALIGNMENT) iVec4
{
public:
	// MOSS_OVERRIDE_NEW_DELETE < Not needed yet

	// Underlying vector type
	//using Type = Moss_i32vec4_t;
	...
	/* 			Constructors		*/
	iVec4() = default; ///< Intentionally not initialized for performance reasons
	iVec4(const iVec4 &inRHS) = default;
	iVec4 &						operator = (const iVec4 &inRHS) = default;
	explicit MOSS_INLINE		iVec4(Vec4Arg inRHS);
	MOSS_INLINE					iVec4(Type inRHS) : mValue(inRHS)				{ CheckW(); }

	/// Load 3 floats from memory
	explicit MOSS_INLINE		iVec4(const Float2 &inV);

	/// Create a vector from 3 components
	MOSS_INLINE					iVec4(float inX, float inY, float inZ);

	/// Vector with all zeros
	static MOSS_INLINE iVec4		sZero();

	/// Vector with all ones
	static MOSS_INLINE iVec4		sOne();

	/// Vector with all NaN's
	static MOSS_INLINE iVec4		sNaN();

	/// Vectors with the principal axis
	static MOSS_INLINE iVec4		sAxisX()										{ return iVec4(1, 0, 0); }
	static MOSS_INLINE iVec4		sAxisY()										{ return iVec4(0, 1, 0); }
	static MOSS_INLINE iVec4		sAxisZ()										{ return iVec4(0, 0, 1); }

	/// Replicate inV across all components
	static MOSS_INLINE iVec4		sReplicate(float inV);

	/// Load 3 floats from memory (reads 32 bits extra which it doesn't use)
	static MOSS_INLINE iVec4		sLoadFloat2Unsafe(const Float2 &inV);

	/// Return the minimum value of each of the components
	static MOSS_INLINE iVec4		sMin(Vec2Arg inV1, Vec2Arg inV2);

	/// Return the maximum of each of the components
	static MOSS_INLINE iVec4		sMax(Vec2Arg inV1, Vec2Arg inV2);

	/// Clamp a vector between min and max (component wise)
	static MOSS_INLINE iVec4		sClamp(Vec2Arg inV, Vec2Arg inMin, Vec2Arg inMax);
	...
	static MOSS_INLINE iVec4	sFusedMultiplyAdd(Vec2Arg inMul1, Vec2Arg inMul2, Vec2Arg inAdd);

	/// Component wise select, returns inNotSet when highest bit of inControl = 0 and inSet when highest bit of inControl = 1
	static MOSS_INLINE iVec4	sSelect(Vec2Arg inNotSet, Vec2Arg inSet, UVec4Arg inControl);

	/// Logical or (component wise)
	static MOSS_INLINE iVec4	sOr(Vec2Arg inV1, Vec2Arg inV2);

	/// Logical xor (component wise)
	static MOSS_INLINE iVec4	sXor(Vec2Arg inV1, Vec2Arg inV2);

	/// Logical and (component wise)
	static MOSS_INLINE iVec4	sAnd(Vec2Arg inV1, Vec2Arg inV2);
	...
	static MOSS_INLINE iVec4		sUnitSpherical(float inTheta, float inPhi);
	...
	static inline iVec4			sRandom(Random &inRandom);
	...
	MOSS_INLINE iVec4			operator * (Vec2Arg inV2) const;

	/// Multiply vector with float
	MOSS_INLINE iVec4			operator * (float inV2) const;

	/// Multiply vector with float
	friend MOSS_INLINE iVec4	operator * (float inV1, Vec2Arg inV2);

	/// Divide vector by float
	MOSS_INLINE iVec4			operator / (float inV2) const;

	/// Multiply vector with float
	MOSS_INLINE iVec4&			operator *= (float inV2);

	/// Multiply vector with vector
	MOSS_INLINE iVec4&			operator *= (Vec2Arg inV2);
	...
	/// Negate
	MOSS_INLINE iVec4			operator - () const;
	...
	MOSS_INLINE iVec4			Swizzle() const;
	...
	MOSS_INLINE iVec4			Abs() const;

	/// Reciprocal vector (1 / value) for each of the components
	MOSS_INLINE iVec4			Reciprocal() const;

	/// Cross product
	MOSS_INLINE iVec4			Cross(Vec2Arg inV2) const;
	MOSS_INLINE float			Cross(Vec2Arg a, Vec2Arg b) const;
	MOSS_INLINE iVec4			Cross(float radian) const;

	/// Dot product, returns the dot product in X, Y and Z components
	MOSS_INLINE iVec4			DotV(Vec2Arg inV2) const;
	...
	/// Normalize vector
	MOSS_INLINE iVec4			Normalized() const;

	MOSS_INLINE bool 			IsNormalized(iVec4 a);

	MOSS_INLINE iVec4			Lerp(iVec4 a, iVec4 b, float t) const;

	MOSS_INLINE float			Distance(iVec4 a) const;

	/// Normalize vector or return inZeroValue if the length of the vector is zero
	MOSS_INLINE iVec4			NormalizedOr(Vec2Arg inZeroValue) const;
	...
	MOSS_INLINE iVec4			Sqrt() const;

	/// Get normalized vector that is perpendicular to this vector
	MOSS_INLINE iVec4			GetNormalizedPerpendicular() const;

	/// Get vector that contains the sign of each element (returns 1.0f if positive, -1.0f if negative)
	MOSS_INLINE iVec4			GetSign() const;
...
static_assert(std::is_trivial<iVec4>(), "Is supposed to be a trivial type!");
