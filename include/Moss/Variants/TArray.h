...
template <class T, class Allocator = STLAllocator<T>> using TArray = std::vector<T, Allocator>;
...
class [[nodiscard]] TArray : private Allocator
{
public:
	using value_type = T;
	...
	/// An iterator that traverses the TArray in reverse order
	class rev_it
	{
	public:
		/// Constructor
	...
	/// A const iterator that traverses the TArray in reverse order
	class crev_it
	{
	public:
		/// Constructor
...
private:
	...
	{
		MOSS_ASSERT(inNewCapacity > 0 && inNewCapacity >= mSize);
...
public:
	...
	/// Resize TArray to new length and initialize all elements with inValue
	inline void				resize(size_type inNewSize, const T &inValue)
	{
		MOSS_ASSERT(&inValue < mElements || &inValue >= mElements + mSize, "Can't pass an element from the TArray to resize");
...
private:
	/// Grow the TArray by at least inAmount elements
...
public:
	/// Replace the contents of this TArray with inBegin .. inEnd
	...
	/// Replace the contents of this TArray with inList
	...
	explicit inline			TArray(const Allocator &inAllocator) :
		Allocator(inAllocator)
	{
	}

	/// Constructor with length
	explicit inline			TArray(size_type inLength, const Allocator &inAllocator = { }) :
		Allocator(inAllocator)
	{
		resize(inLength);
	}
	...
	inline					TArray(size_type inLength, const T &inValue, const Allocator &inAllocator = { }) :
		Allocator(inAllocator)
	{
	...
	inline					TArray(std::initializer_list<T> inList, const Allocator &inAllocator = { }) :
		Allocator(inAllocator)
	{
		assign(inList);
	}
	...
	inline					TArray(const_iterator inBegin, const_iterator inEnd, const Allocator &inAllocator = { }) :
		Allocator(inAllocator)
	{
	...
	/// Copy constructor
	inline					TArray(const TArray<T, Allocator> &inRHS) :
	...
	/// Move constructor
	inline					TArray(TArray<T, Allocator> &&inRHS) noexcept :
	...
	/// Add element to the back of the TArray
	inline void				push_back(const T &inValue)
	{
		MOSS_ASSERT(&inValue < mElements || &inValue >= mElements + mSize, "Can't pass an element from the TArray to push_back");
	...
	/// Construct element at the back of the TArray
	template <class... A>
	...
	/// Remove element from the back of the TArray
	...
	/// Returns true if there are no elements in the TArray
	inline bool				empty() const
	{
		return mSize == 0;
	}

	/// Returns amount of elements in the TArray
	inline size_type		size() const
	{
		return mSize;
	}

	/// Returns maximum amount of elements the TArray can hold
	inline size_type		capacity() const
	{
		return mCapacity;
	}

	/// Reduce the capacity of the TArray to match its size
	void					shrink_to_fit()
	{
		if (mElements != nullptr)
		{
	...
	void					swap(TArray<T, Allocator> &inRHS) noexcept
	{
	...
	{
		MOSS_ASSERT(&inValue < mElements || &inValue >= mElements + mSize, "Can't pass an element from the TArray to insert");
	...
	/// Remove one element from the TArray
	...
	/// Remove multiple element from the TArray
	...
	/// Assignment operator
	TArray<T, Allocator> &	operator = (const TArray<T, Allocator> &inRHS)
	{
	...
	TArray<T, Allocator> &	operator = (TArray<T, Allocator> &&inRHS) noexcept
	{
		if (static_cast<const void *>(this) != static_cast<const void *>(&inRHS))
		{
			destroy();
	...
	/// Assignment operator
	TArray<T, Allocator> &	operator = (std::initializer_list<T> inRHS)
	{
		assign(inRHS);
	...
	/// Comparing arrays
	bool					operator == (const TArray<T, Allocator> &inRHS) const
	{
...
namespace std
{
	/// Declare std::hash for TArray
	template <class T, class Allocator>
	struct hash<TArray<T, Allocator>>
	{
		size_t operator () (const TArray<T, Allocator> &inRHS) const
		{
