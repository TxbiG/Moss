...
bool EigenValueSymmetric(const Matrix &inMatrix, Matrix &outEigVec, Vector &outEigVal)
{
	...
	MOSS_ASSERT(inMatrix.GetCols() == n);
		...
		if (avg_sm < FLT_MIN) // Original code: sm == 0.0f, when the average is denormal, we also consider it machine underflow
		{
			// Sanity checks
				...
				for (uint c = 0; c < n; ++c)
				{
					...
					MOSS_ASSERT(mat_eigvec.IsClose(eigval_eigvec, max(mat_eigvec.LengthSq(), eigval_eigvec.LengthSq()) * 1.0e-6f));
				}
			#endif // MOSS_DEBUG
		...
		for (uint ip = 0; ip < n - 1; ++ip)
			for (uint iq = ip + 1; iq < n; ++iq)
			{
				...
				else if (abs_a_pq > tresh)
				{
					...
					#define MOSS_EVS_ROTATE(a, i, j, k, l)		\
						g = a(i, j),							\
						h = a(k, l),							\
						a(i, j) = g - s * (h + g * tau),		\
						a(k, l) = h + s * (g - h * tau)
					...
					for (j = 0; j < n; ++j)			MOSS_EVS_ROTATE(outEigVec, j, ip, j, iq);

					#undef MOSS_EVS_ROTATE
				}
			}
	...
	// Failure
	MOSS_ASSERT(false, "Too many iterations");
	return false;
}
