...
public:
	...
	public:
		...
		explicit GradientSampler(const Gradient &inGradient) :
			...
			mOffset(inGradient.mMin - inGradient.mMinFraction * mMultiplier), mMin(min(inGradient.mMin, inGradient.mMax)), mMax(max(inGradient.mMin, inGradient.mMax)) { }
	...
	public:
		inline explicit	GridSampler(const HairSettings *inSettings) :
			mGridSizeMin2(inSettings->mGridSize - UVec4::Replicate(2)),
			mGridSizeMin1((inSettings->mGridSize - UVec4::Replicate(1)).ToFloat()),
		...
		inline void		PositionToIndexAndFraction(Vec3Arg inPosition, UVec4 &outIndex, Vec3 &outFraction) const
		{
			...
			Vec3 grid_pos = Vec3::Min(Vec3::Max(inPosition - mOffset, Vec3::Zero()) * mScale, mGridSizeMin1);
			outIndex = UVec4::Min(Vec4(grid_pos).ToInt(), mGridSizeMin2);
...
public:
	...
	/// Ability to externally provide the scalp vertices buffer. This allows skipping skinning the scalp during the simulation update. You may need to override MOSS_SHADER_BIND_SCALP_VERTICES in HairSkinRootsBindings.h to match the format of the provided buffer.
	void								SetScalpVerticesCB(ComputeBuffer *inBuffer)		{ mScalpVerticesCB = inBuffer; }

	/// Ability to externally provide the scalp triangle indices buffer. This allows skipping skinning the scalp in during the simulation update. You may need to override MOSS_SHADER_BIND_SCALP_TRIANGLES in HairSkinRootsBindings.h to match the format of the provided buffer.
	...
	/// Allow setting the render vertices buffer externally in case it has special requirements for the calling application. You may need to override MOSS_SHADER_BIND_RENDER_POSITIONS in HairCalculateRenderPositionsBindings.h to match the format of the provided buffer.
	void								SetRenderPositionsCB(ComputeBuffer *inBuffer)	{ MOSS_ASSERT(mRenderPositionsOverridden, "Must call OverrideRenderPositionsCB first"); mRenderPositionsCB = inBuffer; }
	...
	StridedPtr<const Float3>			GetVelocities() const							{ return { &mVelocities->mVelocity, sizeof(MOSS_HairVelocity) }; }
	StridedPtr<const Float3>			GetAngularVelocities() const					{ return { &mVelocities->mAngularVelocity, sizeof(MOSS_HairVelocity) }; }
...
protected:
	...
	Mat44								mScalpToHead = Mat44::Identity();				// When skipping skinning, this allow specifying a transform that transforms the scalp mesh into head space
	...
	MOSS_HairVelocity*					mVelocities = nullptr;
