...
public:
	...
	virtual void		OnBodyEnd() override
	{
		if (mHadHit)
		{
			...
			MOSS_ASSERT(mPreviousEarlyOutFraction != -FLT_MAX); // Check that we got a call to OnBody
		...
		MOSS_IF_ENABLE_ASSERTS(mPreviousEarlyOutFraction = -FLT_MAX;)
	}
