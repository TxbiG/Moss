...
struct ShapeCastT {
	...
	: mShape(inShape), mScale(inScale), mCenterOfMassStart(inCenterOfMassStart), mDirection(inDirection), mShapeWorldBounds(inWorldSpaceBounds) {}
	...
	ShapeCastType PostTranslated(typename Vec::ArgType inTranslation) const { return { mShape, mScale, mCenterOfMassStart.PostTranslated(inTranslation), mDirection }; }
...
struct RShapeCast : public ShapeCastT<RVec3, RMat44, RShapeCast> {
	...
	explicit operator ShapeCast() const { return ShapeCast(mShape, mScale, mCenterOfMassStart.ToMat44(), mDirection, mShapeWorldBounds); }
};
...
public:
	...
	ShapeCastResult(float inFraction, Vec3Arg inContactPoint1, Vec3Arg inContactPoint2, Vec3Arg inContactNormalOrPenetrationDepth, bool inBackFaceHit, const SubShapeID &inSubShapeID1, const SubShapeID &inSubShapeID2, const BodyID &inBodyID2) :
		...
		mFraction(inFraction), mIsBackFaceHit(inBackFaceHit) { }
