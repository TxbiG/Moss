...
public:
		...
		mIsInsideOut(ScaleHelpers::IsInsideOut(inScale))
	{
		...
		MOSS_ASSERT(IsAligned(this, alignof(GetTrianglesContextVertexList)));
		MOSS_ASSERT(inNumTriangleVertices % 3 == 0);
	}

	/// @see Shape::GetTrianglesNext
	int				GetTrianglesNext(int inMaxTrianglesRequested, Float3 *outTriangleVertices, const PhysicsMaterial **outMaterials)
	{
		MOSS_ASSERT(inMaxTrianglesRequested >= Shape::cGetTrianglesMinTrianglesRequested);
	...
	static void		sCreateUnitOpenCylinder(A &ioVertices, int inDetailLevel)
	{
		...
		{
			float angle1 = 2.0f * MOSS_PI * (float(i) / num_verts);
			float angle2 = 2.0f * MOSS_PI * (float(i + 1) / num_verts);
...
public:
		...
		mIsInsideOut(inIsInsideOut)
	{
		...
		MOSS_ASSERT(IsAligned(this, alignof(GetTrianglesContextMultiVertexList)));
	}
