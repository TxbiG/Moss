...
public:
	...
	/// (internally this will be subtracted so the total size will not grow with the convex radius).
							...
							ConvexHullShapeSettings(const TArray<Vec3> &inPoints, float inConvexRadius = cDefaultConvexRadius, const PhysicsMaterial *inMaterial = nullptr) : ConvexShapeSettings(inMaterial), mPoints(inPoints), mMaxConvexRadius(inConvexRadius) { }
