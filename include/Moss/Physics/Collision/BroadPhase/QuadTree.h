...
private:
	...
	MOSS_INLINE const RootNode &	GetCurrentRoot() const				{ return mRootNode[mRootNodeIndex]; }
	MOSS_INLINE RootNode &		GetCurrentRoot()					{ return mRootNode[mRootNodeIndex]; }
	...
	using LayerToStats = TMap<String, Stat>;
	...
	MOSS_INLINE void				WalkTree(const ObjectLayerFilter &inObjectLayerFilter, const TrackingVector &inTracking, Visitor &ioVisitor MOSS_IF_TRACK_BROADPHASE_STATS(, LayerToStats &ioStats)) const;
