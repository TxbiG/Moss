...
public:
	...
	/// Header for the tree
	struct Header {
		Float2							mRootBoundsMin;
		Float2							mRootBoundsMax;
	...
	public:
		...
		void PrepareNodeAllocate(const AABBTreeBuilder::Node *inNode, uint64 &ioBufferSize) const {
			...
			if (!inNode->HasChildren()) { return; }
			ioBufferSize += sizeof(Node);		// Add size of node
		}
		...
		size_t NodeAllocate(const AABBTreeBuilder::Node *inNode, Vec3Arg inNodeBoundsMin, Vec3Arg inNodeBoundsMax, TArray<const AABBTreeBuilder::Node*> &ioChildren, Vec2 outChildBoundsMin[NumChildrenPerNode], Vec2 outChildBoundsMax[NumChildrenPerNode], ByteBuffer &ioBuffer) const {
			// We don't emit nodes for leafs
			if (!inNode->HasChildren()) { return ioBuffer.size(); }

			size_t node_start = ioBuffer.size();		// Remember the start of the node

			Node *node = ioBuffer.Allocate<Node>();		// Fill in bounds
			...
			for (size_t i = 0; i < 4; ++i) {
				if (i < ioChildren.size()) {
					...
					{ MOSS_ERROR("in NodeCodecQuadTreeHalfFloat, Too many triangles!"); return size_t(-1); }
				}
				else {
		...
		bool NodeFinalize(const AABBTreeBuilder::Node *inNode, size_t inNodeStart, uint inNumChildren, const size_t *inChildrenNodeStart, const size_t *inChildrenTrianglesStart, ByteBuffer &ioBuffer) {
			...
			for (uint i = 0; i < inNumChildren; ++i) {
				...
				if (node->mNodeProperties[i] != 0) {
					...
					mHighestTriangleBlock = max(mHighestTriangleBlock, offset);			// Store highest block with triangles so we can count the number of bits we need
				}
				else{ offset = inChildrenNodeStart[i]; } // This is a node block

				// Store offset of next node / triangles
				if (offset & OFFSET_NON_SIGNIFICANT_MASK) { MOSS_ERROR("NodeCodecQuadTreeHalfFloat: Internal Error: Offset has non-significant bits set"); return false; }
				offset >>= OFFSET_NON_SIGNIFICANT_BITS;
				if (offset > OFFSET_MASK) { MOSS_ERROR("NodeCodecQuadTreeHalfFloat: Offset too large. Too much data."); return false; }
		...
		bool Finalize(Header *outHeader, const AABBTreeBuilder::Node *inRoot, size_t inRootNodeStart, size_t inRootTrianglesStart) const {
			...
			inRoot->mBounds.mMin.StoreFloat2(&outHeader->mRootBoundsMin);
			inRoot->mBounds.mMax.StoreFloat2(&outHeader->mRootBoundsMax);
			...
			if (inRoot->GetTriangleCount() >= TRIANGLE_COUNT_MASK) { MOSS_ERROR("NodeCodecQuadTreeHalfFloat: Too many triangles"); return false; }
