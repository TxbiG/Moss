...
public:
	...
	public:
		...
		size_t NodeAllocate(const AABBTreeBuilder::Node *inNode, Vec3Arg inNodeBoundsMin, Vec3Arg inNodeBoundsMax, TArray<const AABBTreeBuilder::Node *> &ioChildren, Vec3 outChildBoundsMin[NumChildrenPerNode], Vec3 outChildBoundsMax[NumChildrenPerNode], ByteBuffer &ioBuffer, const char *&outError) const {
			// We don't emit nodes for leafs
			if (!inNode->HasChildren())
	...
	public:
		...
		/// Constructor
		MOSS_INLINE explicit DecodingContext(const Header *inHeader) {
