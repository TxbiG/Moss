...
public:
	...
	public:
		...
		bool						IsDegenerate(const IndexedTriangle &inTriangle) const
		{
			...
			Vec3 compress_scale = Vec3::Replicate(COMPONENT_MASK) / Vec3::Max(mBounds.GetSize(), Vec3::Replicate(1.0e-20f));
			for (int i = 0; i < 3; ++i)
				quantized_vertex[i] = ((Vec3(mVertices[inTriangle.mIdx[i]]) - mBounds.mMin) * compress_scale + Vec3::Replicate(0.5f)).ToInt();
	...
	public:
		...
		void						Finalize(const VertexList &inVertices, TriangleHeader *ioHeader, ByteBuffer &ioBuffer) const
		{
			...
			(bounds.GetSize() / Vec3::Replicate(COMPONENT_MASK)).StoreFloat3(&ioHeader->mScale);
		}
	...
	private:
		...
		MOSS_INLINE void				Unpack(const VertexData *inVertices, UVec4Arg inIndex, Vec4 &outX, Vec4 &outY, Vec4 &outZ) const
		{
			// Get compressed data
			...
			// Convert to float
			outX = Vec4::FusedMultiplyAdd(xc.ToFloat(), mScaleX, mOffsetX);
			outY = Vec4::FusedMultiplyAdd(yc.ToFloat(), mScaleY, mOffsetY);
			outZ = Vec4::FusedMultiplyAdd(zc.ToFloat(), mScaleZ, mOffsetZ);
		}
