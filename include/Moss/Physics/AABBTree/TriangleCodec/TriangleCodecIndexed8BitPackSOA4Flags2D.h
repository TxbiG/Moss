...
public:
	...
	enum class EComponentData : uint32 {
		COMPONENT_BITS = 21,
		COMPONENT_MASK = (1 << COMPONENT_BITS) - 1,
	};

	/// Packed X and Y coordinate
	enum class EVertexXY : uint32 {
		COMPONENT_X = 0,
		...
		ValidationContext(const IndexedTriangleList &inTriangles, const VertexList &inVertices) : mVertices(inVertices) {
			...
			for (const IndexedTriangle &i : inTriangles)
				for (uint32 idx : i.mIdx)
					mBounds.Encapsulate(Vec2(inVertices[idx]));
		}
	...
	public:
		...
		explicit EncodingContext(const VertexList &inVertices) : mVertexMap(inVertices.size(), cNotFound) { }
		...
		void FinalizePreparePack(uint64 &ioBufferSize) {
			...
			MOSS_ASSERT(IsAligned(ioBufferSize, 4));
		...
		void Finalize(const VertexList &inVertices, TriangleHeader *ioHeader, ByteBuffer &ioBuffer) const {
			...
			MOSS_ASSERT(mVertices.size() == mVertexCount);
			MOSS_ASSERT(ioBuffer.size() == mVerticesStartIdx);
			...
			for (uint32 v : mVertices)
				bounds.Encapsulate(Vec2(inVertices[v]));
	...
	public:
		...
		MOSS_INLINE void Unpack(const void *inTriangleStart, uint32 inNumTriangles, Vec2 *outTriangles) const {
			MOSS_ASSERT(inNumTriangles > 0);
		...
		MOSS_INLINE float TestRay(Vec2Arg inRayOrigin, Vec2Arg inRayDirection, const void *inTriangleStart, uint32 inNumTriangles, float inClosest, uint32 &outClosestTriangleIndex) const {
			MOSS_ASSERT(inNumTriangles > 0);
			const TriangleBlockHeader *header = reinterpret_cast<const TriangleBlockHeader *>(inTriangleStart);
			const VertexData *vertices = header->GetVertexData();
			const TriangleBlock *t = header->GetTriangleBlock();
		...
		inline void GetTriangle(const void *inTriangleStart, uint32 inTriangleIdx, Vec2 &outV1, Vec2 &outV2, Vec2 &outV3) const {
			const TriangleBlockHeader *header = reinterpret_cast<const TriangleBlockHeader *>(inTriangleStart);
			const VertexData *vertices = header->GetVertexData();
		...
		MOSS_INLINE static uint8 sGetFlags(const void *inTriangleStart, int inTriangleIndex) {
			const TriangleBlockHeader *header = reinterpret_cast<const TriangleBlockHeader *>(inTriangleStart);
		...
		MOSS_INLINE void Unpack(const void *inTriangleStart, uint32 inNumTriangles, Vec2 *outTriangles, uint8 *outTriangleFlags) const {
