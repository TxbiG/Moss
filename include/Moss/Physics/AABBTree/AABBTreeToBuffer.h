...
public:
	...
	bool Convert(const TArray<IndexedTriangle> &inTriangles, const TArray<AABBTreeBuilder::Node> &inNodes, const VertexList &inVertices, const AABBTreeBuilder::Node *inRoot, bool inStoreUserData, const char *&outError) {
		...
		TArray<const AABBTreeBuilder::Node*> child_nodes;
		...
		{	// A scope to free the memory associated with to_estimate and to_estimate_triangles
			TArray<const AABBTreeBuilder::Node *> to_estimate;
			TArray<const AABBTreeBuilder::Node *> to_estimate_triangles;
		...
		TArray<NodeData *> to_process_triangles;
		to_process_triangles.reserve(to_process_triangles_max_size);
		TArray<NodeData> node_list;
		...
		for (;;) {
			while (!to_process.empty()) {
				...
				MOSS_ASSERT(AABox(node_data->mNodeBoundsMin, node_data->mNodeBoundsMax).Contains(node_data->mNode->mBounds), "AABBTreeToBuffer: Bounding box became smaller!");
				...
				if (node_data->mNode->HasChildren()) {
					...
					for (int idx = int(child_nodes.size()) - 1; idx >= 0; --idx) {
						...
						// Due to quantization box could have become bigger, not smaller
						MOSS_ASSERT(AABox(child_bounds_min[idx], child_bounds_max[idx]).Contains(child_node->mBounds), "AABBTreeToBuffer: Bounding box became smaller!");
		...
		MOSS_ASSERT(to_process_max_size == to_process.capacity() || to_process_triangles_max_size == to_process.capacity());
		MOSS_ASSERT(to_process_max_size == to_process_triangles.capacity() || to_process_triangles_max_size == to_process_triangles.capacity());
	...
	/// Get resulting data
	inline const ByteBuffer& GetBuffer() const { return mTree; }

	/// Get resulting data
	inline ByteBuffer& GetBuffer() { return mTree; }

	/// Get header for tree
	inline const NodeHeader* GetNodeHeader() const { return mTree.Get<NodeHeader>(0); }

	/// Get header for triangles
	inline const TriangleHeader* GetTriangleHeader() const { return mTree.Get<TriangleHeader>(HeaderSize); }
