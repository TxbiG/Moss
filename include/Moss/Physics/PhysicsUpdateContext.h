...
public:
	...
	using JobHandleArray = TStaticArray<JobHandle, cMaxConcurrency>;
	...
	{
							Step() = default;
							Step(const Step &)										{ MOSS_ASSERT(false); } // vector needs a copy constructor, but we're never going to call it
	...
	using Steps = TArray<Step, STLTempAllocator<Step>>;
