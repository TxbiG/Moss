...
public:
	...
	EStatus				GetPenetrationDepthStepGJK(const AE &inAExcludingConvexRadius, float inConvexRadiusA, const BE &inBExcludingConvexRadius, float inConvexRadiusB, float inTolerance, Vec3 &ioV, Vec3 &outPointA, Vec3 &outPointB)
	{
		...
		MOSS_IF_ENABLE_ASSERTS(mGJKTolerance = inTolerance;)
	...
	bool				GetPenetrationDepthStepEPA(const AI &inAIncludingConvexRadius, const BI &inBIncludingConvexRadius, float inTolerance, Vec3 &outV, Vec3 &outPointA, Vec3 &outPointB)
	{
		...
		MOSS_ASSERT(inTolerance >= FLT_EPSILON);
		...
		case 1:
			{
				...
				MOSS_ASSERT(support_points.mY[0].IsNearZero(Square(mGJKTolerance)));
		...
		MOSS_ASSERT(support_points.mY.size() >= 3);
...
#endif
#ifdef MOSS_EPA_PENETRATION_DEPTH_DEBUG
			MOSS_TRACE("EncapsulateOrigin: verts = (%d, %d, %d), closest_dist_sq = %g, centroid = (%g, %g, %g), normal = (%g, %g, %g)",
		...
		do
		{
...
#endif
#ifdef MOSS_EPA_PENETRATION_DEPTH_DEBUG
			MOSS_TRACE("FindClosest: verts = (%d, %d, %d), closest_len_sq = %g, centroid = (%g, %g, %g), normal = (%g, %g, %g)",
				t->mEdge[0].mStartIdx, t->mEdge[1].mStartIdx, t->mEdge[2].mStartIdx,
				t->mClosestLenSq,
				t->mCentroid.GetX(), t->mCentroid.GetY(), t->mCentroid.GetZ(),
				t->mNormal.GetX(), t->mNormal.GetY(), t->mNormal.GetZ());
...
#ifdef MOSS_EPA_PENETRATION_DEPTH_DEBUG
			MOSS_TRACE("FindClosest: w = (%g, %g, %g), dot = %g, dist_sq = %g",
...
#endif // MOSS_EPA_PENETRATION_DEPTH_DEBUG
				break;
			}
			...
			{
#ifdef MOSS_EPA_PENETRATION_DEPTH_DEBUG
				MOSS_TRACE("Not facing triangle");
#endif // MOSS_EPA_PENETRATION_DEPTH_DEBUG
				break;
			}
			...
			{
#ifdef MOSS_EPA_PENETRATION_DEPTH_DEBUG
				MOSS_TRACE("Could not add point");
#endif // MOSS_EPA_PENETRATION_DEPTH_DEBUG
				break;
			}
	...
	{
		MOSS_IF_ENABLE_ASSERTS(mGJKTolerance = inCollisionTolerance;)
