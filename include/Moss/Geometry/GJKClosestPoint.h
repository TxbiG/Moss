...
#ifdef MOSS_GJK_DEBUG
		MOSS_TRACE("GetClosest: set = 0b%s, v = [%s], |v| = %g", NibbleToBinary(set), ConvertToString(v).c_str(), (double)v.Length());
#endif
...
#ifdef MOSS_GJK_DEBUG
		MOSS_TRACE("New closer point is further away, failed to converge");
#endif
		return false;
	}
...
public:
	...
	bool		Intersects(const A &inA, const B &inB, float inTolerance, Vec3 &ioV)
	{
		...
		{
#ifdef MOSS_GJK_DEBUG
			MOSS_TRACE("v = [%s], num_points = %d", ConvertToString(ioV).c_str(), mNumPoints);
#endif
	...
	float		GetClosestPoints(const A &inA, const B &inB, float inTolerance, float inMaxDistSq, Vec3 &ioV, Vec3 &outPointA, Vec3 &outPointB)
	{
		...
		for (;;)
		{
			...
			{
#ifdef MOSS_GJK_DEBUG
				MOSS_TRACE("Distance bigger than max");
#endif
#ifdef MOSS_DEBUG
				memset(&outPointA, 0xcd, sizeof(outPointA));
				memset(&outPointB, 0xcd, sizeof(outPointB));
			...
			// If v is very small compared to the length of y, we also consider this a collision
#ifdef MOSS_GJK_DEBUG
			MOSS_TRACE("Check v small compared to y: %g <= %g", (double)v_len_sq, (double)(FLT_EPSILON * GetMaxYLengthSq()));
#endif
			if (v_len_sq <= FLT_EPSILON * GetMaxYLengthSq())
			{
#ifdef MOSS_GJK_DEBUG
			...
			// If the squared length of v is not changing enough, we've converged and there is no collision
#ifdef MOSS_GJK_DEBUG
			MOSS_TRACE("Check v not changing enough: %g <= %g", (double)(prev_v_len_sq - v_len_sq), (double)(FLT_EPSILON * prev_v_len_sq));
#endif
			MOSS_ASSERT(prev_v_len_sq >= v_len_sq);
			if (prev_v_len_sq - v_len_sq <= FLT_EPSILON * prev_v_len_sq)
			{
		...
		MOSS_ASSERT(ioV.LengthSq() == v_len_sq);
		return v_len_sq;
	}
	...
	bool		CastRay(Vec3Arg inRayOrigin, Vec3Arg inRayDirection, float inTolerance, const A &inA, float &ioLambda)
	{
		...
		{
#ifdef MOSS_GJK_DEBUG
			MOSS_TRACE("v = [%s], num_points = %d", ConvertToString(v).c_str(), mNumPoints);
#endif
...
#ifdef MOSS_GJK_DEBUG
			MOSS_TRACE("v . w = %g", (double)v_dot_w);
#endif
			if (v_dot_w > 0.0f)
			{
...
#ifdef MOSS_GJK_DEBUG
				MOSS_TRACE("v . r = %g", (double)v_dot_r);
#endif
				if (v_dot_r >= 0.0f)
					return false;
				...
				lambda -= delta;
#ifdef MOSS_GJK_DEBUG
				MOSS_TRACE("lambda = %g, delta = %g", (double)lambda, (double)delta);
#endif
