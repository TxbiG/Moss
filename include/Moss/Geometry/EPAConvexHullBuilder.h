...
//#define MOSS_EPA_CONVEX_BUILDER_VALIDATE

// Define to draw the building of the hull for debugging purposes
//#define MOSS_EPA_CONVEX_BUILDER_DRAW
...
public:
	...
	using Edges = TStaticArray<Edge, cMaxEdgeLength>;
	using NewTriangles = TStaticArray<Triangle*, cMaxEdgeLength>;
	...
	// Typedefs
	using PointsBase = TStaticArray<Vec3, cMaxPoints>;
	using Triangles = TStaticArray<Triangle *, cMaxTriangles>;
	...
	void				FreeTriangle(Triangle *inT)
	{
		...
		MOSS_ASSERT(inT->mRemoved);
		for (const Edge &e : inT->mEdge)
			MOSS_ASSERT(e.mNeighbourTriangle == nullptr);
#endif

#if defined(MOSS_EPA_CONVEX_BUILDER_VALIDATE) || defined(MOSS_EPA_CONVEX_BUILDER_DRAW)
		// Remove from list of all triangles
		Triangles::iterator i = std::find(mTriangles.begin(), mTriangles.end(), inT);
		MOSS_ASSERT(i != mTriangles.end());
		mTriangles.erase(i);
#endif
...
private:
	...
	{
		MOSS_ASSERT(inEdge1 >= 0 && inEdge1 < 3);
		MOSS_ASSERT(inEdge2 >= 0 && inEdge2 < 3);
		Edge &e1 = inT1->mEdge[inEdge1];
		Edge &e2 = inT2->mEdge[inEdge2];

		// Check not connected yet
		MOSS_ASSERT(e1.mNeighbourTriangle == nullptr);
		MOSS_ASSERT(e2.mNeighbourTriangle == nullptr);

		// Check vertices match
		MOSS_ASSERT(e1.mStartIdx == inT2->GetNextEdge(inEdge2).mStartIdx);
		MOSS_ASSERT(e2.mStartIdx == inT1->GetNextEdge(inEdge1).mStartIdx);
	...
	void				UnlinkTriangle(Triangle *inT)
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			if (edge.mNeighbourTriangle != nullptr)
			{
				...
				MOSS_ASSERT(neighbour_edge.mNeighbourTriangle == inT);
				MOSS_ASSERT(neighbour_edge.mNeighbourEdge == i);
	...
	bool				FindEdge(Triangle *inFacingTriangle, Vec3Arg inVertex, Edges &outEdges)
	{
		...
		MOSS_ASSERT(inFacingTriangle->IsFacing(inVertex));
		...
		MOSS_ASSERT(outEdges.empty() || outEdges[0].mStartIdx == next_expected_start_idx);
	...
	void				ValidateTriangle(const Triangle *inT) const
	{
		if (inT->mRemoved)
		{
			...
			for (const Edge &my_edge : inT->mEdge)
				MOSS_ASSERT(my_edge.mNeighbourTriangle == nullptr);
		}
		else
		{
			for (int i = 0; i < 3; ++i)
			{
				...
				MOSS_ASSERT(nb != nullptr);

				if (nb != nullptr)
				{
					// Assert that our neighbours edge points to us
					const Edge &nb_edge = nb->mEdge[my_edge.mNeighbourEdge];
					MOSS_ASSERT(nb_edge.mNeighbourTriangle == inT);
					MOSS_ASSERT(nb_edge.mNeighbourEdge == i);
