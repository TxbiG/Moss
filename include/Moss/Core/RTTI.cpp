...
{
	MOSS_ASSERT(inDestructObject != nullptr, "Object cannot be destructed");
}
...
{
	MOSS_ASSERT(inOffset >= 0 && inOffset < mSize, "Base class not contained in derived class");
...
bool RTTI::operator == (const RTTI &inRHS) const
{
	...
	MOSS_ASSERT(strcmp(mName, inRHS.mName) != 0);
	return false;
}
