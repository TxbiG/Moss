...
private:
	...
	MOSS_INLINE void			SetControlValue(size_type inIndex, uint8 inValue)
	{
		MOSS_ASSERT(inIndex < mMaxSize);
	...
	MOSS_INLINE void			GetIndexAndControlValue(const Key &inKey, size_type &outIndex, uint8 &outControl) const
	{
		// Calculate hash
		...
		if (old_data != nullptr)
		{
			...
			for (size_type i = 0; i < old_max_size; ++i)
				if (old_control[i] & cBucketUsed)
				{
					...
					MOSS_IF_ENABLE_ASSERTS(bool inserted =) InsertKey</* InsertAfterGrow= */ true>(HashTableDetail::sGetKey(*element), index);
					MOSS_ASSERT(inserted);
	...
	bool					InsertKey(const Key &inKey, size_type &outIndex)
	{
		...
		if (mLoadLeft == 0)
		{
			...
			else
			{
				...
				{
					MOSS_ASSERT(false, "Overflow in hash table size, can't grow!");
					return false;
				}
...
public:
	...
	{
		MOSS_ASSERT(inIterator.IsValid());
			...
			if (mControl[src] == cBucketDeleted)
				for (;;)
				{
					...
					else
					{
						...
						MOSS_ASSERT(mControl[dst] == cBucketDeleted);
						SetControlValue(dst, src_control);
...
private:
	...
	static constexpr bool	cNeedsAlignedAllocate = alignof(KeyValue) > (MOSS_CPU_ADDRESS_BITS == 32? 8 : 16);
