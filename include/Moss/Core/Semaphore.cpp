...
Semaphore::Semaphore()
{
	...
	{
		MOSS_TRACE("Failed to create semaphore");
		std::abort();
	}
	...
	if (!MOSS_PLATFORM_BLUE_SEMAPHORE_INIT(mSemaphore))
	{
		MOSS_TRACE("Failed to create semaphore");
		std::abort();
	}
...
#elif defined(MOSS_PLATFORM_BLUE)
	MOSS_PLATFORM_BLUE_SEMAPHORE_DESTROY(mSemaphore);
#endif
}
...
void Semaphore::Release(uint inNumber)
{
	...
	#elif defined(MOSS_PLATFORM_BLUE)
		MOSS_PLATFORM_BLUE_SEMAPHORE_SIGNAL(mSemaphore, num_to_release);
	#endif
	}
#else
...
void Semaphore::Acquire(uint inNumber)
{
	...
	#elif defined(MOSS_PLATFORM_BLUE)
		MOSS_PLATFORM_BLUE_SEMAPHORE_WAIT(mSemaphore, num_to_acquire);
	#endif
	}
#else
