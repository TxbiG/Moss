...
public:
	...
	static constexpr bool	needs_aligned_allocate = alignof(T) > (MOSS_CPU_ADDRESS_BITS == 32? 8 : 16);
	...
	{
		MOSS_ASSERT(inNewSize > 0); // Reallocating to zero size is implementation dependent, so we don't allow it
...
namespace std
{
	...
	struct hash<String>
	{
		inline size_t operator () (const String &inRHS) const
		{
...
#endif // (!MOSS_PLATFORM_WINDOWS || MOSS_COMPILER_MINGW) && !MOSS_DISABLE_CUSTOM_ALLOCATOR
