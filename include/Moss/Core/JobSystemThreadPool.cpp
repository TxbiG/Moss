...
#include <Moss/Core/JobSystemThreadPool.h>
...
{
#if !defined(MOSS_CPU_WASM) || defined(__EMSCRIPTEN_PTHREADS__) // If we're running without threads support we cannot create threads and we ignore the inNumThreads parameter
...
JobHandle JobSystemThreadPool::CreateJob(const char *inJobName, ColorArg inColor, const JobFunction &inJobFunction, uint32 inNumDependencies)
{
	...
	for (;;)
	{
		...
		if (index != AvailableJobs::cInvalidObjectIndex)
			break;
		MOSS_ASSERT(false, "No jobs available!");
...
void JobSystemThreadPool::QueueJobs(Job **inJobs, uint inNumJobs)
{
...
#if !defined(MOSS_COMPILER_MINGW) // MinGW doesn't support __try/__except)
	...
	static void SetThreadName(const char* inName)
	{
		...
		MOSS_CLANG_SUPPRESS_WARNING("-Wcast-function-type") // error : cast from 'FARPROC' (aka 'long long (*)()') to 'SetThreadDescriptionFunc' (aka 'long (*)(void *, const wchar_t *)') converts to incompatible function type
		MOSS_CLANG_SUPPRESS_WARNING("-Wcast-function-type-strict") // error : cast from 'FARPROC' (aka 'long long (*)()') to 'SetThreadDescriptionFunc' (aka 'long (*)(void *, const wchar_t *)') converts to incompatible function type
		MOSS_MSVC_SUPPRESS_WARNING(4191) // reinterpret_cast' : unsafe conversion from 'FARPROC' to 'SetThreadDescriptionFunc'. Calling this function through the result pointer may cause your program to fail
...
#if !defined(MOSS_COMPILER_MINGW)
	...
	static void SetThreadName(const char *inName)
	{
		MOSS_ASSERT(strlen(inName) < 16); // String will be truncated if it is longer
		prctl(PR_SET_NAME, inName, 0, 0, 0);
	}
#endif // MOSS_PLATFORM_LINUX
...
#if defined(MOSS_PLATFORM_WINDOWS) || defined(MOSS_PLATFORM_LINUX)
	SetThreadName(name);
#endif // MOSS_PLATFORM_WINDOWS && !MOSS_COMPILER_MINGW

	// Enable floating point exceptions
	FPExceptionsEnable enable_exceptions;
	MOSS_UNUSED(enable_exceptions);

	MOSS_PROFILE_THREAD_START(name);
