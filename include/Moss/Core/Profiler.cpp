...
void Profiler::RemoveThread(ProfileThread *inThread)
{
	...
	TArray<ProfileThread *>::iterator i = std::find(mThreads.begin(), mThreads.end(), inThread);
	MOSS_ASSERT(i != mThreads.end());
	mThreads.erase(i);
}
...
void Profiler::sAggregate(int inDepth, uint32 inColor, ProfileSample *&ioSample, const ProfileSample *inEnd, Aggregators &ioAggregators, KeyToAggregator &ioKeyToAggregator)
{
	...
	{
		MOSS_ASSERT(sample[-1].mStartCycle <= sample->mStartCycle);
		MOSS_ASSERT(sample->mStartCycle >= ioSample->mStartCycle);
		MOSS_ASSERT(sample->mEndCycle <= ioSample->mEndCycle);
	...
	MOSS_ASSERT(sample[-1].mStartCycle <= ioSample->mEndCycle);
	MOSS_ASSERT(sample >= inEnd || sample->mStartCycle >= ioSample->mEndCycle);
	ioSample = sample - 1;
}
		...
		<script type="text/javascript">
			...
			function startChart()
			{
				...
				for (var t = 0; t < threads.length; t++)
				{
					...
					thread.darkened_color = new TArray(thread.color.length);
				...
				ctx.font = bar_font;
				aggregated.name_width = new TArray(aggregated.name.length);
