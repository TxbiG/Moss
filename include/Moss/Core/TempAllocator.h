...
public:
	...
	explicit						TempAllocatorImpl(size_t inSize) :
		mBase(static_cast<uint8 *>(AlignedAllocate(inSize, MOSS_RVECTOR_ALIGNMENT))),
		mSize(inSize)
	{
	}
	...
	virtual void *					Allocate(uint inSize) override
	{
		...
		{
			size_t new_top = mTop + AlignUp(inSize, MOSS_RVECTOR_ALIGNMENT);
			if (new_top > mSize)
			{
				MOSS_TRACE("TempAllocator: Out of memory trying to allocate %u bytes", inSize);
				std::abort();
			}
	...
	virtual void					Free(void *inAddress, uint inSize) override
	{
		...
		{
			mTop -= AlignUp(inSize, MOSS_RVECTOR_ALIGNMENT);
			if (mBase + mTop != inAddress)
			{
				MOSS_TRACE("TempAllocator: Freeing in the wrong order");
				std::abort();
			}
		}
	}
