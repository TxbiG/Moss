#include <Moss/Renderer/VK/VKTEST.h>
...
static VKAPI_ATTR VkBool32 VKAPI_CALL sVulkanDebugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT inSeverity, [[maybe_unused]] VkDebugUtilsMessageTypeFlagsEXT inType, const VkDebugUtilsMessengerCallbackDataEXT *inCallbackData, [[maybe_unused]] void *inUserData)
{
	...
	MOSS_ASSERT((inSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) == 0);
	return VK_FALSE;
}
...
void RendererVK::Initialize(Moss_Window* window) {
...
#elif defined(MOSS_PLATFORM_MACOS)
	...
	TArray<const char *> required_device_extensions;
	required_device_extensions.push_back(VK_KHR_SWAPCHAIN_EXTENSION_NAME);
#ifdef MOSS_PLATFORM_MACOS
	...
	// Create surface
    Moss_CreateWindowSurface(&window, mInstance, nullptr, &mSurface);
	...
	TArray<VkDescriptorSetLayout> layouts(cFrameCount, mDescriptorSetLayoutUBO);
...
void RendererVK::EndShadowPass()
{
	...
	MOSS_ASSERT(mImageIndex < mSwapChainFramebuffers.size());
...
{
	TArray<uint8> data = ReadData((String("Shaders/VK/") + inName + ".vert.spv").c_str());
...
{
	TArray<uint8> data = ReadData((String("Shaders/VK/") + inName + ".frag.spv").c_str());
...
void RendererVK::FreeBuffer(BufferVK &ioBuffer)
{
	...
	{
		MOSS_ASSERT(mFrameIndex < cFrameCount);
