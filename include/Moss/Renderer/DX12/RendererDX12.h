...
public:
	...
	unique_ptr<ConstantBufferDX12>	CreateConstantBuffer(uint32 inBufferSize);
...
private:
	...
	TArray<ComPtr<ID3D12Object>>		mDelayReleased[cFrameCount];		///< Objects that are potentially referenced by the GPU so can only be freed when the GPU finishes
