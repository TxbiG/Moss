...
void Moss_WindowResizeVK(Moss_Renderer* handle, uint32_t width, uint32_t height) {
    vkDeviceWaitIdle(handle->mDevice);

    // Destroy old swapchain resources
    for (auto framebuffer : handle->mSwapChainFramebuffers)
        vkDestroyFramebuffer(handle->mDevice, framebuffer, nullptr);

    for (auto view : handle->m_SwapChainImageViews)
        vkDestroyImageView(handle->mDevice, view, nullptr);

    vkDestroyImage(handle->mDevice, handle->m_DepthImage, nullptr);
    vkFreeMemory(handle->mDevice, handle->m_DepthImageMemory, nullptr);
    
    vkDestroySwapchainKHR(handle->mDevice, handle->m_SwapChain, nullptr);

    // Create new swapchain with new width/height
    handle->m_SwapChainExtent.width = width;
    handle->m_SwapChainExtent.height = height;
    CreateSwapChain(handle->mPhysicalDevice);

    // Recreate framebuffers and depth buffer
    CreateDepthResources(handle);
    CreateFramebuffers(handle);
}